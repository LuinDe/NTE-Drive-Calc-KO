...
def _equip_card(
    ...
    if is_discarded:
        status_labels.append(_status_label("폐기", "#ff7b72", "#ff7b72", "rgba(218,54,51,0.16)"))
    if is_duplicate_drive:
        status_labels.append(_status_label("중복", "#ffb86c", "#ff9d3d", "rgba(255,152,0,0.16)"))
    ...
    if replacement_callback:
        replacement_btn = QPushButton(str(replacement_text or ("최적화" if shape_id else "교체")))
