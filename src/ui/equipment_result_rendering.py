...
def _render_results(self, plan):
    ...
    if locked_roles:
        locked_label = QLabel(
            "잠긴 방안 유지:"
            + "、".join(locked_roles)
            + ". 해당 실제 장비는 점수·스탯 필터·풀이 전에 제외되어 이번에는 이 캐릭터들을 덮어쓰지 않습니다."
        )
    ...
    for role, p in plan.items():
        ...
        if board:
            gl.addWidget(self._section_label("퍼즐 청사진:"))
            bp_row = QHBoxLayout()
        ...
        if tape:
            ...
            gl.addWidget(self._section_label("카트리지:"))
            gl.addWidget(
                self._equip_card(
                    tape.set_name,
                    tape.main_stats,
            ...
            missing_core = QLabel(
                ...
                + str(
                    ...
                    or "현재 고정 스냅샷에 캐릭터 제약을 만족하면서 유일하게 분배할 수 있는 카트리지가 없습니다"
                )
                + "(드라이브 방안은 계속 저장할 수 있음)"
            )
        ...
        if drives:
            gl.addWidget(self._section_label(f"드라이브 ({len(drives)}개):"))
            for d in drives:
                score = d.role_scores.get(role, 0) if hasattr(d, "role_scores") else 0
                grade = self._calc_grade(score, d.area)
                mvp_tag = f" 👑{d.pick_order}순위" if getattr(d, "is_mvp", False) else ""
