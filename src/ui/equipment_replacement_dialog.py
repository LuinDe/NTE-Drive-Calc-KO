...
def show_equipment_replacement_dialog(
    ...
    *,
    ...
    header = QLabel(f"장착 캐릭터: {role_name}")
    ...
    comparison_group = QGroupBox("후보 장비와 현재 장비")
    ...
    comparison_text = QLabel("아래 후보 카드를 클릭해 비교하세요.")
    ...
    selected_placeholder = QLabel("아래 카드를 클릭하면\n여기에 비교 사본이 생성됩니다", selected_host)
    ...
    candidate_group = QGroupBox(f"후보 장비 ({len(candidates)}개)")
    ...
    def select(choice: EquipmentReplacementCard) -> None:
        ...
        comparison_text.setText(
            f"선택됨: {choice.item_view.get('display_name') or choice.key}\n"
            f"직접 피해 한계 이득: {current_direct_text} → {selected_direct_text}"
    ...
    for index, choice in enumerate(candidates):
        ...
        card.setToolTip("클릭해 선택하고 위에서 현재 장비와 비교")
