...
class MainWindowThemeMixin:
    ...
    def _prompt_restart_for_theme_change(self):
        ...
        box.setWindowTitle("재시작 후 적용")
        box.setText("테마를 바꾸려면 앱을 재시작해야 합니다. 지금 재시작하고 적용할까요?")
    ...
    def _restart_application_as_admin(self):
        ...
        if sys.platform == "win32":
            ...
            if result <= 32:
                QMessageBox.warning(self, "재시작 실패", "관리자 권한으로 앱을 재시작하지 못해 테마 설정을 취소했습니다.")
                return False
            QApplication.quit()
            return True
        QMessageBox.warning(self, "재시작 실패", "현재 시스템은 자동 관리자 재시작을 지원하지 않아 테마 설정을 취소했습니다.")
        return False
