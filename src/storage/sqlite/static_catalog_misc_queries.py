...
class StaticCatalogMiscQueriesMixin(StaticDataDaoMixinHost):
    ...
    def _pagination(limit: int, offset: int) -> tuple[int, int]:
        ...
        if not 1 <= normalized_limit <= _MAX_PAGE_SIZE:
            raise ValueError(f"limit은 1에서 {_MAX_PAGE_SIZE} 사이여야 합니다")
    ...
    def _search_term(query: str) -> tuple[str, str]:
        ...
        if len(normalized) > _MAX_QUERY_LENGTH:
            raise ValueError(f"검색어는 {_MAX_QUERY_LENGTH}자를 넘을 수 없습니다")
        escaped = (
    ...
    def search_catalog_entries(
        ...
        *,
        ...
        if entries_sql is None:
            raise ValueError(f"지원하지 않는 자료실 영역: {domain_key!r}")
    ...
    def get_source_trace(
        ...
        *,
        ...
        raise ValueError("source_row_id와 source_file_id 중 하나 이상을 제공해야 합니다")
    ...
    def get_equipment_catalog_detail(
        ...
        if not key:
            raise ValueError("entity_key는 비워 둘 수 없습니다")
        ...
        if entity_kind == "equipment_plan":
            try:
                ...
                raise ValueError("장비 청사진 key는 캐릭터 정식 ID여야 합니다") from exc
            return self.get_equipment_plan(character_id)
        if entity_kind == "graduation_template":
            try:
                character_id = int(key)
            except ValueError as exc:
                raise ValueError("졸업 템플릿 key는 캐릭터 정식 ID여야 합니다") from exc
            ...
            return row
        raise ValueError(f"지원하지 않는 장비 상세 유형: {entity_kind!r}")
    ...
    def get_effect_catalog_detail(
        ...
        if entity_kind == "combat_curve":
            ...
            if not separator:
                raise ValueError("전투 곡선 key가 유효하지 않음")
        ...
        if entity_kind == "gameplay_tag":
            ...
            if not separator:
                raise ValueError("Gameplay Tag key가 유효하지 않음")
            return self._one(
                """
        ...
        if entity_kind == "roguelike_modifier":
            ...
            return row
        raise ValueError(f"지원하지 않는 스킬 효과 상세 유형: {entity_kind!r}")
    ...
    def get_asset_catalog_detail(
        ...
        if entity_kind == "montage":
            ...
            return row
        raise ValueError(f"지원하지 않는 리소스 상세 유형: {entity_kind!r}")
    ...
    def _count_for(self, table: str, field: str, value: str) -> int:
        ...
        if (table, field) not in allowed:
            raise ValueError("지원하지 않는 관계 카운트")
    ...
    def list_asset_relations(
        ...
        *,
        ...
        if spec is None:
            raise ValueError("지원하지 않는 리소스 관계 유형")
