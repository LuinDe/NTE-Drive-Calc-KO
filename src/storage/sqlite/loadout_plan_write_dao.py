...
class LoadoutPlanWriteDaoMixin(UserDataDaoMixinHost):
    def save_loadout_plan(
        ...
        *,
        ...
        if not plan_name:
            raise UserDataValidationError("장착 방안 이름은 비워 둘 수 없습니다")
        raw_character_id = _integer(character_id, "character_id", minimum=1)
        raw_status = str(status).strip()
        if not raw_status:
            raise UserDataValidationError("장착 방안 상태는 비워 둘 수 없습니다")
        ...
        for ordinal, raw_assignment in enumerate(assignments):
            ...
            if kind not in ("module", "core"):
                raise UserDataValidationError("장착 항목 kind는 module 또는 core여야 합니다")
            if (serial, slot) in seen:
                raise UserDataValidationError("같은 장착 방안에서 같은 UID를 중복 사용할 수 없습니다")
            ...
            for coordinate in ("target_row", "target_column"):
                ...
                if value is not None and _integer(value, coordinate) not in range(1, 6):
                    raise UserDataValidationError(f"{coordinate}은(는) 1에서 5 사이여야 합니다")
            normalized.append((serial, slot, kind, assignment))
        if is_active:
            if source_snapshot_id is None:
                raise UserDataValidationError("활성 장착 방안은 출처 가방 스냅샷을 기록해야 합니다")
        ...
        try:
            ...
            if cursor.lastrowid is None:
                raise UserDataError("장비 세팅 방안 생성 후 plan_id가 반환되지 않았습니다")
    ...
    def _repair_active_loadout_plan_conflicts_in_transaction(
        ...
        *,
        ...
        for plan_id, removed_uids in removed_by_plan.items():
            ...
            if cursor.lastrowid is None:
                raise UserDataError("잔여 장비 세팅 방안 생성 후 plan_id가 반환되지 않았습니다")
        ...
        try:
            ...
            if duplicate is not None:
                raise UserDataValidationError(
                    "복구 후에도 여러 활성 방안이 차지한 장비가 있습니다"
                )
            connection.commit()
            return repaired
        except sqlite3.Error as exc:
            connection.rollback()
            raise UserDataError("활성 장착 방안의 장비 점유 충돌을 복구할 수 없습니다") from exc
    ...
    def replace_active_loadout_plans(
        ...
        for plan_index, raw_plan in enumerate(plans):
            ...
            if character_id in target_characters:
                raise UserDataValidationError("일괄 저장에서 같은 캐릭터를 중복해서 덮어쓸 수 없습니다")
            ...
            for ordinal, raw_assignment in enumerate(plan.get("assignments") or ()):
                ...
                if kind not in ("module", "core"):
                    raise UserDataValidationError(
                        "장착 항목 kind는 module 또는 core여야 합니다"
                    )
                uid = (slot, serial)
                ...
                if virtual:
                    if slot != 0 or serial <= 0:
                        raise UserDataValidationError(
                            "가상 자리 표시 장비는 slot=0인 양의 정수 가상 UID를 사용해야 합니다"
                        )
                elif slot <= 0 or serial <= 0:
                    raise UserDataValidationError(
                        "실제 장착 항목은 양의 정수 원본 UID를 사용해야 합니다"
                    )
                if uid in role_uids:
                    raise UserDataValidationError(
                        "같은 장착 방안에서 같은 UID를 중복 사용할 수 없습니다"
                    )
                if not virtual and uid in claimed_uids:
                    raise UserDataValidationError(
                        f"일괄 저장의 장비 UID {uid}이(가) 여러 캐릭터에게 동시에 분배되었습니다"
                    )
                role_uids.add(uid)
                if not virtual:
                ...
                for coordinate in ("target_row", "target_column"):
                    ...
                    if value is not None and _integer(value, coordinate) not in range(1, 6):
                        raise UserDataValidationError(
                            f"{coordinate}은(는) 1에서 5 사이여야 합니다"
                        )
                normalized_assignments.append(assignment)
            if not normalized_assignments:
                raise UserDataValidationError("각 활성 방안에는 장비가 최소 하나 필요합니다")
        ...
        if not normalized_plans:
            raise UserDataValidationError("저장할 장착 방안이 없습니다")
        ...
        try:
            ...
            for plan in normalized_plans:
                ...
                if slot is None or str(slot["slot_key"]) != "primary":
                    raise UserDataValidationError(
                        "활성 장비 세팅 호환 진입점은 주력 슬롯만 덮어쓸 수 있습니다. 예비 슬롯은 슬롯 저장 인터페이스를 사용하세요"
                    )
            ...
            for snapshot_id in {
                ...
                if (
                    ...
                    raise UserDataValidationError(
                        f"가방 스냅샷을 저장에 사용할 수 없습니다: {snapshot_id}"
                    )
            ...
            for plan in normalized_plans:
                ...
                for assignment in plan["assignments"]:
                    ...
                    if (inventory.get(uid) or {}).get("kind") != assignment["kind"]:
                        raise UserDataValidationError(
                            f"장비 UID {uid}이(가) 방안에 고정된 가방 스냅샷에 없습니다"
                        )
            affected_plans = [
                plan
            ...
            if duplicate is not None:
                raise UserDataValidationError(
                    "저장 후에도 여러 활성 방안이 차지한 장비 UID가 있습니다:"
            ...
            if isinstance(exc, UserDataValidationError):
                raise
            raise UserDataError("활성 장착 방안을 원자적으로 덮어쓸 수 없습니다") from exc
        except BaseException:
            connection.rollback()
            raise
