...
def _sqlite_diagnostics(error: sqlite3.Error) -> dict[str, object]:
    ...
    safe_message = (
        ...
        else "SQLite 원본 메시지는 공개하지 않습니다. 오류 코드와 실패 단계로 원인을 파악하세요"
    )
...
class InventorySnapshotSaveError(UserDataError):
    ...
    def __init__(
        ...
        *,
        ...
        sqlite_name = details.get("sqlite_errorname", "SQLite 오류 이름 미제공")
        sqlite_code = details.get("sqlite_errorcode", "未知")
        rollback_detail = (
            f"; 롤백 실패: {self.diagnostics['rollback_error']}"
            if rollback_error is not None else "; 롤백 완료"
        )
        super().__init__(
            f"가방 스냅샷을 가져올 수 없습니다: {self.error_code}; 단계={stage}; "
