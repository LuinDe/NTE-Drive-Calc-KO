...
class CustomCharacterDaoMixin:
    ...
    def save_custom_character_target_suit_id(
        ...
        try:
            ...
            if connection.execute(
                ...
                raise UserDataValidationError("사용자 정의 캐릭터가 없습니다")
            now = _utc_now()
            connection.execute(
            ...
            connection.rollback()
            raise UserDataError("사용자 정의 캐릭터 기본 세트 저장 실패") from exc
    ...
    def save_custom_character_shape_bonus(
        ...
        *,
        ...
        if len(label) > 40:
            raise UserDataValidationError("추가 형태 태그 길이는 1~40자여야 합니다")
        ...
        if len(values) > 1 or any(not key or value < 0 for key, value in values):
            raise UserDataValidationError("사용자 정의 캐릭터의 추가 형태는 유효한 속성 보너스 하나만 지원합니다")
        ...
        try:
            ...
            connection.rollback()
            raise UserDataError("사용자 정의 캐릭터 추가 형태 저장 실패") from exc
        except BaseException:
            connection.rollback()
            raise
    ...
    def save_custom_character_board_cells(self, character_id: int, cells: list[dict[str, Any]]) -> None:
        ...
        if len(cells) != 25 or len(coordinates) != 25 or len(enabled) != 20:
            raise UserDataValidationError("사용자 정의 보드는 25칸을 포함하고 그중 정확히 20칸이 활성화되어야 합니다")
        connection = self._db()
        try:
            connection.execute("BEGIN IMMEDIATE")
            ...
            connection.rollback()
            raise UserDataError("사용자 정의 캐릭터 보드를 저장할 수 없습니다") from exc
    ...
    def create_custom_character(
        ...
        if not name or len(name) > 40 or not alias or len(alias) > 40:
            raise UserDataValidationError("캐릭터 이름과 게임 내 이름은 1~40자여야 합니다")
        connection = self._db()
        try:
            connection.execute("BEGIN IMMEDIATE")
            ...
            if existing is not None:
                raise UserDataValidationError(f"사용자 정의 캐릭터 이름이 이미 있습니다: {name}")
            ...
            connection.rollback()
            raise UserDataError("사용자 정의 캐릭터를 만들 수 없습니다") from exc
        except BaseException:
            connection.rollback()
            raise
        return self._one(
    ...
    def delete_custom_character(self, character_id: int) -> None:
        ...
        try:
            ...
            if job_reference is not None:
                raise UserDataValidationError("이 캐릭터에 이미 고속 장착 작업 기록이 있어 방안과 작업 기록을 먼저 유지합니다")
            connection.execute(
            ...
            connection.rollback()
            raise UserDataError("사용자 정의 캐릭터 삭제 실패") from exc
        except BaseException:
            connection.rollback()
            raise
