...
class CharacterProfileDaoMixin(UserDataDaoMixinHost):
    ...
    def _preference_number(value: Any, label: str) -> float:
        if isinstance(value, bool):
            raise UserDataValidationError(f"{label}은(는) 유한한 수치여야 합니다")
        try:
            number = float(value)
        except (TypeError, ValueError) as exc:
            raise UserDataValidationError(f"{label}은(는) 유한한 수치여야 합니다") from exc
    ...
    def _validated_character_weight_rows(
        ...
        if not isinstance(properties, Sequence) or isinstance(properties, (str, bytes)):
            raise UserDataValidationError("캐릭터 스탯 가중치는 목록이어야 합니다")
        normalized = []
        ...
        for ordinal, source in enumerate(properties):
            ...
            if property_id in seen_ids:
                raise UserDataValidationError("캐릭터 스탯 가중치에 중복 property_id를 포함할 수 없습니다")
            ...
            if weight < 0 or main_weight < 0:
                raise UserDataValidationError("캐릭터 스탯 가중치는 0보다 작을 수 없습니다")
            normalized.append({
    ...
    def seed_character_weight_preferences(
        ...
        *,
        ...
        try:
            ...
            connection.rollback()
            raise UserDataError("캐릭터 스탯 가중치를 초기화할 수 없습니다") from exc
        ...
        *,
        ...
        try:
            ...
            if connection.execute(
                ...
                raise UserDataValidationError("캐릭터 스탯 가중치가 아직 정적 라이브러리에서 초기화되지 않았습니다")
            connection.execute(
            ...
            connection.rollback()
            raise UserDataError("캐릭터 스탯 가중치를 저장할 수 없습니다") from exc
        except BaseException:
            connection.rollback()
            raise
        result = self.get_character_weight_preferences(raw_character_id)
    ...
    def refresh_unmodified_character_weight_preferences(
        ...
        *,
        ...
        try:
            ...
            connection.rollback()
            raise UserDataError("수정되지 않은 캐릭터 스탯 가중치를 새로 고칠 수 없습니다") from exc
        except BaseException:
            connection.rollback()
            raise
    ...
    def save_character_shape_bonus_preferences(
        ...
        *,
        ...
        if len(label) > 100:
            raise UserDataValidationError("추가 형태 태그는 100자를 넘을 수 없습니다")
        if not isinstance(property_values, Mapping):
            raise UserDataValidationError("추가 형태 보너스는 객체여야 합니다")
        rows = []
        for ordinal, (raw_property_id, raw_value) in enumerate(property_values.items()):
            property_id = str(raw_property_id or "").strip()
            if not property_id:
                raise UserDataValidationError("추가 형태 보너스 property_id는 비워 둘 수 없습니다")
            value = self._preference_number(raw_value, "추가 형태 보너스 수치")
            if value < 0:
                raise UserDataValidationError("추가 형태 보너스 수치는 0보다 작을 수 없습니다")
            rows.append((raw_character_id, property_id, value, ordinal))
        if len({row[1] for row in rows}) != len(rows):
            raise UserDataValidationError("추가 형태 보너스에 중복 속성을 포함할 수 없습니다")
        connection = self._db()
        try:
            connection.execute("BEGIN IMMEDIATE")
            connection.execute(
            ...
            connection.rollback()
            raise UserDataError("캐릭터 추가 형태 보너스를 저장할 수 없습니다") from exc
        except BaseException:
            connection.rollback()
            raise
    ...
    def reset_character_profile(self, character_id: int) -> bool:
        ...
        try:
            ...
            connection.rollback()
            raise UserDataError("캐릭터 육성 포인터를 초기화할 수 없습니다") from exc
    ...
    def reset_all_character_profiles(self) -> int:
        ...
        try:
            ...
            connection.rollback()
            raise UserDataError("전체 캐릭터 육성 포인터를 초기화할 수 없습니다") from exc
    ...
    def save_character_profile(
        ...
        *,
        ...
        if raw_level > 80:
            raise UserDataValidationError("character_level은 80을 넘을 수 없습니다")
        raw_breakthrough = _integer(breakthrough_stage, "breakthrough_stage", minimum=0)
        if raw_breakthrough > 6:
            raise UserDataValidationError("breakthrough_stage는 6을 넘을 수 없습니다")
        ...
        if raw_awakening > 6:
            raise UserDataValidationError("awakening_level은 6을 넘을 수 없습니다")
        ...
        if raw_fork_id is None:
            ...
            if raw_fork_level > 80:
                raise UserDataValidationError("fork_level은 80을 넘을 수 없습니다")
            ...
            if raw_fork_breakthrough > 6:
                raise UserDataValidationError("fork_breakthrough_stage는 6보다 클 수 없습니다")
            ...
            if raw_refinement > 5:
                raise UserDataValidationError("fork_refinement_level은 5를 넘을 수 없습니다")
        ...
        for effect_id in selected_awaken_effect_ids or ():
            ...
            if normalized in normalized_awaken_effects:
                raise UserDataValidationError("selected_awaken_effect_ids는 중복될 수 없습니다")
        ...
        if selection_initialized and len(normalized_awaken_effects) != raw_awakening:
            raise UserDataValidationError("각성 레벨은 선택한 일반 각성 수와 같아야 합니다")
        ...
        try:
            ...
            connection.rollback()
            raise UserDataValidationError("캐릭터 페이지 순서 또는 육성 포인터가 잘못되었습니다") from exc
        except sqlite3.Error as exc:
            connection.rollback()
            raise UserDataError("캐릭터 육성 포인터를 저장할 수 없습니다") from exc
