...
def _json_object(value: Mapping[str, Any], label: str) -> str:
    try:
        ...
        raise UserDataValidationError(f"{label}을(를) 직렬화할 수 없습니다") from error
...
def _microseconds(value: Any, label: str) -> int | None:
    ...
    if not math.isfinite(normalized):
        raise UserDataValidationError(f"{label}은(는) 유한한 숫자여야 합니다")
...
def _plain_object(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise UserDataValidationError(f"{label}은(는) 객체여야 합니다")
    return dict(value)


def _integer(value: Any, label: str, *, minimum: int | None = None) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise UserDataValidationError(f"{label}은(는) 정수여야 합니다")
    if minimum is not None and value < minimum:
        raise UserDataValidationError(f"{label}은(는) {minimum}보다 작을 수 없습니다")
    return value
...
def _finite_number(
    ...
    *,
    ...
    if minimum is not None and normalized < minimum:
        raise UserDataValidationError(f"{label}은(는) {minimum:g}보다 작을 수 없습니다")
    if maximum is not None and normalized > maximum:
        raise UserDataValidationError(f"{label}은(는) {maximum:g}보다 클 수 없습니다")
    return normalized
