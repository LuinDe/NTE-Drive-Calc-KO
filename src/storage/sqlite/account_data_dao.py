...
class AccountDataDaoMixin(UserDataDaoMixinHost):
    def profile(self) -> dict[str, Any]:
        ...
        if profile is None:
            raise UserDataError("사용자 데이터베이스에 계정 정보가 없습니다")
    ...
    def list_application_setting_copies(self) -> dict[str, dict[str, Any]]:
        ...
        for row in self._rows(
            ...
            if not isinstance(value, dict):
                raise UserDataError(
                    f"계정 설정 사본 {row['setting_key']!r}이(가) JSON 객체가 아닙니다"
                )
    ...
    def replace_application_setting_copy(
        ...
        if not key:
            raise UserDataValidationError("setting_key는 비워 둘 수 없습니다")
        if not isinstance(value, Mapping):
            raise UserDataValidationError("계정 설정 사본은 객체여야 합니다")
        self._db().execute(
            """
    ...
    def get_ui_item_order(self, scope: str) -> list[str]:
        ...
        if not normalized_scope:
            raise UserDataValidationError("scope는 비워 둘 수 없습니다")
        return [
            str(row["item_key"])
            for row in self._rows(
                """
    ...
    def replace_ui_item_order(
        ...
        if any(not item_key for item_key in normalized_keys):
            raise UserDataValidationError("item_key는 비워 둘 수 없습니다")
        if len(set(normalized_keys)) != len(normalized_keys):
            raise UserDataValidationError("화면 항목 순서에 중복 항목을 포함할 수 없습니다")
        connection = self._db()
        now = _utc_now()
        try:
    ...
    def get_sync_settings(self) -> dict[str, Any]:
        ...
        if settings is None:
            raise UserDataError("사용자 데이터베이스에 동기화 설정이 없습니다")
    ...
    def update_sync_settings(
        ...
        *,
        ...
        if inventory_method not in SYNC_METHODS:
            raise UserDataValidationError("inventory_sync_method는 nte_core 또는 gamepad여야 합니다")
        if apply_method not in SYNC_METHODS:
            raise UserDataValidationError("equipment_apply_method는 nte_core 또는 gamepad여야 합니다")
        ...
        if settle <= 0:
            raise UserDataValidationError("inventory_settle_seconds는 0보다 커야 합니다")
        auto_start = (
