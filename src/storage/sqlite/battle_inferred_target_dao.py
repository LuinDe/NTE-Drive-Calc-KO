...
class BattleInferredTargetDaoMixin(UserDataDaoMixinHost):
    ...
    def save_battle_inferred_target_snapshot(
        ...
        *,
        ...
        if not algorithm:
            raise UserDataValidationError("대상 추론 알고리즘 버전은 비워 둘 수 없습니다")
        status = str(inference_status or "").strip()
        if status not in {"resolved", "unresolved"}:
            raise UserDataValidationError("대상 추론 상태가 잘못되었습니다")
        ...
        payload_json = _json_object(inferred_payload, "대상 추론 스냅샷")
        connection = self._db()
        try:
        ...
        if saved is None:
            raise RuntimeError("대상 추론 스냅샷이 저장 후 사라졌습니다")
        return saved
