...
class BattleAxisFinalizationDaoMixin(UserDataDaoMixinHost):
    ...
    def replace_staged_battle_axis(
        ...
        *,
        ...
        if not pages and reason is None:
            raise UserDataValidationError("완전한 최종 축에는 최소 한 페이지가 필요합니다")
        normalized_pages = [dict(page) for page in pages]
        if any(
            _optional_text(page.get("generation")) != generation
            for page in normalized_pages
        ):
            raise UserDataValidationError("최종 히트별 페이지에 서로 다른 generation이 있습니다")
        source_ids = {
            _required_text(page.get("battle_record_id"), "page.battle_record_id")
            for page in normalized_pages
        }
        if len(source_ids) > 1:
            raise UserDataValidationError("최종 히트별 페이지에 서로 다른 업스트림 전투 기록이 있습니다")
            ...
            if normalized_pages:
                ...
                for page in normalized_pages:
                    ...
                    if not isinstance(rows, Sequence) or isinstance(
                        ...
                        raise UserDataValidationError("최종 히트별 페이지 rows는 배열이어야 합니다")
                    for hit in rows:
                        if not isinstance(hit, Mapping):
                            raise UserDataValidationError("최종 히트별 페이지에 잘못된 히트가 있습니다")
    ...
    def discard_battle_axis_capture(self, capture_operation_id: str) -> bool:
        ...
        try:
            ...
            connection.rollback()
            raise UserDataError("완료되지 않은 전투의 히트별 기록을 정리할 수 없습니다") from error
