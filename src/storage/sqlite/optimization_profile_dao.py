...
class OptimizationProfileDaoMixin(CharacterProfileDaoMixin):
    ...
    def _validated_optimization_characters(
        ...
        if not isinstance(characters, Sequence) or isinstance(characters, (str, bytes)):
            raise UserDataValidationError("characters는 캐릭터 선호도 목록이어야 합니다")
        ...
        for index, value in enumerate(characters):
            ...
            if character_id in seen_character_ids:
                raise UserDataValidationError("characters에 중복 character_id를 포함할 수 없습니다")
            if ordinal in seen_ordinals:
                raise UserDataValidationError("characters에 중복 ordinal을 포함할 수 없습니다")
            ...
            if suit_requirement_mode not in SUIT_REQUIREMENT_MODES:
                raise UserDataValidationError("suit_requirement_mode는 none, two_piece 또는 four_piece여야 합니다")
            ...
            if suit_requirement_mode != "none" and target_suit_id is None:
                raise UserDataValidationError(
                    "two_piece 또는 four_piece 세트 제약에는 target_suit_id를 제공해야 합니다"
                )
            weights_source = row.get("property_weights", {})
            if not isinstance(weights_source, Mapping):
                raise UserDataValidationError(f"characters[{index}].property_weights는 객체여야 합니다")
            ...
            if not isinstance(priorities_source, Sequence) or isinstance(priorities_source, (str, bytes)):
                raise UserDataValidationError(f"characters[{index}].substat_priorities는 목록이어야 합니다")
            priorities: list[str] = []
            for property_id in priorities_source:
                ...
                if normalized_property_id in priorities:
                    raise UserDataValidationError("substat_priorities에 중복 property_id를 포함할 수 없습니다")
            ...
            if not isinstance(blacklist_source, Sequence) or isinstance(blacklist_source, (str, bytes)):
                raise UserDataValidationError(f"characters[{index}].substat_blacklist는 목록이어야 합니다")
            blacklist: list[str] = []
            for property_id in blacklist_source:
                ...
                if normalized_property_id in blacklist:
                    raise UserDataValidationError("substat_blacklist에 중복 property_id를 포함할 수 없습니다")
            ...
            if not isinstance(equal_priority, bool):
                raise UserDataValidationError(
                    f"characters[{index}].equal_priority는 불리언이어야 합니다"
                )
            if not isinstance(ignore_grade_limit, bool):
                raise UserDataValidationError(
                    f"characters[{index}].ignore_grade_limit는 불리언이어야 합니다"
                )
            if not isinstance(blacklist_zero_weight, bool):
                raise UserDataValidationError(
                    f"characters[{index}].blacklist_zero_weight는 불리언이어야 합니다"
                )
            min_grade_limit = str(row.get("min_grade_limit") or "A").upper()
            if min_grade_limit not in cls._GRADE_LIMITS:
                raise UserDataValidationError(
                    f"characters[{index}].min_grade_limit가 잘못되었습니다"
                )
            ...
            if crit_threshold is not None and not 0.0 <= crit_threshold <= 100.0:
                raise UserDataValidationError(
                    f"characters[{index}].crit_threshold는 0에서 100 사이여야 합니다"
                )

            limits_source = row.get("property_limits", {})
            if not isinstance(limits_source, Mapping):
                raise UserDataValidationError(f"characters[{index}].property_limits는 객체여야 합니다")
            ...
            for property_id, bounds in limits_source.items():
                ...
                if minimum is None and maximum is None:
                    raise UserDataValidationError(f"{normalized_property_id}에는 minimum 또는 maximum이 최소 하나 필요합니다")
                ...
                if minimum_number is not None and maximum_number is not None and minimum_number > maximum_number:
                    raise UserDataValidationError(f"{normalized_property_id}의 minimum은 maximum보다 클 수 없습니다")
    ...
    def _optimization_strategy(value: Any) -> str:
        ...
        if strategy not in ALLOCATION_STRATEGIES:
            raise UserDataValidationError("allocation_strategy가 잘못되었습니다")
        return strategy
    ...
    def _insert_optimization_profile_version(
        ...
        if cursor.lastrowid is None:
            raise UserDataError("최적화 프로필 버전 생성 후 profile_version_id가 반환되지 않았습니다")
    ...
    def create_optimization_profile(
        ...
        *,
        ...
        try:
            ...
            if cursor.lastrowid is None:
                raise UserDataError("최적화 프로필 생성 후 profile_id가 반환되지 않았습니다")
            ...
            connection.rollback()
            raise UserDataValidationError(f"최적화 선호도 프로필이 이미 있습니다: {profile_name}") from exc
        except sqlite3.Error as exc:
            connection.rollback()
            raise UserDataError("최적화 선호도 프로필을 만들 수 없습니다") from exc
    ...
    def create_optimization_profile_version(
        ...
        *,
        ...
        try:
            ...
            if profile is None:
                raise UserDataValidationError("최적화 선호도 프로필이 없거나 비활성화되었습니다")
            ...
            connection.rollback()
            raise UserDataError("최적화 선호도 버전을 저장할 수 없습니다") from exc
        except BaseException:
            connection.rollback()
            raise
