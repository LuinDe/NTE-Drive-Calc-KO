...
class InventorySnapshotDaoMixin(UserDataDaoMixinHost):
    ...
    def _validated_item(item: Any, index: int) -> tuple[dict[str, Any], int, int]:
        ...
        if kind not in ("module", "core"):
            raise UserDataValidationError(f"items[{index}].kind는 module 또는 core여야 합니다")
        if not isinstance(row.get("item_id"), str) or not row["item_id"]:
            raise UserDataValidationError(f"items[{index}].item_id는 비워 둘 수 없습니다")
        for field in ("level", "max_level"):
            _integer(row.get(field), f"items[{index}].{field}", minimum=0)
        if row["max_level"] < row["level"]:
            raise UserDataValidationError(f"items[{index}].max_level은 level보다 작을 수 없습니다")
        for field in ("locked", "equipped"):
            if not isinstance(row.get(field), bool):
                raise UserDataValidationError(f"items[{index}].{field}는 불리언이어야 합니다")
    ...
    def _validated_stats(item: Mapping[str, Any], item_index: int) -> list[tuple[str, int, dict[str, Any]]]:
        ...
        for source_field, stat_group in (("main_stats", "main"), ("sub_stats", "sub")):
            ...
            if not isinstance(stats, list):
                raise UserDataValidationError(f"items[{item_index}].{source_field}는 배열이어야 합니다")
            for ordinal, raw_stat in enumerate(stats):
                ...
                if not isinstance(stat.get("property_id"), str) or not stat["property_id"]:
                    raise UserDataValidationError("가방 스탯 property_id는 비워 둘 수 없습니다")
                value = stat.get("value")
                if isinstance(value, bool) or not isinstance(value, (int, float)):
                    raise UserDataValidationError("가방 스탯 value는 숫자여야 합니다")
                if not isinstance(stat.get("percent"), bool):
                    raise UserDataValidationError("가방 스탯 percent는 불리언이어야 합니다")
    ...
    def import_inventory_snapshot(
        ...
        *,
        ...
        if source not in SNAPSHOT_SOURCES:
            raise UserDataValidationError(f"지원하지 않는 가방 데이터 출처: {source}")
        original, payload = self._snapshot_payload(snapshot)
        if payload.get("complete") is not True:
            raise UserDataValidationError("완전한 가방 스냅샷만 현재 데이터로 설정할 수 있습니다")
        items = payload.get("items")
        if not isinstance(items, list):
            raise UserDataValidationError("스냅샷 items는 배열이어야 합니다")
        ...
        if declared_count != len(items):
            raise UserDataValidationError(
                f"스냅샷 item_count는 {declared_count}이지만 items는 실제로 {len(items)}건입니다"
            )
        ...
        for index, raw_item in enumerate(items):
            ...
            uid = (serial, slot)
            if uid in seen_uids:
                raise UserDataValidationError(f"가방 UID 중복: serial={serial}, slot={slot}")
            seen_uids.add(uid)
        ...
        if not isinstance(raw_characters, list):
            raise UserDataValidationError("스냅샷 characters는 배열이어야 합니다")
        ...
        if declared_character_count is not None:
            if _integer(declared_character_count, "snapshot character_count", minimum=0) != len(raw_characters):
                raise UserDataValidationError(
                    f"스냅샷 character_count는 {declared_character_count}이지만 characters는 실제로 {len(raw_characters)}건입니다"
                )
        ...
        for index, raw_character in enumerate(raw_characters):
            ...
            if character_id in seen_character_ids or (character_slot, character_serial) in seen_character_uids:
                raise UserDataValidationError("스냅샷에 중복 캐릭터 인스턴스가 있습니다")
        ...
        try:
            ...
            if cursor.lastrowid is None:
                raise UserDataError("가방 스냅샷 생성 후 snapshot_id가 반환되지 않았습니다")
    ...
    def prune_inventory_snapshots(
        ...
        *,
        ...
        try:
            ...
            connection.rollback()
            raise UserDataError("기록 가방 스냅샷을 정리할 수 없습니다") from exc
    ...
    def export_inventory_snapshot(
        ...
        try:
            ...
            if summary is None:
                raise UserDataValidationError(f"가방 스냅샷이 없습니다: {raw_snapshot_id}")
            items = self.list_inventory_items(raw_snapshot_id)
            if int(summary["stored_item_count"]) != len(items):
                raise UserDataError(
                    f"가방 스냅샷 {raw_snapshot_id}의 아이템 수가 일치하지 않습니다:"
                    f"요약={summary['stored_item_count']}, 항목={len(items)}"
                )
            connection.rollback()
            return summary, items
        except sqlite3.Error as exc:
            connection.rollback()
            raise UserDataError(f"가방 스냅샷을 원자적으로 읽을 수 없습니다: {raw_snapshot_id}") from exc
    ...
    def list_inventory_items(
        ...
        *,
        ...
        if kind not in (None, "module", "core"):
            raise UserDataValidationError("kind는 module, core 또는 None이어야 합니다")
