...
class SharedDataDao:
    ...
    def __init__(self, database_path: str | Path) -> None:
        ...
        try:
            ...
            raise SharedDataError(
                f"공용 오버라이드 데이터베이스를 열 수 없습니다: {self.database_path}"
            ) from exc
    ...
    def _db(self) -> sqlite3.Connection:
        if self._connection is None:
            raise SharedDataError("공용 오버라이드 데이터베이스 DAO가 닫혔습니다")
    ...
    def _initialize_or_migrate(self) -> None:
        ...
        if not self._table_exists("schema_migration"):
            try:
                ...
                connection.rollback()
                raise SharedDataError("공용 오버라이드 데이터베이스를 초기화할 수 없습니다") from exc
        ...
        if version > SCHEMA_VERSION:
            raise SharedDataError(
                f"공용 오버라이드 데이터베이스 버전 {version}이(가) 프로그램 지원 버전 {SCHEMA_VERSION}보다 높습니다"
            )
        for target_version in range(version + 1, SCHEMA_VERSION + 1):
            migration = MIGRATIONS.get(target_version)
            if migration is None:
                raise SharedDataError(f"공용 오버라이드 데이터베이스 마이그레이션 v{target_version}이(가) 없습니다")
            try:
                connection.execute("BEGIN IMMEDIATE")
                ...
                connection.rollback()
                raise SharedDataError(
                    f"공용 오버라이드 데이터베이스 마이그레이션 v{target_version} 실패"
                ) from exc
    ...
    def _validate_schema(self) -> None:
        ...
        if row is None or int(row["version"] or 0) != SCHEMA_VERSION:
            raise SharedDataError("공용 오버라이드 데이터베이스 구조 버전이 불완전합니다")
        ...
        if profile is None or profile["database_kind"] != "app_shared":
            raise SharedDataError("파일이 NTE 로컬 공용 오버라이드 데이터베이스가 아닙니다")
    ...
    def upsert_shape_bonus_override(
        ...
        *,
        ...
        key = str(logical_character_key or "").strip()
        if not key:
            raise ValueError("logical_character_key는 비워 둘 수 없습니다")
        if int(representative_character_id) <= 0:
            raise ValueError("representative_character_id는 양의 정수여야 합니다")
        if not str(shape_label or "").strip() or int(shape_grid_count) <= 0:
            raise ValueError("추가 형태 태그와 칸 수가 잘못되었습니다")
        ...
        for ordinal, raw in enumerate(properties):
            ...
            if not property_id or display_value < 0:
                raise ValueError("추가 형태 오버라이드에 잘못된 속성 또는 수치가 있습니다")
            if property_id in seen_properties:
                raise ValueError(f"추가 형태 오버라이드에 중복 속성이 있습니다: {property_id}")
        ...
        try:
            ...
            connection.rollback()
            raise SharedDataError("공용 추가 형태 오버라이드를 저장할 수 없습니다") from exc
    ...
    def delete_shape_bonus_override(self, logical_character_key: str) -> bool:
        ...
        try:
            ...
            connection.rollback()
            raise SharedDataError("공용 추가 형태 오버라이드를 삭제할 수 없습니다") from exc
    ...
    def record_migration(
        ...
        *,
        ...
        try:
            ...
            raise SharedDataError("공용 오버라이드 데이터 마이그레이션을 기록할 수 없습니다") from exc
        ...
        *,
        ...
        for raw in records:
            ...
            if not key or representative_id <= 0 or not shape_label or grid_count <= 0:
                raise ValueError(f"이전 버전 추가 형태 기록이 잘못되었습니다: {key or '<empty>'}")
            ...
            for ordinal, property_row in enumerate(raw.get("properties") or ()):
                ...
                if not property_id or value < 0 or property_id in seen:
                    raise ValueError(f"이전 버전 추가 형태 속성이 잘못되었습니다: {key}")
        ...
        try:
            ...
            connection.rollback()
            raise SharedDataError("이전 버전 공용 오버라이드 차이 마이그레이션 실패") from exc
        return inserted
