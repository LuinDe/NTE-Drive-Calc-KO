...
class StaticGameDataTerminologyQueriesMixin(_TerminologyQuerySource):
        ...
        *,
        ...
        if kind == "item":
            ...
            names = self._decode_names(
                row.get("names_json"),
                description=f"아이템 {canonical_id!r}",
            )
            return self._record(
                entity_kind="item",
        ...
        if kind in {"item_quality", "item_quality_color"}:
            ...
            names = self._decode_names(
                row.get("names_json"),
                description=f"아이템 품질 {requested_id!r}",
            )
            return self._record(
                entity_kind=kind,
        ...
        for row in rows:
            ...
            if title is None:
                self._raise_static_data_error(
                    f"아크 한정 뽑기 풀 {row['pool_id']!r}에 중앙 제목 용어가 없습니다"
                )
    ...
    def _decode_names(self, raw_names: Any, *, description: str) -> dict[str, str]:
        try:
            ...
            self._raise_static_data_error(
                f"{description}의 현지화 이름이 유효한 JSON이 아닙니다"
            )
        if not isinstance(names, dict):
            self._raise_static_data_error(
                f"{description}의 현지화 이름은 객체여야 합니다"
            )
        return {
            str(locale): str(name)
