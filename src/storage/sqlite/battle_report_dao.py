...
def _non_negative_number(value: Any, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise UserDataValidationError(f"{label}은(는) 숫자여야 합니다")
    number = float(value)
    if not math.isfinite(number) or number < 0:
        raise UserDataValidationError(f"{label}은(는) 유한한 음이 아닌 수여야 합니다")
    return number
...
def _optional_sha256(value: Any, label: str) -> str | None:
    ...
    if len(normalized) != 64:
        raise UserDataValidationError(f"{label}은(는) 64자리 SHA-256이어야 합니다")
    try:
        int(normalized, 16)
    except ValueError as error:
        raise UserDataValidationError(f"{label}은(는) 16진수 SHA-256이어야 합니다") from error
...
class BattleReportDaoMixin(UserDataDaoMixinHost):
    ...
    def _normalized_history_row(row: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(character_ids, list) or any(
            ...
            raise UserDataError("전투 리포트 캐릭터 인덱스가 손상되었습니다")
        ...
        *,
        ...
        if context_kind not in {"abyss", "non_abyss"}:
            raise UserDataValidationError(
                "combat_context_kind는 abyss 또는 non_abyss여야 합니다"
            )
        normalized_floor = (
            None
        ...
        if context_kind == "non_abyss" and normalized_floor is not None:
            raise UserDataValidationError("심연이 아닌 전투 리포트에는 abyss_floor를 저장할 수 없습니다")
        if bool(abyss_detected) != (context_kind == "abyss"):
            raise UserDataValidationError("심연 인식 상태가 전투 컨텍스트와 일치하지 않습니다")
        ...
        try:
            ...
            raise UserDataValidationError("raw_summary_json이 유효한 JSON이 아닙니다") from error
        if not isinstance(decoded_payload, Mapping):
            raise UserDataValidationError("raw_summary_json의 최상위는 객체여야 합니다")
        ...
        if supplied_sha256 != expected_sha256:
            raise UserDataValidationError("raw_summary_sha256이 원본 JSON과 일치하지 않습니다")
        ...
        try:
            ...
            if existing is not None:
                if str(existing["raw_summary_sha256"]) != supplied_sha256:
                    raise UserDataValidationError(
                        "같은 capture_operation_id에 다른 전투 리포트 payload가 대응됩니다"
                    )
                ...
                if record is None:
                    raise UserDataError("멱등 전투 리포트 기록이 제출 후 존재하지 않습니다")
                return {
                    "record": record,
                    "inserted": False,
            ...
            if cursor.lastrowid is None:
                raise UserDataError("전투 리포트 생성 후 battle_record_id가 반환되지 않았습니다")
        ...
        record = self._battle_record_history_row(record_id)
        if record is None:
            raise UserDataError("전투 리포트가 제출 후 존재하지 않습니다")
        return {
            "record": record,
            "inserted": True,
    ...
    def list_battle_records(
        ...
        *,
        ...
        if normalized_limit > BATTLE_REPORT_MAX_RECORDS:
            raise UserDataValidationError(
                f"limit은 {BATTLE_REPORT_MAX_RECORDS}을(를) 넘을 수 없습니다"
            )
    ...
    def promote_battle_record_to_manual(
        ...
        try:
            ...
            if retention is None:
                raise UserDataValidationError("저장할 전투 리포트가 없습니다")
        ...
        if record is None:
            raise UserDataError("수동 저장한 현재 전투 리포트가 예기치 않게 제거되었습니다")
        return {
            "record": record,
            "changed": changed,
            "pruned_battle_record_ids": tuple(pruned_ids),
    ...
    def unmark_manual_battle_record(
        ...
        try:
            ...
            if retention is None:
                raise UserDataValidationError("저장을 취소할 전투 리포트가 없습니다")
        ...
        record = self._battle_record_history_row(normalized_id)
        if record is None:
            raise UserDataError("수동 저장 취소 후 전투 리포트가 존재하지 않습니다")
    ...
    def update_battle_report_analysis_state(
        ...
        *,
        ...
        if (start_us is None) != (end_us is None):
            raise UserDataValidationError("분석 구간의 시작과 끝은 모두 비우거나 모두 지정해야 합니다")
        if start_us is not None:
            normalized_start = _integer(start_us, "start_us", minimum=0)
            normalized_end = _integer(end_us, "end_us", minimum=1)
            if normalized_end <= normalized_start:
                raise UserDataValidationError("분석 구간의 끝은 시작보다 늦어야 합니다")
        else:
    ...
    def _prune_oldest_battle_records(
        ...
        *,
        ...
        if len(record_ids) != excess:
            raise UserDataError("전투 리포트 보관 상한을 넘었지만 제거할 수 있는 기록이 충분하지 않습니다")
