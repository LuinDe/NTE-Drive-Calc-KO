...
def _selected_target_profiles(value: object) -> list[dict[str, Any]]:
    ...
    if not isinstance(value, (list, tuple)):
        raise UserDataValidationError("대상 속성 프로필은 배열이어야 합니다")
    result = []
    for ordinal, raw in enumerate(value, start=1):
        if not isinstance(raw, dict):
            raise UserDataValidationError(f"{ordinal}번째 대상 속성 프로필은 객체여야 합니다")
        ...
        if not static_target_id or not selection_target_id:
            raise UserDataValidationError(f"{ordinal}번째 대상 속성 프로필에 정식 ID가 없습니다")
        resistances = raw.get("resistances") or {}
        if not isinstance(resistances, dict):
            raise UserDataValidationError(f"{ordinal}번째 대상 속성 프로필의 저항은 객체여야 합니다")
        result.append({
            ...
            "max_hp": _finite_number(
                raw.get("max_hp"), f"{ordinal}번째 대상 최대 HP", minimum=1.0
            ),
...
class BattleAnalysisDaoMixin(UserDataDaoMixinHost):
    ...
    def save_battle_target_condition(
        ...
        if not target_name:
            raise UserDataValidationError("대상 이름은 비워 둘 수 없습니다")
        scene = str(condition.get("scene") or "").strip()
        if scene not in {"outer_realm", "open_world"}:
            raise UserDataValidationError("적 장면은 궤외지경 또는 오픈 월드여야 합니다")
        ...
        if environment_kind not in {
            ...
            raise UserDataValidationError("전투 환경 유형이 잘못되었습니다")
        ...
        if not isinstance(raw_target_ids, (list, tuple)):
            raise UserDataValidationError("선택한 대상은 배열이어야 합니다")
        ...
        witch_value = (
            ...
            else _finite_number(raw_witch_value, "마녀의 축복 수치")
        )
        ...
        if bool(witch_buff_id) != bool(witch_property and witch_value is not None):
            raise UserDataValidationError("마녀의 축복 ID, 속성, 수치는 함께 있어야 합니다")
        if witch_buff_id and not witch_buff_name:
            raise UserDataValidationError("마녀의 축복 이름은 비워 둘 수 없습니다")
        resistances = condition.get("resistances")
        if not isinstance(resistances, dict):
            raise UserDataValidationError("적 속성별 저항은 객체여야 합니다")
        values = {
        ...
        try:
            ...
            if connection.execute(
                ...
                raise UserDataValidationError("전투 리포트 기록이 존재하지 않습니다")
            connection.execute(
                """
        ...
        if saved is None:
            raise RuntimeError("적 조건이 저장 후 사라졌습니다")
        return saved
