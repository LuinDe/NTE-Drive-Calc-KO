...
class StaticCatalogFormulaQueries:
    ...
    def __init__(self, database_path: str | Path | None = None) -> None:
        ...
        try:
            ...
            raise StaticGameDataError(
                "공식 자료에 필요한 정적 데이터베이스를 읽기 전용으로 열 수 없습니다"
            ) from exc
        ...
        try:
            ...
            self.close()
            raise StaticGameDataError("정적 데이터베이스에 공식 자료 schema가 없습니다") from exc
    ...
    def _scalar(self, sql: str, parameters: tuple[object, ...] = ()) -> object:
        if self._connection is None:
            raise StaticGameDataError("공식 자료 정적 조회가 이미 닫혔습니다")
        row = self._connection.execute(sql, parameters).fetchone()
        if row is None:
            raise StaticGameDataError("공식 자료 정적 조회가 기록을 반환하지 않았습니다")
        return row[0]
    ...
    def _table_count(self, table: str) -> int:
        ...
        try:
            ...
            raise StaticGameDataError("공식 자료가 등록되지 않은 정적 테이블을 요청했습니다") from exc
    ...
    def evidence_snapshot(self) -> StaticFormulaEvidenceSnapshot:
        ...
        if dataset is None:
            raise StaticGameDataError("정적 데이터베이스에 dataset 메타 정보가 없습니다")
    ...
    def reaction_damage_curves(self) -> tuple[StaticReactionCurve, ...]:
        ...
        for effect_id, points in grouped.items():
            if tuple(tier for tier, _value in points) != tuple(range(16)):
                raise StaticGameDataError(
                    f"사이클 곡선 {effect_id!r}에 완전한 원본 구간 0–15가 없습니다"
                )
