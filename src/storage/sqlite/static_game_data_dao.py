...
def resolve_static_database(database_path: str | Path | None = None) -> Path:
    ...
    if database_path is not None:
        ...
        if resolved.is_file():
            return resolved
        raise StaticGameDataError(f"정적 데이터베이스가 없습니다: {resolved}")
    for candidate in static_database_candidates():
        if candidate.is_file():
            return candidate
    checked = "、".join(str(path) for path in static_database_candidates())
    raise StaticGameDataError(f"정적 데이터베이스를 찾을 수 없습니다. 확인한 위치: {checked}")
...
class StaticGameDataDao(
    ...
    def _rows(self, sql: str, parameters: Iterable[Any] = ()) -> list[dict[str, Any]]:
        if self._connection is None:
            raise StaticGameDataError("정적 데이터베이스 DAO가 닫혔습니다")
    ...
    def summary(self) -> dict[str, Any]:
        ...
        if dataset is None:
            raise StaticGameDataError("정적 데이터베이스에 데이터셋 메타 정보가 없습니다")
        counts = {}
    ...
    def application_setting_defaults(self) -> dict[str, dict[str, Any]]:
        ...
        for row in self._rows(
            ...
            try:
                ...
                raise StaticGameDataError(
                    f"정적 설정 기본값 {row['setting_key']!r}이(가) 유효한 JSON이 아닙니다"
                ) from exc
            if not isinstance(value, dict):
                raise StaticGameDataError(
                    f"정적 설정 기본값 {row['setting_key']!r}이(가) JSON 객체가 아닙니다"
                )
