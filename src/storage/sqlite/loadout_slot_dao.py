...
class LoadoutSlotDaoMixin(UserDataDaoMixinHost):
        ...
        try:
            ...
            if int(visible[0] if visible is not None else 0) >= MAX_VISIBLE_LOADOUT_SLOTS:
                raise UserDataValidationError(
                    f"캐릭터마다 장비 세팅 슬롯을 최대 {MAX_VISIBLE_LOADOUT_SLOTS}개 만들 수 있습니다"
                )
            ...
            if cursor.lastrowid is None:
                raise UserDataError("장비 세팅 슬롯 생성 후 slot_id가 반환되지 않았습니다")
            connection.commit()
            ...
            if isinstance(exc, UserDataValidationError):
                raise
            raise UserDataError("장비 세팅 슬롯을 만들 수 없습니다") from exc
    ...
    def archive_loadout_slot(self, slot_id: int) -> bool:
        ...
        if current_plan.get("allocation_locked"):
            raise UserDataValidationError("잠긴 방안이 있는 슬롯은 보관 처리할 수 없습니다. 먼저 잠금을 해제하세요")
        connection = self._db()
        try:
            connection.execute("BEGIN IMMEDIATE")
            now = _utc_now()
            if slot["slot_key"] == PRIMARY_LOADOUT_SLOT_KEY:
                ...
                if replacement is None:
                    raise UserDataValidationError("장비 세팅 슬롯을 최소 하나 남긴 뒤에야 기본 슬롯을 삭제할 수 있습니다")
                connection.execute(
                    """
                    UPDATE role_loadout_slot
    ...
    def save_replacement_plan_to_slot(
        ...
        *,
        ...
        if source_snapshot_id is None:
            raise UserDataValidationError("교체 최적화는 출처 가방 스냅샷을 유지해야 합니다")
        ...
        for row in current_slots:
            ...
            if owner_plan.get("allocation_locked"):
                raise UserDataValidationError(
                    "잠긴 슬롯 방안의 장비는 대여할 수 없습니다. 먼저 원래 슬롯의 잠금을 해제하세요"
                )
            owners.append({"slot": owner_slot, "plan": owner_plan, "removed_uids": removed_uids})

        connection = self._db()
        if connection.in_transaction:
            raise UserDataError("교체 최적화 저장은 트랜잭션을 중첩할 수 없습니다")
        try:
            connection.execute("BEGIN IMMEDIATE")
    ...
    def save_plans_to_slots(
        ...
        if not plans:
            raise UserDataValidationError("슬롯 방안을 최소 하나 저장해야 합니다")
        slot_ids = [
            _integer(row.get("slot_id"), "slot_id", minimum=1)
            for row in plans
        ]
        if len(set(slot_ids)) != len(slot_ids):
            raise UserDataValidationError("같은 저장에서 같은 장비 세팅 슬롯을 중복해서 덮어쓸 수 없습니다")
        connection = self._db()
        if connection.in_transaction:
            raise UserDataError("장비 세팅 슬롯 일괄 저장은 트랜잭션을 중첩할 수 없습니다")
        try:
            connection.execute("BEGIN IMMEDIATE")
            plan_ids = tuple(
    ...
    def _resolve_loadout_slot_id(
        ...
        *,
        ...
        if slot is None:
            raise UserDataValidationError("장비 세팅 슬롯이 이 캐릭터의 것이 아니거나 이미 보관 처리되었습니다")
    ...
    def _ensure_primary_loadout_slot(
        ...
        *,
        ...
        if cursor.lastrowid is None:
            raise UserDataError("기본 장비 세팅 슬롯 생성 후 slot_id가 반환되지 않았습니다")
        slot = {
    ...
    def _normalize_slot_name(name: str) -> str:
        ...
        if not value or len(value) > 24:
            raise UserDataValidationError("장비 세팅 슬롯 이름은 1~24자여야 합니다")
        return value
    ...
    def _normalize_slot_key(value: str) -> str:
        ...
        if not key or len(key) > 40 or any(char not in "abcdefghijklmnopqrstuvwxyz0123456789-_" for char in key):
            raise UserDataValidationError("장비 세팅 슬롯 키는 소문자, 숫자, 하이픈, 밑줄만 사용할 수 있습니다")
        return key
