...
class StaticGameDataBuffQueriesMixin(StaticDataDaoMixinHost):
    def get_combat_curve(
        ...
        if not table_path or not normalized_id:
            raise ValueError("curve_table_asset_path와 curve_id는 비워 둘 수 없습니다")
        curve = self._one(
            """
    ...
    def get_equipment_modify_pack(
        ...
        if not normalized:
            raise ValueError("modify_pack_id는 비워 둘 수 없습니다")
        pack = self._one(
            """
    ...
    def get_equipment_buff_curve(self, curve_id: str) -> dict[str, Any] | None:
        ...
        if not normalized:
            raise ValueError("curve_id는 비워 둘 수 없습니다")
        curve = self._one(
            """
    ...
    def get_buff_definition(self, asset_path: str) -> dict[str, Any] | None:
        ...
        if not normalized:
            raise ValueError("asset_path는 비워 둘 수 없습니다")
