...
def enable_session_log(
    *,
    ...
    logger.info(
        "logging.session_started | 독립 타임스탬프 실행 로그를 켰습니다"
...
def disable_session_log(
    *,
    ...
    if _session_sink_id is not None:
        logger.info(
            "logging.session_stopped | 독립 타임스탬프 실행 로그를 껐습니다"
...
def set_log_dir(
    ...
    *,
    ...
    try:
        ...
        sys.stderr.write(f"로그 디렉터리 전환 시 이전 로그 sink 제거 실패, 새 sink를 계속 추가합니다: {exc}\n")
    LOG_DIR = new_dir
    _file_sink_id = _add_runtime_sink(LOG_DIR)
    if session_was_enabled and reopen_session:
        _session_sink_id, _session_log_path = _add_session_sink(LOG_DIR)
        logger.info(
            "logging.session_started | 로그 디렉터리 전환 후 새 독립 타임스탬프 실행 로그를 만들었습니다"
