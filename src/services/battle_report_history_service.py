...
class BattleReportHistoryService(
    ...
    def load_analysis(
        ...
        *,
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="load",
            message="고정 전투 리포트, 히트별 축, 캐릭터 설정을 읽는 중…",
        )
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="prepare",
            message="히트 식별 정보, 대상 인스턴스, 고정 캐릭터 입력을 정리하는 중…",
        )
        ...
        _analyze = partial(BattleCounterfactualAnalysisService.analyze,
            ...
            checkpoint=lambda: report_battle_analysis_progress(
                progress_callback, phase="prepare", message="고정 Buff 상태를 일괄 추론하는 중…",
            ),
            battle_record_id=battle_record_id,
            evidence=evidence,
            build=build,
        ...
        if needs_encounter_fit and inferred_encounter is not None:
            fit_analyze = partial(
                ...
                compute_backend=available_battle_compute(self._direct_formula_backend),
                checkpoint=lambda: report_battle_analysis_progress(
                    progress_callback, phase="prepare", message="후보 대상의 고정 상태를 준비하는 중…",
                ),
                battle_record_id=battle_record_id,
                evidence=evidence,
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="analyze",
            message="동작, 치유, Buff 구간, 피해 구성을 추론하는 중…",
        )
        analysis = replace(
            _analyze(),
            inferred_character_facts=inferred_character_facts,
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="replay",
            message="원본 고정 축 히트별 공식 리플레이를 실행하는 중…",
        )
        ...
        analysis = replace(
            ...
            hit_replays=BattleHitReplayService.replay(
                ...
                progress_message="원본 고정 축 히트별 공식 리플레이를 실행하는 중…",
            ),
        ...
        if critical_events:
            report_battle_analysis_progress(
                ...
                message="아크 치명타 발동이 감지되어 고정 분기를 재구성하는 중…",
            )
            replayed = analysis
            analysis = BattleInferredTargetConditionService.apply(
                replace(
            ...
            analysis = replace(
                ...
                hit_replays=BattleHitReplayService.replay(
                    ...
                    progress_message="아크 치명타 발동 이후의 고정 축을 리플레이하는 중…",
                ),
                hit_replay_model_version=HIT_REPLAY_MODEL_VERSION,
        ...
        report_battle_analysis_progress(
            ...
            message="Buff와 패시브 반사실 작업량을 계획하는 중…",
        )
    ...
    def load_build_editor_data(
        ...
        *,
        ...
        if build is None:
            raise UserDataError("현재 전투 리포트에 편집할 수 있는 캐릭터 설정 스냅샷이 없습니다")
        ...
        for original in build.get("characters") or ():
            ...
            source_title = (
                ...
                ("이번 전투 원본 고정 장비 세팅" if equipment_editable else "가져오기 번들 고정화 장비 세팅")
            )
            equipment_contexts = {
                "battle": {
                    "title": source_title,
