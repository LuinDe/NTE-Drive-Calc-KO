        ...
        if event.mechanic_kind in {
            ...
            if not linked_ids or linked_baseline <= 0.0:
                ...
                gaps = (BattleQuantificationGap(
                    ...
                    property_ids=(),
                    explanation="라크리모사 5각성 정산에 연동할 수 있는 噩梦 출처 히트가 없습니다.",
                ),)
            else:
            ...
            if ratio is not None and status in {"complete", "partial"}:
                if state is None or base_max <= 0.0:
                    # A residual-only capture cannot safely rebuild absolute HP.
                    ...
                    hit_delta = sum(
                        ...
                        and (
                            hits_by_event[event_id].relative_time_us
                    ...
                    current_max = max(
                        0.0,
                        base_max - cumulative_reduction_delta.get(target_key, 0.0),
                    )
                    current_hp = max(0.0, min(
                        current_max,
                        base_hp
                        - hit_delta
                    ...
                    cumulative_reduction_delta[target_key] = (
                        ...
                        + changed_reduction - base_reduction
                    )
