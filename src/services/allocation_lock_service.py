...
def build_allocation_lock_snapshot(
    ...
    *,
    ...
    for plan in sorted(locked_plans, key=lambda row: int(row["plan_id"])):
        ...
        if not role_name:
            raise UserDataValidationError("잠긴 방안에 캐릭터 이름이 없습니다. 잠금을 해제한 뒤 다시 저장하세요")
        ...
        if not assignments:
            raise UserDataValidationError(f"잠긴 방안 [{role_name}]이(가) 비어 있습니다. 잠금을 해제한 뒤 다시 시도하세요")
        for assignment in assignments:
            ...
            if is_virtual_equipment_assignment(resolved_assignment):
                raise UserDataValidationError(
                    f"잠긴 방안 [{role_name}]에 가상 장비가 있습니다. 잠금을 해제한 뒤 다시 시도하세요"
                )
            try:
                ...
                raise UserDataValidationError(
                    f"잠긴 방안 [{role_name}]에 잘못된 장비 UID가 있습니다. 잠금을 해제한 뒤 다시 시도하세요"
                ) from exc
            if kind not in {"module", "core"} or slot <= 0 or serial <= 0:
                raise UserDataValidationError(
                    f"잠긴 방안 [{role_name}]에 실제가 아닌 장비가 있습니다. 잠금을 해제한 뒤 다시 시도하세요"
                )
            if current_items.get((slot, serial)) != kind:
                raise UserDataValidationError(
                    f"잠긴 방안 [{role_name}]의 장비 ({slot}, {serial})이(가) 현재 안정 가방 스냅샷에 없습니다."
                    "가방을 동기화한 뒤 방안을 확인하거나 잠금을 해제하세요."
                )
...
def verify_allocation_lock_snapshot(
    ...
    if current != snapshot:
        raise UserDataValidationError(
            "세팅 잠금 상태가 계산 중에 바뀌었습니다. 다시 계산한 뒤 저장하세요"
        )
