...
def _curve_values(static_dao: Any, table: str, curve_id: str) -> tuple[float, ...]:
    ...
    if not values:
        raise ValueError(f"캐릭터 스킬 곡선 누락: {curve_id}")
    return values
...
def _constant_curve(static_dao: Any, table: str, curve_id: str) -> float:
    ...
    if any(value != values[0] for value in values[1:]):
        raise ValueError(f"캐릭터 스킬 곡선이 상수가 아닙니다: {curve_id}")
    return values[0]
        ...
        if haniel is not None and haniel_base_attack is not None:
            ...
            definitions = (
                (
                    ...
                    "Buff_Haniel_Skill_Atkup", "Cau_Haniel_AtkUp", "咕咕子: 팀 전체 공격력",
                ),
                (
