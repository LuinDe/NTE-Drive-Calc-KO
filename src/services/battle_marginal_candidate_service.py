...
class BattleMarginalCandidateService:
    ...
    def freeze(
        ...
        *,
        ...
        if not profiles:
            raise ValueError("한계 이득 후보는 비어 있을 수 없습니다")
        ...
        if any(character_id <= 0 for character_id in character_ids):
            raise ValueError("한계 이득 후보에 잘못된 캐릭터가 포함되어 있습니다")
        if len(set(character_ids)) != len(character_ids):
            raise ValueError("한계 이득 후보에 중복된 캐릭터가 포함되어 있습니다")
    ...
    def replace_equipment(
        ...
        for key in ("items", "calculation_items"):
            ...
            context[key] = rows
        if not replaced:
            raise ValueError("대상 장비가 현재 한계 이득 후보에 속하지 않습니다")
    ...
    def with_core_main_stat(
        ...
        if not any(
            ...
            raise ValueError("콘솔 메인 속성 반사실 캐릭터가 현재 후보에 속하지 않습니다")
        return replace(
            candidate,
            ...
            if (
                ...
                if candidate.equipment_editable:
                    ...
                    if character is None:
                        raise ValueError("가져온 전투 리포트 메인 속성 반사실에 원본 고정 장비 세팅이 없습니다")
                ...
                if len(cores) != 1:
                    raise ValueError("콘솔 메인 속성 반사실에는 유일한 고정 콘솔이 필요합니다")
                core = cores[0]
                core["stats"] = [
