        ...
        for commission_id, difficulty_id in sorted(groups):
            ...
            if targets:
                result.append(BattleEncounterCandidate(
                    ...
                    environment_name=(
                        f"고위험 의뢰 · {rows[0].get('name_zh') or commission_id}"
                        f" · 난이도 {difficulty_id}"
                    ),
                    scope_half="",
            ...
            if targets:
                result.append(BattleEncounterCandidate(
                    ...
                    environment_name=(
                        ...
                        f"{floor}층{'上半' if half == 'upper' else '下半'}"
                    ),
                    scope_half=half,
            ...
            if targets:
                result.append(BattleEncounterCandidate(
                    ...
                    environment_name=(
                        ...
                        f"난이도 {rows[0].get('difficulty_level') or ordinal + 1}"
                    ),
                    scope_half="",
                    outer_realm_floor=None,
                    difficulty_id=None,
        ...
        for stage in stages:
            ...
            for difficulty in difficulties:
                for health_category, option_id, health_up in health_options:
                    ...
                    option_suffix = (
                        "" if not option_id else f" · HP +{health_up * 100:.0f}%"
                    )
                    ...
                    result.append(BattleEncounterCandidate(
                        ...
                        environment_name=(
                            f"경합의 향연 · {stage.get('name_zh') or stage.get('stage_id')}"
