...
class BattleReportTransferService:
    ...
    def suggested_filename(self) -> str:
        ...
        safe = safe[:48] or "현재 계정"
        timestamp = datetime.now().astimezone().strftime("%Y%m%d-%H%M%S")
        return f"전투리포트-{safe}-{timestamp}{BATTLE_REPORT_BUNDLE_EXTENSION}"
    ...
    def export_reports(
        ...
        if missing_ids:
            raise UserDataValidationError("선택한 전투 리포트가 더 이상 없습니다. 목록을 새로 고친 뒤 다시 시도하세요")
        ...
        def validate_before_replace() -> None:
            ...
            if self._load_static_metadata() != static_metadata:
                raise StaleBattleReportTransferContextError("정적 dataset이 변경되었습니다")
    ...
    def _prepare_import_row_graph(
        ...
        *,
        ...
        if len(record_rows) != 1:
            raise UserDataValidationError("전투 리포트 번들에 유일한 원본 전투 리포트 행이 없습니다")
        ...
        if not source_account or not export_account or not bundle_id:
            raise UserDataValidationError("전투 리포트 번들의 출처 계정 정보가 불완전합니다")
        ...
        for raw in report.get("locked_equipment_at_export") or ():
            if not isinstance(raw, Mapping):
                raise UserDataValidationError("전투 리포트 번들의 고정화 장비 세팅 행이 잘못되었습니다")
            character_id = int(raw.get("character_id") or 0)
            equipment = raw.get("equipment")
            if isinstance(equipment, (str, bytes)) or not isinstance(equipment, Sequence):
                raise UserDataValidationError("전투 리포트 번들의 고정화 장비 세팅은 배열이어야 합니다")
            ...
            if digest != str(raw.get("equipment_sha256") or ""):
                raise UserDataValidationError("전투 리포트 번들의 고정화 장비 세팅 SHA-256이 일치하지 않습니다")
            ...
            if source_kind not in {
                ...
                raise UserDataValidationError("전투 리포트 번들의 고정화 장비 세팅 출처가 잘못되었습니다")
            locks.append({
                "battle_record_id": record_id,
                "character_id": character_id,
                "equipment_source_kind": source_kind,
        ...
        if (
            ...
            raise UserDataValidationError("전투 리포트 번들은 캐릭터마다 유일한 고정화 장비 세팅을 제공해야 합니다")
        ...
        for row in tables.get("battle_character_build_edit") or ():
            try:
                ...
                raise UserDataValidationError("전투 리포트 캐릭터 수정 사본 JSON이 잘못되었습니다") from error
            for key in (
                "equipment_override",
    ...
    def _validate_bundle(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
        ...
        if not isinstance(format_info, Mapping) or (
            ...
            raise UserDataValidationError("전투 리포트 번들의 내부 형식 버전을 지원하지 않습니다")
        if not isinstance(bundle, Mapping):
            raise UserDataValidationError("전투 리포트 번들에 bundle 메타데이터가 없습니다")
        if (
            ...
            raise UserDataValidationError("전투 리포트 번들의 bundle 메타데이터가 불완전합니다")
        ...
        if (
            ...
            raise UserDataValidationError(
                f"전투 리포트 번들의 사용자 데이터베이스 스키마 버전이 호환되지 않습니다 (현재 지원: {supported})"
            )
        ...
        if not isinstance(manifest, Mapping) or manifest.get("report_count") != len(reports):
            raise UserDataValidationError("전투 리포트 번들의 manifest 기록 수가 일치하지 않습니다")
        normalized = []
        for report in reports:
            if (
                ...
                raise UserDataValidationError("전투 리포트 번들에 가져올 수 있는 데이터베이스 행이 없습니다")
    ...
    def _open_current_dao(self) -> UserDataDao:
        ...
        if not path.is_file():
            raise UserDataError("고정된 계정의 사용자 데이터베이스가 없습니다")
        ...
        try:
            if (
                ...
                raise StaleBattleReportTransferContextError("전투 리포트 내보내기 계정 컨텍스트가 바뀌었습니다")
    ...
    def _load_static_metadata(self) -> dict[str, Any]:
        try:
            ...
            raise UserDataError("정적 dataset manifest를 읽을 수 없습니다") from error
    ...
    def _normalize_report_ids(values: Sequence[int]) -> tuple[int, ...]:
        if isinstance(values, (str, bytes)):
            raise UserDataValidationError("선택한 전투 리포트 ID는 배열이어야 합니다")
        result = []
        for value in values:
            if isinstance(value, bool) or not isinstance(value, int) or value < 1:
                raise UserDataValidationError("선택한 전투 리포트 ID가 잘못되었습니다")
            if value not in result:
                result.append(value)
        if not result:
            raise UserDataValidationError("전투 리포트를 하나 이상 선택하세요")
        return tuple(result)
    ...
    def _gameplay_label(record: Mapping[str, Any]) -> str:
        ...
        return "轨外之境" if floor is None else f"궤외지경 · {int(floor)}층"
    ...
    def _completeness_label(record: Mapping[str, Any]) -> str:
        ...
        return ("히트 기록 완전" if bool(complete) else "히트 기록 불완전") + f" · {stored}건"

    @staticmethod
    def _cursor_label(status: Mapping[str, Any] | None) -> str:
        if status is None:
            return "axis cursor 없음"
        ...
        if bool(status.get("axis_complete")):
            return "마지막 페이지 소진됨" + (f" · cursor {next_cursor}" if next_cursor else "")
        detail = f"cursor {next_cursor}" if next_cursor else "cursor 미저장"
