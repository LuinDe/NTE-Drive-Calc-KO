    ...
    def infer(
        ...
        *,
        ...
        return BattleInferredEncounter(
            ...
            inference_basis=(
                f"전체 조우/하프의 대상 인스턴스 {len(observed)}개와 초기 최대 HP"
                f" [{hp_text}]을(를) 모두 기본 환경의 정적 슬롯에 일대일로 주입할 수 있습니다;"
                + (
                    "단일 대상 피격 GameplayEffect의 인덱스·ID가 정식 정적 클래스 경로와"
                    "일치하여 몬스터 신원을 보완했습니다;"
                ...
                + (
                    "; 엄격 후보의 공식 프로필이 등가가 아니므로 우선 안정 기본 프로필로 공식에 참여하고,"
                    "충돌하는 후보 전부를 원본 히트별 잔차 판정에 넘깁니다."
                    if profile_ambiguous
                    else "; 엄격 후보의 공식 프로필은 등가이지만 신원이 유일하지 않아 안정 기본 신원만 유지합니다."
