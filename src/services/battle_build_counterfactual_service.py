    ...
    def compare(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("현재 기준선과 후보 구성이 같은 분석 시간대에 고정되지 않았습니다")
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="build_compare",
            message="후보 구성과 현재 기준선의 차이를 히트별로 집계하는 중…",
            completed=0,
            total=total_hits,
        )
        ...
        for ordinal, (event_id, hit) in enumerate(
            ...
            if (
                ...
                quantification = BattleCounterfactualRatio.not_applicable(
                    ...
                    explanation=(
                        "현재 기준선과 후보의 고정 히트, 캐릭터 패널, 리플레이 입력이 모두 변하지 않았습니다."
                    ),
                )
            if (
                ...
                quantification = cls._unavailable_ratio(
                    ...
                    explanation=(
                        "후보 구성은 변했지만 이 히트에 쌍을 이루는 공식이 없어 변화와 무관하다고 증명할 수 없습니다."
                    ),
                )
        ...
        assumptions = (
            "원본 전투 리포트의 동작·히트·대상·시간대를 고정하고 캐릭터 구성만 교체해 리플레이합니다.",
            "각 히트는 정식 치명타 정책에 따라 후보와 원본 이론 기댓값을 비교하고, 원본 실측 피해로 가중합니다; 이번 전투의 치명타 결과는 고정하지 않습니다.",
            "각 히트는 하프와 대상 인스턴스별로 고정 프로필을 소비합니다. 식별 정보를 몰라도 프로필이 동등하면 완전히 정량화할 수 있으며, 변하지 않은 미확인 공통 곱연산 구간은 상쇄를 허용합니다.",
            "대상 민감 peer는 하프나 대상을 넘지 않습니다. peer는 독립 heuristic만 형성하며 정량화된 이득에는 포함되지 않습니다.",
            "미정량화 히트는 원래 값으로만 고정 축에 유지되며, 후보 값이나 정확한 0 이득으로 가장하지 않습니다.",
        )
        ...
        *,
        ...
        for event in sorted(
            ...
            quantification = cls._unavailable_ratio(
                ...
                explanation="안전하게 연동할 수 있는 출처 공식이 없어 원래 값은 축 위의 사실로만 유지됩니다.",
            )
            ...
            if event.mechanic_kind == "lacrimosa_nightmare_awaken_5":
                if (
                    ...
                    quantification = BattleCounterfactualRatio.complete(
                        ...
                        explanation="후보 구성에서 라크리모사 5각성이 활성화되지 않아 이번 HP 상한 정산은 0이 됩니다.",
                    )
                else:
                ...
                if source_ratio is not None:
                    ...
                    if quantification.method != "mechanic_disabled":
                        quantification = replace(
                            ...
                            explanation=(
                                "실측 HP 상한 유효 피해를 고정 축 앵커로 삼아,"
                                "정식 噩梦 출처의 후보/원본 피해 비율로 연동합니다."
                            ),
                        )
            elif (
    ...
    def _linked_ratio(
        ...
        if not rows:
            return cls._unavailable_ratio(
                ...
                explanation="연동할 수 있는 출처 히트를 찾지 못했습니다.",
            ), None
        ...
        if linked_ratio is None or not has_quantified:
            return cls._unavailable_ratio(
                ...
                explanation="출처 히트를 모두 정량화할 수 없어 HP 상한 정산을 연동할 수 없습니다.",
        ...
        if gaps:
            quantification = BattleCounterfactualRatio.partial(
                ...
                cancelled_dimension_ids=(),
                gaps=gaps,
                explanation="출처 히트의 정량화된 성분만으로 연동합니다. 부족분이 0 이득을 뜻하지는 않습니다.",
            )
        else:
            quantification = BattleCounterfactualRatio.complete(
                linked_ratio,
                ...
                dependency_scope="mechanic_specific",
                included_dimension_ids=("linked_source_hits",),
                explanation="귀속된 출처 히트의 완전한 후보/원본 피해 비율로 연동합니다.",
            )
