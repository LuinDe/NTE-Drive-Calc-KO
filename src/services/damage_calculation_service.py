...
class TimedReactionState:
    ...
    def apply(self, now: float, duration: float) -> "TimedReactionState":
        if duration < 0:
            raise ValueError("반응 지속 시간은 음수일 수 없습니다.")
...
class DarkStarInstances:
    ...
    def apply(self, owner_id: str, now: float, duration: float) -> "DarkStarInstances":
        if duration < 0:
            raise ValueError("黯星 지속 시간은 음수일 수 없습니다.")
...
class DamageCalculationService:
    ...
    def calculate_direct(values: DirectDamageInput) -> DirectDamageResult:
        ...
        if values.skill_multiplier < 0:
            raise ValueError("스킬 피해 배율은 음수일 수 없습니다.")
        if values.character_level < 0 or values.enemy_level < 0:
            raise ValueError("캐릭터 레벨과 적 레벨은 음수일 수 없습니다.")
    ...
    def calculate_dot(values: DotDamageInput) -> DotDamageResult:
        ...
        if values.max_stacks < 1:
            raise ValueError("DOT 최대 중첩은 최소 1이어야 합니다.")
        if len(values.remaining_durations) > values.max_stacks:
            raise ValueError("DOT 현재 중첩은 최대 중첩을 넘을 수 없습니다.")
        if any(duration < 0 for duration in values.remaining_durations):
            raise ValueError("DOT 남은 시간은 음수일 수 없습니다.")
    ...
    def calculate_topple(values: ToppleDamageInput) -> ToppleDamageResult:
        ...
        if values.level_multiplier < 0:
            raise ValueError("倾陷 캐릭터 레벨 곱연산 구간은 음수일 수 없습니다.")
        if values.character_topple_strength < 0 or values.enemy_topple_limit < 0:
            raise ValueError("倾陷强度와 적 倾陷 상한은 음수일 수 없습니다.")
        if (
    ...
    def calculate_weave_followup(
        ...
        if not damage_attribute:
            raise ValueError("헥스는 기록된 원본 피해의 피해 속성을 이어받아야 합니다.")
...
def _enemy_defense_offset(scene: DamageScene) -> float:
    ...
    if scene is DamageScene.OPEN_WORLD:
        return 100.0
    raise ValueError(f"지원하지 않는 전투 장면: {scene}")
...
def calculate_enemy_defense_from_profile(
    ...
    if profile.defense_base < 0:
        raise ValueError("적 DefBase는 음수일 수 없습니다.")
...
def select_ring_owner(*participants: RingCharacter) -> RingCharacter:
    ...
    if not participants:
        raise ValueError("环合에는 참여 캐릭터가 최소 한 명 필요합니다.")
...
def calculate_weave_strength_multiplier(ring_strength: float) -> float:
    ...
    if ring_strength < 0:
        raise ValueError("环合强度는 음수일 수 없습니다.")
...
def calculate_special_multiplier(multipliers: tuple[float, ...]) -> float:
    ...
    for multiplier in multipliers:
        if multiplier < 0:
            raise ValueError("특수 곱연산 구간은 음수일 수 없습니다.")
        result *= multiplier
    return result
...
def calculate_dissonance_topple_reduction(enemy_topple_limit: float) -> float:
    ...
    if enemy_topple_limit < 0:
        raise ValueError("적 倾陷 상한은 음수일 수 없습니다.")
...
def reaction_tier_for_character_level(character_level: int) -> int:
    ...
    if not 1 <= character_level <= 80:
        raise ValueError("环合 레벨은 1에서 80 사이여야 합니다.")
...
def reaction_multiplier_for_character_level(
    ...
    if len(reaction_damage_tiers) != 16:
        raise ValueError("环合 피해 곡선은 16개 구간을 포함해야 합니다.")
...
def skill_tier_for_effective_level(effective_skill_level: int, tier_count: int = 15) -> int:
    ...
    if effective_skill_level < 1:
        raise ValueError("유효 스킬 레벨은 최소 1이어야 합니다.")
    if tier_count < 1:
        raise ValueError("스킬 배율 구간 수는 최소 1이어야 합니다.")
...
def effective_skill_level(base_skill_level: int, awakening_level: int) -> int:
    ...
    if base_skill_level < 1:
        raise ValueError("기본 스킬 레벨은 최소 1이어야 합니다.")
    if awakening_level < 0:
        raise ValueError("각성 레벨은 음수일 수 없습니다.")
...
def calculate_weave_followup_damage(
    ...
    if actual_damage < 0:
        raise ValueError("覆纹 기록의 실제 피해는 음수일 수 없습니다.")
