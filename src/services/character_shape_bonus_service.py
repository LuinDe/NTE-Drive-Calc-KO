...
def _save_public_character_shape_bonus(
    ...
    *,
    ...
    if character is not None:
        raise ValueError(
            f"공식 캐릭터 {raw_character_id}의 추가 형태는 정적 리소스 라이브러리가 관리하므로 편집할 수 없습니다"
        )
    raise ValueError("추가 형태는 사용자 정의 캐릭터 저장 입구를 통해서만 편집할 수 있습니다")
...
def reset_public_character_shape_bonus(
    ...
    *,
    ...
    with operation_scope(
        ...
        message="이전 버전 공식 추가 형태 공용 오버라이드 정리",
        character_id=int(character_id),
    ) as span:
...
def _reset_public_character_shape_bonus(
    ...
    *,
    ...
    with StaticGameDataDao(database_path) as static_dao:
        ...
        if not logical_key:
            raise ValueError(f"공용 정적 데이터베이스에 캐릭터 {raw_character_id}의 논리 캐릭터 식별자가 없습니다")
