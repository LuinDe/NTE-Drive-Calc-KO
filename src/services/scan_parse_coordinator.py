...
class ScanParseCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        if self.capture_queue_maxsize < 0:
            raise ValueError("capture_queue_maxsize는 0보다 작을 수 없습니다")
    ...
    def run(self) -> ScanParseResult:
        ...
        if self.parse_during_scan:
            if self.capture_queue_maxsize == UNBOUNDED_CAPTURE_QUEUE_MAXSIZE:
                logger.info("전체 스캔 분석 모드: 스캔과 동시에 분석, 빠른 마우스 경로는 스크린샷 경로 적체를 허용합니다.")
            else:
                logger.info("전체 스캔 분석 모드: 스캔과 동시에 분석.")
        elif self.low_load_mode:
            logger.info("전체 스캔 분석 모드: 이상 호환, 스캔 완료 후 더 낮은 부하로 스크린샷을 분석합니다.")
        else:
            logger.info("전체 스캔 분석 모드: 저부하, 스캔 완료 후 스크린샷을 분석합니다.")
        ...
        def on_capture(path: str, index: int, total: int) -> None:
            if parse_errors:
                raise RuntimeError(f"파이프라인 분석 스레드 예외: {parse_errors[0]}") from parse_errors[0]
    ...
    def _process_payload(self, payload: tuple[str, int, int], result: ScanParseResult) -> None:
        ...
        try:
            ...
            logger.warning(f"분석 보완 입력 대기: {filename} | {exc}")
            result.parse_ms += item_ms
            ...
            logger.error(f"분석 실패: {filename} | {exc}")
            result.parse_ms += item_ms
