...
def score_allocation_candidate(
    ...
    if candidates_by_legacy_uid.get(legacy_uid) is None:
        raise ValueError(f"후보 장비 {candidate.uid}이(가) 고정된 가방 컨텍스트에 없습니다")
    item = next((item for item in inventory if item.uid == legacy_uid), None)
    if item is None:
        raise ValueError(f"후보 장비 {candidate.uid}을(를) 점수 객체로 투영할 수 없습니다")
    names = _attribute_names(context)
...
def run_legacy_allocation(
    ...
    *,
    ...
    if any(not role_data["weights"] for role_data in roles_db.values()):
        ...
        raise ValueError(f"이환 공방 또는 사용자 오버라이드 스탯 가중치가 없습니다: {', '.join(missing)}")
