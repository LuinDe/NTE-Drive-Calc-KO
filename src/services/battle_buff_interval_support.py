...
class BattleBuffIntervalSupportMixin:
    ...
    def _basis(rule: Any, occurrence: BuffOccurrence) -> str:
        if rule.source_kind == "confirmed_character_passive":
            return (
                f"이번 전투의 고정 캐릭터 돌파에서 {rule.source_effect_definition_id}이(가) 해제되었습니다."
                f"감사된 캐릭터 패시브 이벤트 {rule.event_type}에 따라 투영합니다."
                "런타임 버프 이벤트를 사용할 수 있으면 여전히 고신뢰 근거가 우선합니다."
            )
        if rule.event_type == "STATIC_EQUIPPED_SOURCE":
            return (
                "고정 장비 세팅에서 이 런타임 효과 출처가 활성화되어 있습니다. 전투 리포트 고정 패널은 전투 전 기본값이며"
                "입장 후 아크·세트·각성 버프를 포함하지 않으므로 전체 전투 저신뢰 구간으로 투영합니다."
            )
        return (
            f"고정 장비 세팅 효과 {rule.source_effect_definition_id}의 정적 이벤트 "
            f"{rule.event_type}이(가) 추산 동작/히트와 정렬됩니다. 정식 발동 조건 "
            f"{getattr(rule, 'application_requirement_asset_path', '') or '无'};"
            "구간 상태 신뢰도"
            f" {occurrence.state_confidence}, nte-core 실측 버프가 아닙니다."
        )
