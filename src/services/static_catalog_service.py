    ...
    def __init__(
        ...
        *,
        ...
        if GLOBAL_DOMAIN_KEY in provider_map:
            raise ValueError(f"도메인 key {GLOBAL_DOMAIN_KEY!r}은(는) 전역 검색용으로 예약되어 있습니다")
    ...
    def search(
        ...
        *,
        ...
        if domain_key != GLOBAL_DOMAIN_KEY:
            ...
            if provider is None:
                raise StaticCatalogServiceError(f"알 수 없는 자료 도메인: {domain_key}")
    ...
    def relations(
        ...
        *,
        ...
        if not callable(loader):
            raise StaticCatalogServiceError(f"자료 도메인이 페이지네이션 관계를 지원하지 않습니다: {domain_key}")
        return loader(
            request.release,
            str(record_id),
            str(relation_kind),
    ...
    def close(self) -> None:
        ...
        if errors:
            raise StaticCatalogServiceError(
                f"게임 자료실를 닫는 중 데이터 제공자 {len(errors)}개가 실패했습니다"
            ) from errors[0]
