...
class AccountSettingsService:
    ...
    def _defaults(self) -> dict[str, dict[str, Any]]:
        ...
        if missing:
            raise RuntimeError(
                "정적 데이터베이스에 설정 기본값이 없습니다:" + "、".join(sorted(missing))
            )
        return defaults
    ...
    def _read_legacy_json(path: Path) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.warning(f"이전 설정 파일 마이그레이션 실패, 건너뜁니다: {path} | {exc}")
            return None
        if not isinstance(value, dict):
            logger.warning(f"이전 설정 파일 마이그레이션 실패, 내용이 객체가 아닙니다: {path}")
            return None
        return value
    ...
    def _setting_key(setting_key: str) -> str:
        ...
        if key not in SETTING_GROUPS:
            raise UserDataValidationError(f"알 수 없는 설정 그룹: {key or '<空>'}")
        return key
        ...
        if key == "sync":
            for name in ("inventory_sync_method", "equipment_apply_method"):
                ...
                if method not in {"nte_core", "gamepad"}:
                    raise UserDataValidationError(f"{name}은(는) nte_core 또는 gamepad여야 합니다")
            ...
            try:
                ...
                raise UserDataValidationError(
                    "inventory_settle_seconds는 숫자여야 합니다"
                ) from exc
            if not math.isfinite(settle) or settle <= 0:
                raise UserDataValidationError(
                    "inventory_settle_seconds는 0보다 커야 합니다"
                )
            ...
            if isinstance(retention, bool):
                raise UserDataValidationError(
                    "inventory_snapshot_retention_count는 양의 정수여야 합니다"
                )
            try:
            ...
            for name in ("capture", "finish", "stop", "battle_rerecord"):
                ...
                if not hotkey:
                    raise UserDataValidationError(f"{name} 단축키는 비워 둘 수 없습니다")
