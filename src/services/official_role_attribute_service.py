...
def calculate_official_role_combat_stat_components(
    ...
    add("AtkUp", "공격력 증가", totals.get("AtkUp", 0.0), percent=True)
    add("AtkAdd", "고정 공격력", totals.get("AtkAdd", 0.0))
    add(
        "PanelAtk",
        "总攻击力",
        calculate_attribute_value(
    ...
    add("HPMaxBase", "기본 HP", float(growth.get("hp_base") or 0.0) + totals.get("HPMaxBase", 0.0))
    add("HPMaxUp", "HP 증가", totals.get("HPMaxUp", 0.0), percent=True)
    add("HPMaxAdd", "고정 HP", totals.get("HPMaxAdd", 0.0))
    add(
        "PanelHP",
        "总生命值",
        calculate_attribute_value(
    ...
    add("DefBase", "기본 방어력", float(growth.get("def_base") or 0.0) + totals.get("DefBase", 0.0))
    add("DefUp", "방어력 증가", totals.get("DefUp", 0.0), percent=True)
    add("DefAdd", "고정 방어력", totals.get("DefAdd", 0.0))
    add(
        "PanelDef",
        "总防御力",
        calculate_attribute_value(
    ...
    add("DefIgnore", "방어 무시", totals.get("DefIgnore", 0.0), percent=True)
