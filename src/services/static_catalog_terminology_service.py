    ...
    def resolve(
        ...
        *,
        ...
        if not normalized_kind:
            raise ValueError("entity_kind는 비워 둘 수 없습니다")
        if not normalized_id:
            raise ValueError("stable_id는 비워 둘 수 없습니다")
        if not normalized_locale:
            raise ValueError("locale은 비워 둘 수 없습니다")
        ...
        if record.entity_kind != normalized_kind:
            raise ValueError(
                "용어 소스가 일치하지 않는 entity_kind를 반환했습니다:"
                f"{record.entity_kind!r} != {normalized_kind!r}"
            )
        if not str(record.canonical_id or "").strip():
            raise ValueError("용어 소스가 반환한 canonical_id는 비워 둘 수 없습니다")
