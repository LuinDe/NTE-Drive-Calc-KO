...
def report_bulk_apply_progress(
    ...
    *,
    ...
    try:
        ...
        logger.debug("고속 장착 진행률 콜백 실패", exc_info=True)
...
class BulkEquipmentApplyService:
    ...
    def _prepare_resume(
        ...
        if job is None:
            raise RuntimeError(f"장착 작업 {job_id}이(가) 없습니다")
    ...
    def _prepare_new(
        ...
        if snapshot_id is None:
            raise RuntimeError("사용자 데이터베이스에 아직 안정 가방 스냅샷이 없습니다")
        prepared: list[dict] = []
                    ...
                    "preflight_errors": [
                        {
                            ...
                            "error": "사용자 정의 캐릭터에는 게임 캐릭터 인스턴스가 없어 고속 장착을 사용할 수 없습니다. 자동 장착을 사용하세요.",
                        }
        ...
        for role_name, plan in selected_plans:
            ...
            if source_summary is None or source_summary.get("source") != "nte_core":
                raise RuntimeError(
                    f"장착 전 검사 [{role_name}] 실패, 비전 스캔 인벤토리에는 로컬 구성 요소가 사용할 수 있는 원본 UID가 없습니다."
                    "자동 장착으로 바꾸세요. 고속 장착은 패킷 캡처 안정 스냅샷만 지원합니다."
                )
            try:
        ...
        try:
            ...
            return (
                ...
                None,
                {
                    "applied": [],
                    "identity_requests": identity_requests,
                    "preflight_errors": [{"role_name": "전체 캐릭터 방안", "error": str(exc)}],
                },
            )
    ...
    def _resolve_character_targets(
        ...
        if not targets:
            raise RuntimeError("현재 주인공 인스턴스가 수동 선택 결과와 일치하지 않습니다")
        if override is not None and int(override["character_id"]) != int(targets[0]["character_id"]):
            raise RuntimeError("수동으로 선택한 캐릭터 인스턴스가 현재 주인공 장착 대상과 일치하지 않습니다")
        return targets
    ...
    def _plan_preflight_errors(
        ...
        try:
            ...
            return [{"role_name": "전체 캐릭터 방안", "error": str(exc)}]
        return []
    ...
    def _execute_prepared(
        ...
        for index, role in enumerate(prepared, start=1):
            ...
            report_bulk_apply_progress(
                ...
                current=index - 1,
                total=len(prepared),
                message=f"[{role_name}]의 장착 명령을 전송하는 중…",
            )
    ...
    def _apply_role(apply_service, role: dict, snapshot_id: int):
        ...
        for index, target in enumerate(targets):
            try:
                ...
                if index:
                    logger.warning("주인공 여성 인스턴스 장착 실패 후 남성 인스턴스로 전송에 성공했습니다")
                break
            except Exception as exc:
                last_error = exc
                if index + 1 >= len(targets):
                    raise
                logger.warning(
                    "주인공 여성 인스턴스 장착 실패, 남성 인스턴스를 시도하는 중: {}",
                    exc,
                )
        if result is None:
            raise last_error or RuntimeError("주인공 장착이 결과를 반환하지 않았습니다")
