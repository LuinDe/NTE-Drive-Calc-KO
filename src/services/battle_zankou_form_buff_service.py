...
def _single_curve_value(static_dao: Any, curve_id: str) -> float:
    ...
    if len(points) != 1 or not isinstance(points[0].get("value"), (int, float)):
        raise ValueError(f"잔홍 형태 곡선 {curve_id}은(는) 단일 값 정적 근거가 아닙니다")
...
class BattleZankouFormBuffService:
    ...
    def infer(
        ...
        *,
        ...
        form_label = "1각성" if effect_one_enabled else "0각성"
        ...
        for kind, ranges, asset_path, name, scope, property_id, value, requirement in definitions:
            for ordinal, (start, end) in enumerate(ranges):
                ...
                intervals.append(BattleInferredBuffInterval(
                    ...
                    inference_basis=(
                        (
                            "고정된 각성 선택으로 1각성 활성이 확인되었습니다. 狩와 惑는 전투 시작부터 상시 유지되며 형태 전환이나 퇴장"
                            "으로도 제거되지 않습니다. 현재 축에 잔홍 사망 사실이 없으므로 전투 종료"
                            "까지 투영합니다."
                            if effect_one_enabled
                            else "고정된 각성 선택으로 1각성 미활성이 확인되었습니다. 형태는 绯影闪, 离魂错"
                            ", Q, 캐릭터 전환과 현실/환상경 피해 항목으로 추정하며, 스킬 형태 전환은 동작 근거"
                            "의 마지막 히트 시점을 우선 사용하고, 지속 시간은 정적 곡선을 따르며 시간 정지를 제외합니다."
                        )
                    ),
