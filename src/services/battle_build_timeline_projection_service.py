        ...
        for group in analysis.timeline_damage_groups:
            ...
            if vital is not None:
                ...
                if status == "unavailable":
                    projection_line = "후보 미정량화; 축에는 원본 정산값 유지"
                else:
                    label = "완전 후보" if vital.candidate_damage is not None else "정량화된 변화"
                    projection_line = f"{label} {value:,.2f} ({gain:+,.2f})"
                detail_lines = group.detail_lines + (
                    projection_line,
                    (
                        f"{vital.quantification.explanation};"
                        f"신뢰도 {vital.quantification.confidence}"
                    ),
                )
        ...
        groups.extend(
            BattleTimelineDamageGroup(
                ...
                detail_lines=(
                    ...
                    f"신뢰도 {row.quantification.confidence}",
                ),
            )
            for row in derived_rows
        )
