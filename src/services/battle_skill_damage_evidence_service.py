...
def _co_timed_damage_ids(
    ...
    for hit in analysis.hits:
        ...
        inferred[hit.event_id] = (
            damage_id,
            "같은 캐릭터의 같은 마이크로초에 식별된 피해 항목이 하나뿐이어서 그 GE를 약한 근거로 보충",
        )
    return inferred
...
def _skill_level_ability_id(
    ...
    *,
    ...
    if damage_id.casefold() in _NO_SKILL_LEVEL_DAMAGE_IDS:
        return "", "정식 패시브 파생 배율은 A 또는 다른 스킬 레벨을 읽지 않음"
    ...
    return (
        selected,
        f"정식 스킬 레벨 힌트에 따라 파생 피해 {imported_ability_id or damage_id} "
        f"항목을 {selected}에 귀속",
    )
        ...
        for hit in analysis.hits:
            ...
            if canonical_reaction_damage_id is not None:
                ...
                if explicit_reaction is not None:
                    inferred_basis = (
                        f"명시적 {explicit_reaction[1]} 정산 식별 정보를 우선 적용; Core가 재사용한 출처 GE "
                        f"{original_damage_id}"
                    )
                elif original_damage_id.casefold() != damage_id.casefold():
                    inferred_basis = (
                        f"고정된 잔홍 돌파가 정식 스코치 기록 {original_damage_id} "
                        f"→ {damage_id}(으)로 교체"
                    )
                ...
                if len(formal_owner_ids) == 1:
                    ...
                    if formal_owner_id != definition_owner_character_id:
                        inferred_basis = (
                            ...
                            f"정식 피해 스킬은 캐릭터 {formal_owner_id}에 속하므로,"
                            f"Core 세션 귀속 캐릭터 {definition_owner_character_id}의 공식 정의를 사용하지 않음"
                        )
            ...
            if linko_inference is not None:
                ...
                level_owner_basis = (
                    "동조 합동 공격 추론은 패널과 레벨 출처만 다룹니다: 배율 정의는 여전히 실행 캐릭터의 피해 항목에서 가져오며,"
                    f"레벨은 캐릭터 {skill_level_character_id}의 {ability_id}에서 읽습니다;"
                    f"신뢰도 {linko_inference.confidence}"
                )
            else:
            ...
            basis = (
                f"skill_damage[{tier}] 원본 배율 배열,"
                f"유효 레벨 {effective_level}"
            )
            if inferred_basis:
            ...
            if damage_id.casefold() == _ZANKOU_SCORCH_DAMAGE_ID.casefold():
                basis += "; 잔홍 패시브가 스코치를 생성·갱신하므로 공식 귀속은 잔홍으로 고정"
            elif damage_id.casefold() == _ORDINARY_SCORCH_DAMAGE_ID.casefold():
                basis += "; Core hit.character_id를 이번 실측 사이클 공식 귀속으로 사용"
            ...
            evidence.append(BattleSkillDamageEvidence(
                ...
                state_multiplier_label=(
                    "각성 스킬 배율 (미해석)" if unresolved_awakening else
