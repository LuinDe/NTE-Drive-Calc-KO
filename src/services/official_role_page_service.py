...
def calculate_official_role_damage_breakdown(
    ...
    for source, source_stats in (
        ("가구 보너스", _world_bonus_property_stats(detail)),
        ("호감도 10레벨", _likeability_property_stats(detail)),
        ("아크", fork_stats),
        ("콘솔/드라이브", raw_equipment_stats),
        ("추가 형태", shape_stats),
    ):
    ...
    factor_rows = [
        {
            ...
            "detail": "캐릭터 패널은 일괄 100%로 계산",
        },
        {
            "name": f"배율 대응 속성 ({scaling_names[values.scaling_stat]})",
        ...
        },
        {
            ...
            "detail": (
                f"적 방어 {result.enemy_defense:g}; 캐릭터 {values.character_level:g}레벨 /"
                f"적 {values.enemy_level:g}레벨; 관통 {values.defense_penetration * 100:g}% /"
                f"방어 감소 {values.defense_reduction * 100:g}%"
            ),
        },
        {
            "name": "저항 구간",
            ...
            "detail": (
                f"기본 저항 {values.boss_resistance * 100:g}% - 저항 감소"
                f"{sum(values.enemy_resistance_reductions) * 100:g}% - 관통"
...
def save_official_role_tab_order(
    ...
    if any(character_id <= 0 for character_id in normalized):
        raise ValueError("캐릭터 탭 순서에 잘못된 character_id가 있습니다")
    if len(set(normalized)) != len(normalized):
        raise ValueError("캐릭터 탭 순서에 중복 캐릭터를 포함할 수 없습니다")
    ...
    *,
    ...
    with (
        ...
        if character is None:
            raise ValueError(f"공식 캐릭터가 없습니다: {character_id}")
        ...
        for item in saved_items:
            ...
            item["names"] = {
                "zh_cn": item_names.get(
                    item.get("item_id"), item.get("item_id") or "빈 장비"
                )
            }
            item["suit_names"] = {
    ...
    return {
        ...
        "equipment_contexts": {
            ...
            "saved": {
                "title": f"저장된 세팅 · {selected_slot_name}",
                "items": saved_items,
