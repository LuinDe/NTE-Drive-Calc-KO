...
class BattleTargetControlPolicyService:
    ...
    def default_control_succeeds(
        ...
        if policy in {CONTROL_BLOCKED_BOSS, CONTROL_CONFIRMED_ALL_BOSS}:
            return False, "정식 몬스터 목록이 분석된 대상을 Boss로 분류하므로, 제어 성공을 기본값으로 두지 않습니다."
        if policy == CONTROL_BLOCKED_FORMAL_IMMUNITY:
            return False, "정식 대상 태그가 제어 면역을 나타내므로, 제어 성공을 기본값으로 두지 않습니다."
        return True, (
            "대상 식별 정보를 알 수 없거나 정식 제어 면역 근거가 없습니다; 식별 가능한 제어 스킬에 의한 낮은 신뢰도 추론만 허용하며,"
            "이는 Core 실측 제어 상태와 같지 않습니다."
        )
