...
class BattleMarginalBenefitService:
        ...
        *,
        ...
        if profile is None:
            return BattleMarginalBenefits(
                ...
                core_notice="현재 한계 이득 후보에 이 캐릭터 구성이 없습니다.",
            )
        baseline = next(
        ...
        if baseline is None:
            return BattleMarginalBenefits(
                character_id=character_id,
                core_notice="현재 고정 축에 이 캐릭터의 패널 기준선이 없습니다.",
            )
    ...
    def _core_main_stats(
        ...
        *,
        ...
        if core is None:
            return (), "현재 캐릭터에 반사실 계산에 쓸 콘솔 사본이 없습니다."
        current_main = next(
            (
        ...
        if current_main is None:
            return (), "현재 콘솔에 정식 메인 속성 기록이 없습니다."
        current_property = str(current_main.get("property_id") or "").strip()
        if not current_property:
            return (), "현재 콘솔 메인 속성 식별자가 잘못되었습니다."
        if not core_catalog:
            return (), "공식 금색 콘솔 메인 속성 곡선을 사용할 수 없습니다."
        ...
        for ordinal, property_id in enumerate(_CORE_MAIN_PROPERTIES, start=1):
            ...
            if matches_current:
                ...
                report_battle_analysis_progress(
                    progress_callback,
                    phase="core_main",
                    message=f"금색 콘솔 메인 속성 계산 중: {label}…",
                    completed=ordinal - 1,
                    total=len(core_catalog),
                )
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="core_main",
            message="금색 콘솔 메인 속성 한계 이득을 집계했습니다.",
        ...
        *,
        ...
        if not fork_id:
            return BattleForkMarginal(
                ...
                skill=None,
                comprehensive=None,
                unavailable_reason="현재 캐릭터가 아크를 장착하지 않았습니다.",
            )
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="fork_benefit",
            message=f"{fork_name}의 아크 없음 기준선을 만드는 중…",
        )
        ...
        if loaded_no_fork is None:
            return BattleForkMarginal(
                ...
                skill=None,
                comprehensive=None,
                unavailable_reason="아크 없음 고정 축 기준선을 만들 수 없습니다.",
            )
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="fork_benefit",
            message=f"{fork_name}의 상시 속성과 스킬 메커니즘을 분리하는 중…",
        )
        ...
        if loaded_stats_only is None:
            return BattleForkMarginal(
                ...
                skill=None,
                comprehensive=None,
                unavailable_reason="아크 상시 패널만 남긴 분석 상태를 만들 수 없습니다.",
            )
