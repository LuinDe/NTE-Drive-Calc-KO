...
def _snapshot_parts(snapshot: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if message.get("method") == "event.inventory.snapshot":
        ...
        if not isinstance(params, Mapping):
            raise ValueError("가방 이벤트 params는 객체여야 합니다")
...
def _canonical_characters(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(characters, list):
        raise ValueError("가방 스냅샷 characters는 배열이어야 합니다")
    ...
    if declared_count is not None:
        if isinstance(declared_count, bool) or not isinstance(declared_count, int) or declared_count < 0:
            raise ValueError("가방 스냅샷 character_count는 음이 아닌 정수여야 합니다")
        if declared_count != len(characters):
            raise ValueError(
                f"가방 스냅샷은 캐릭터 {declared_count}명을 선언했지만 실제로는 {len(characters)}건을 포함합니다"
            )
    ...
    for index, raw_character in enumerate(characters):
        if not isinstance(raw_character, Mapping):
            raise ValueError(f"캐릭터 인스턴스 characters[{index}]는 객체여야 합니다")
        ...
        if character_id is None or character_id <= 0 or not isinstance(uid, Mapping):
            raise ValueError(f"캐릭터 인스턴스 characters[{index}]에 유효한 character_id 또는 uid가 없습니다")
        ...
        if slot is None or serial is None or slot <= 0 or serial <= 0:
            raise ValueError(f"캐릭터 인스턴스 characters[{index}].uid는 양의 정수 slot/serial을 포함해야 합니다")
        if character_id in seen_character_ids or (slot, serial) in seen_uids:
            raise ValueError("가방 스냅샷에 중복 캐릭터 인스턴스가 있습니다")
...
def _validated_content(
    ...
    if payload.get("complete") is not True:
        raise ValueError("complete=true인 완전한 가방 스냅샷만 받습니다")

    item_count = payload.get("item_count")
    if isinstance(item_count, bool) or not isinstance(item_count, int) or item_count < 0:
        raise ValueError("가방 스냅샷 item_count는 음이 아닌 정수여야 합니다")
    items = payload.get("items")
    if not isinstance(items, list):
        raise ValueError("가방 스냅샷 items는 배열이어야 합니다")
    if item_count != len(items):
        raise ValueError(
            f"가방 스냅샷은 {item_count}개를 선언했지만 실제로는 {len(items)}건을 포함합니다"
        )
    ...
    for index, raw_item in enumerate(items):
        if not isinstance(raw_item, Mapping):
            raise ValueError(f"가방 항목 items[{index}]는 객체여야 합니다")
        uid = raw_item.get("uid")
        if not isinstance(uid, Mapping):
            raise ValueError(f"가방 항목 items[{index}].uid는 객체여야 합니다")
        slot = _integer_or_none(uid.get("slot"))
        serial = _integer_or_none(uid.get("serial"))
        if slot is None or serial is None:
            raise ValueError(f"가방 항목 items[{index}].uid는 음이 아닌 정수 slot/serial을 포함해야 합니다")
        key = (slot, serial)
        if key in uids:
            raise ValueError(f"가방 스냅샷에 중복 UID가 있습니다: slot={slot}, serial={serial}")
        uids.add(key)
...
class InventorySnapshotStabilizer:
    ...
    def offer(
        ...
        *,
        ...
        try:
            ...
            for prefix, code in (
                ("complete=true인", "incomplete_snapshot"),
                ("가방 스냅샷은", "declared_count_mismatch"),
                ("가방 스냅샷에 중복 UID가 있습니다", "duplicate_item_uid"),
                ("가방 스냅샷에 중복 캐릭터 인스턴스가 있습니다", "duplicate_character_instance"),
            ):
        ...
        if required_uids is not None and uids != required_uids:
            return SnapshotOfferResult(
                ...
                reason="현재 장착 세션이 받은 스냅샷에 고정 인벤토리의 모든 장비가 포함되지 않았습니다",
        ...
        if self._is_out_of_order(generation, sequence):
            return SnapshotOfferResult(
                "ignored",
                item_count=item_count,
                reason="중복되거나 순서가 어긋난 generation/sequence를 무시합니다",
    ...
    def mark_committed(self, fingerprint: str) -> None:
        ...
        if candidate is None or candidate.fingerprint != fingerprint:
            raise ValueError("현재 이미 안정된 후보 스냅샷만 제출할 수 있습니다")
