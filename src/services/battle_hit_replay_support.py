...
def dot_final_replay_factors(
    ...
    return (replay_factor(
        ...
        formula=(
            "1 + min(정산 전 DOT 종류 수 × 25%, 100%)"
            if value > 1.0 else "1로 고정"
        ),
    ),)
...
def replay_source_terms(
    ...
    terms = [
        BattleHitReplayTerm(
            ...
            evidence_basis=f"{baseline.source} 캐릭터 속성 출처 스냅샷",
        )
    ...
    for modifier in projection.modifiers:
        ...
        terms.append(BattleHitReplayTerm(
            ...
            evidence_basis=f"히트 시 Buff 투영 (신뢰도 {modifier.confidence})",
        ))
    if terms:
        return tuple(terms)
    ...
    return tuple(
        BattleHitReplayTerm(
            ...
            evidence_basis=f"{baseline.source} 합계값; 과거 출처 미분리",
        )
...
def apply_observed_damage_correction(
    ...
    if hit.damage_correction_kind in {
        ...
        basis = (
            "nte-core 원본 보고 피해; Calc의 공유 HP 풀 중복 차감은 전투 리포트 유효"
            "피해에만 영향을 주며, 이번 히트 공식 정산값은 바꾸지 않습니다"
            if hit.damage_correction_kind
            == "calc_hp_pool_alias_reconciliation_v1"
            else (
                "nte-core 원본 보고 피해; overkill은 전투 리포트 유효 총 피해에서만 차감되며,"
                "이번 히트 공식 정산값은 바꾸지 않습니다"
            )
        )
    ...
    return replace(
        ...
        missing_evidence=tuple(dict.fromkeys((
            *result.missing_evidence,
            (
                f"원본 보고 피해 {hit.raw_damage:g};"
                f"이번 히트 유효 피해를 {hit.damage:g}(으)로 보정했습니다."
...
def replay_target_profile_basis(
    ...
    if not inferred_target:
        return resolved_target, "사용자가 확인한 대상 속성 패키지"
    if resolved_target:
        return resolved_target, "전체 대상 수·초기 최대 HP·정식 신원이 유일하게 일치하는 정적 환경 대상 파라미터"
    return resolved_target, "전체 대상 수와 초기 최대 HP 다중집합이 유일하게 일치하는 정적 환경 대상 파라미터"
