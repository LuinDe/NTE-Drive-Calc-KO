...
def _required_text(value: Any, label: str) -> str:
    ...
    if not text:
        raise AllocationContextError(f"{label}은(는) 비워 둘 수 없습니다")
    return text
...
def _official_attribute_id(value: Any, known_attribute_ids: set[str], label: str) -> str:
    ...
    if attribute_id not in known_attribute_ids:
        raise AllocationContextError(f"정적 데이터베이스에 공식 속성 ID가 없습니다: {attribute_id}")
    return attribute_id
    ...
    return tuple(
        OfficialStat(
            property_id=_official_attribute_id(
                row.get("property_id"), known_attribute_ids, "가방 스탯 property_id"
            ),
...
def _candidate(
    ...
    kind = _required_text(row.get("kind"), "가방 아이템 kind")
    item_id = _required_text(row.get("item_id"), "가방 아이템 item_id")
    ...
    if (
        ...
        matches = [
            ...
            and (
                kind != "module"
                or _geometry_key(value.get("geometry_id"), "공식 모듈 geometry_id")
                == _geometry_key(geometry, "가방 모듈 geometry")
            )
        ]
        if kind == "core":
            matches.sort(
    ...
    if template is None:
        raise AllocationContextError(f"정적 데이터베이스에 공식 장비 템플릿이 없습니다: {item_id}")
    if kind != _required_text(template.get("kind"), "공식 장비 템플릿 kind"):
        raise AllocationContextError(f"가방 아이템 {item_id}의 kind가 공식 템플릿과 일치하지 않습니다")
    suit_id = _optional_text(row.get("suit_id"))
    if suit_id is not None and suit_id not in known_suit_ids:
        raise AllocationContextError(f"정적 데이터베이스에 공식 세트 ID가 없습니다: {suit_id}")
    geometry = _optional_text(row.get("geometry"))
    frozen_geometry = geometry
    if kind == "module":
        candidate_geometry = _geometry_key(geometry, "가방 모듈 geometry")
        template_geometry_id = _required_text(template.get("geometry_id"), "공식 모듈 geometry_id")
        template_geometry = _geometry_key(template_geometry_id, "공식 모듈 geometry_id")
        if candidate_geometry not in known_geometry_keys:
            raise AllocationContextError(f"정적 데이터베이스에 공식 형태 ID가 없습니다: {geometry}")
        if candidate_geometry != template_geometry:
            raise AllocationContextError(f"가방 모듈 {item_id}의 geometry가 공식 템플릿과 일치하지 않습니다")
    ...
    if equipped_character_id is not None and equipped_character_id not in known_character_ids:
        raise AllocationContextError(f"정적 데이터베이스에 장착 캐릭터가 없습니다: {equipped_character_id}")
...
def _suit_constraint(
    ...
    required_shape_ids = tuple(
        _required_text(shape_id, "세트 required_shape_id")
        for shape_id in suit.get("required_shape_ids") or ()
    )
    for shape_id in required_shape_ids:
        if _geometry_key(shape_id, "세트 required_shape_id") not in known_geometry_keys:
            raise AllocationContextError(f"정적 데이터베이스에 공식 형태 ID가 없습니다: {shape_id}")
...
def _role_equipment_constraints(
    ...
    *, known_character_ids: set[int],
    ...
    if character_id not in known_character_ids:
        raise AllocationContextError(f"정적 데이터베이스에 캐릭터 {character_id}이(가) 없습니다")
    blueprint = static_dao.get_equipment_plan(character_id)
    if blueprint is None:
        raise AllocationContextError(f"캐릭터 {character_id}에게 공식 캐릭터 보드가 없습니다")
    cells = tuple(
    ...
    if len(cells) != 20 or len(positions) != 20 or any(
        ...
        raise AllocationContextError(f"캐릭터 {character_id}의 공식 보드는 유일하고 유효한 칸 20개를 포함해야 합니다")
...
def _custom_role_equipment_constraints(
    ...
    if len(enabled_cells) != 20 or len(positions) != 20 or any(
        ...
        raise AllocationContextError(f"사용자 정의 캐릭터 {character_id}의 보드는 유일하고 유효한 칸 20개를 포함해야 합니다")
...
def _role_preference(
    ...
    *,
    ...
    if not isinstance(weights, Mapping) or not isinstance(limits, Mapping):
        raise AllocationContextError("최적화 선호도 버전에 잘못된 속성 설정이 있습니다")
    profile_weights = {
        _official_attribute_id(property_id, known_attribute_ids, "속성 가중치 property_id"): float(weight)
    ...
    if target_suit_id is not None and target_suit_id not in known_suit_ids:
        raise AllocationContextError(f"정적 데이터베이스에 공식 세트 ID가 없습니다: {target_suit_id}")
    return AllocationRolePreference(
        ...
        core_main_property_id=(
            _official_attribute_id(
                row["core_main_property_id"], known_attribute_ids, "코어 메인 스탯 property_id"
            )
        ...
        substat_priorities=tuple(
            _official_attribute_id(property_id, known_attribute_ids, "서브 스탯 우선순위 property_id")
            for property_id in row.get("substat_priorities") or ()
        ),
        substat_blacklist=tuple(
            _official_attribute_id(property_id, known_attribute_ids, "서브 스탯 블랙리스트 property_id")
        ...
        property_limits=tuple(
            PropertyLimit(
                property_id=_official_attribute_id(
                    property_id, known_attribute_ids, "속성 제한 property_id"
                ),
...
def build_allocation_context(
    ...
    *,
    ...
    if min(pinned_snapshot_id, pinned_profile_id, pinned_profile_version) < 1:
        raise AllocationContextError("snapshot_id, profile_id, profile_version은 0보다 커야 합니다")

    account = user_dao.profile()
    try:
        snapshot, inventory_rows = user_dao.export_inventory_snapshot(pinned_snapshot_id)
    except Exception as exc:
        raise AllocationContextError(f"가방 스냅샷을 고정할 수 없습니다: {pinned_snapshot_id}") from exc
    ...
    if profile is None or profile.get("version") is None:
        raise AllocationContextError(
            f"최적화 선호도 버전이 없습니다: profile_id={pinned_profile_id}, version={pinned_profile_version}"
        )
    ...
    known_attribute_ids = {
        _required_text(attribute.get("attribute_id"), "정적 장비 속성 ID")
        for attribute in raw_attributes
    }
    attribute_id_by_name: dict[str, str] = {}
    official_attributes: list[OfficialAttribute] = []
    for attribute in raw_attributes:
        attribute_id = _required_text(attribute.get("attribute_id"), "정적 장비 속성 ID")
        display_name = _required_text(attribute.get("display_name_zh"), "정적 장비 속성 표시 이름")
    ...
    suits_by_id = {
        _required_text(suit.get("suit_id"), "공식 세트 ID"): suit
        for suit in static_dao.list_suits()
    }
    raw_shapes = static_dao.list_shapes()
    known_geometry_keys = {
        _geometry_key(shape.get("shape_id"), "공식 형태 ID")
    ...
    official_shapes = tuple(
        OfficialShape(
            shape_id=_required_text(shape.get("shape_id"), "공식 형태 ID"),
    ...
    equipment_by_id = {
        _required_text(template.get("item_id"), "공식 장비 템플릿 item_id"): template
    ...
    candidates = tuple(
        _candidate(
            ...
            snapshot_source=_required_text(snapshot.get("source"), "가방 스냅샷 source"),
        )
    ...
    return AllocationContext(
        ...
        static_dataset=StaticDatasetReference(
            ...
            dataset_id=_required_text(dataset.get("dataset_id"), "정적 데이터셋 dataset_id"),
            importer_version=int(dataset["importer_version"]),
            built_at_utc=_required_text(dataset.get("built_at_utc"), "정적 데이터셋 built_at_utc"),
        ),
        snapshot=InventorySnapshotReference(
            snapshot_id=int(snapshot["snapshot_id"]),
            source=_required_text(snapshot.get("source"), "가방 스냅샷 source"),
            ...
            captured_at_utc=_required_text(snapshot.get("captured_at_utc"), "가방 스냅샷 captured_at_utc"),
