...
def resolve_battle_target_condition(
    ...
    return BattleTargetCondition(
        target_name=_text(value.get("target_name"), "사용자 확인 대상"),
    ...
    if kind == "outer_realm":
        ...
        if len(parts) >= 3 and parts[1].isdigit():
            half = (
                ...
                else "전반" if "FirstHalf" in parts[2]
                else "후반"
            )
            return f"轨外之境 {int(parts[1])}층 {half}"
        return "轨外之境"
    prefix = {
        "open_world": "大世界",
        "clone": "재료 / 육성 던전",
        "feast": "争锋赏宴",
        "high_risk_commission": "고위험 의뢰",
...
def battle_witch_buff_interval(
    ...
    return BattleInferredBuffInterval(
        ...
        state_confidence="高",
        value_confidence="高",
        inference_basis="사용자가 이 전투 리포트에 대해 선택한 마녀의 축복; 오늘 상태를 읽어 과거 기록을 덮어쓰지 않습니다.",
