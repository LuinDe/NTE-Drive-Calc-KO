...
def _build_action(
    ...
    action_name = "적신자" if godslayer else (
        first.damage_name
    ...
    return BattleInferredAction(
        ...
        timing_confidence="低",
        inference_basis=(
            "같은 캐릭터·같은 입력 유형·안정적인 스킬 출처의 연속 피해 발생 구간이며, 같은 시점의 일반 공격과 반격은"
            "하나의 A 조작으로 묶습니다. 시작·종료 지점은 첫/마지막 피해 근거이며,"
            "정확한 키 입력, 선딜레이나 후딜레이를 나타내지 않습니다."
        ),
...
def _resolve_animation_candidate(
    ...
    if end_offset_us is None:
        ...
        if (
            ...
            end_source = "히트가 속한 애니메이션 Section"
    if (
...
def _expand_action_from_animation(
    ...
    return replace(
        ...
        timing_confidence=(
            ...
            in {"BP_TriggerEndAbilityEffect", "스킬 종료 이벤트"}
            else "低"
        ),
        inference_basis=(
            f"{action.inference_basis} 정식 GE가 GA 이벤트 바인딩을 통해 정적 애니메이션 "
            f"{reference.candidate.selector_key}의 히트 Notify에 유일하게 정렬됩니다."
            f"종료는 {reference.end_source}을(를) 기준으로 합니다."
            + (
                " 정적 길게 누르기 절차에 따르면 피해는 계속 누르고 있는 동안 발생합니다."
                if hold_mode == "during_hold"
                else " 정적 길게 누르기 절차에 따르면 피해는 손을 뗀 뒤의 출력 구간에서 발생합니다."
                if hold_mode == "after_hold"
                else ""
            )
        ),
        ...
        for index, action in enumerate(ordered):
            ...
            if action.start_us < next_action.start_us < action.end_us:
                ...
                action = replace(
                    ...
                    input_end_us=input_end_us,
                    inference_basis=(
                        f"{action.inference_basis} 후속 동작이 시작되어 이전 동작의 후딜레이를 잘라냈습니다."
                    ),
                )
            result.append(action)
    return tuple(
        ...
        anchored.append(
            replace(
                ...
                inference_basis=(
                    ...
                    "이 연관은 동작 표시 경계만 보정하며, 일시정지가 해당 Q로 촉발됐음을 증명하지도, 정확한 키 입력 시점을 나타내지도 않습니다."
                ),
            )
        )
    return tuple(
