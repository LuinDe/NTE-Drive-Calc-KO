...
def _loads(value: object, *, context: str) -> Any:
    try:
        ...
        raise StaticCatalogProjectionError(f"{context}이(가) 유효한 JSON이 아닙니다") from exc


def _string_tuple(value: object, *, context: str) -> tuple[str, ...]:
    parsed = _loads(value, context=context)
    if not isinstance(parsed, list):
        raise StaticCatalogProjectionError(f"{context}은(는) JSON 배열이어야 합니다")
...
class StaticCatalogCharacterService:
    ...
    def _awakenings(self, character_id: int) -> tuple[AwakeningEffect, ...]:
        ...
        for row in self._queries.list_catalog_awakenings(character_id):
            ...
            structured = _loads(
                row["modify_data_json"], context=f"각성 {effect_id} modify_data_json",
            )
            result.append(AwakeningEffect(
                ...
                gameplay_effect_ids=_string_tuple(
                    ...
                    context=f"각성 {effect_id} gameplay_effect_ids_json",
                ),
    ...
    def _costs(value: object, *, context: str) -> tuple[CostItem, ...]:
        ...
        for item in parsed:
            if not isinstance(item, Mapping) or not str(item.get("ID") or "").strip():
                raise StaticCatalogProjectionError(f"{context}에 잘못된 재료 항목이 있습니다")
                ...
                damage_items=tuple(
                    SkillDamageItem(
                        ...
                        atk_rates=_float_tuple(
                            ...
                            context=f"스킬 {skill_id} 피해 공격력 배율",
                        ),
                        def_rates=_float_tuple(
                            item["def_rate_base_json"],
                            context=f"스킬 {skill_id} 피해 방어력 배율",
                        ),
                        hp_rates=_float_tuple(
                            item["hp_rate_base_json"],
                            context=f"스킬 {skill_id} 피해 HP 배율",
                        ),
                    )
    ...
    def _skill_level(self, skill_id: str, row: Mapping[str, Any]) -> SkillLevel:
        return SkillLevel(
            ...
            costs=self._costs(
                row["cost_items_json"],
                context=f"스킬 {skill_id} 레벨 {row['level']} 소모",
            ),
        )
    ...
    def _skill_hint(skill_id: str, row: Mapping[str, Any]) -> SkillLevelHint:
        return SkillLevelHint(
            ...
            damage_effect_ids=_string_tuple(
                ...
                context=f"스킬 {skill_id} 레벨 안내 damage_effect_ids_json",
            ),
            defense_effect_ids=_string_tuple(
                row["defense_effect_ids_json"],
                context=f"스킬 {skill_id} 레벨 안내 defense_effect_ids_json",
            ),
            health_effect_ids=_string_tuple(
                row["health_effect_ids_json"],
                context=f"스킬 {skill_id} 레벨 안내 health_effect_ids_json",
            ),
        )
    ...
    def _graduation(self, character_id: int) -> GraduationTemplate | None:
        ...
        if row is None:
            return None
        equipment = _loads(
            row["equipment_json"], context=f"캐릭터 {character_id} 졸업 장비 템플릿",
        )
        if not isinstance(equipment, list):
            raise StaticCatalogProjectionError("졸업 장비 템플릿은 JSON 배열이어야 합니다")
    ...
    def _equipment_plan(self, character_id: int) -> CharacterEquipmentPlan | None:
        ...
        if module_counts != anchor_counts:
            raise StaticCatalogProjectionError(
                f"캐릭터 {character_id}의 청사진 모듈 수와 앵커 수가 일치하지 않습니다"
            )
        ...
        for item in row.get("modules", ()):
            ...
            if anchor_row is not None and anchor_column is not None:
                ...
                if any(
                    ...
                    raise StaticCatalogProjectionError(
                        f"캐릭터 {character_id}의 청사진 모듈이 5×5 보드를 벗어납니다"
                    )
            ordinal = int(item["ordinal"])
            for cell in occupied:
                if cell in board:
                    raise StaticCatalogProjectionError(
                        f"캐릭터 {character_id}의 청사진 모듈이 겹칩니다"
                    )
                board[cell] = ordinal
        ...
        if len(board) != 20:
            raise StaticCatalogProjectionError(
                f"캐릭터 {character_id}의 정식 청사진은 20칸을 덮어야 하지만 실제로는 {len(board)}칸입니다"
            )
        cells = tuple(
    ...
    def _gaps(
        ...
        if progression is None:
            gaps.append(CatalogGap(
                ...
                label="캐릭터 레벨업·돌파 소모",
                status="unavailable",
                reason="이 캐릭터에게는 독립된 정식 캐릭터 육성 패키지가 없습니다",
            ))
        if growth_count == 0:
            gaps.append(CatalogGap(
                field_key="character_panel_growth",
                label="1–80레벨 캐릭터 패널 곡선",
                status="unavailable",
                reason="이 캐릭터에게 character_panel_growth 행이 없습니다. 전투 변신 캐릭터나 완전히 가져오지 않은 캐릭터에서 흔히 발생합니다",
            ))
        return tuple(gaps)
