...
class SqliteLoadoutOptimizer:
    ...
    def _weights(
        ...
        for raw_property_id, raw_weight in source.items():
            ...
            if not property_id:
                raise ValueError("property_id는 비워 둘 수 없습니다")
            weight = float(raw_weight)
            if weight < 0:
                raise ValueError("속성 가중치는 0보다 작을 수 없습니다")
            result[property_id] = weight
        if not result:
            raise ValueError("공식 속성 ID 가중치가 최소 하나 필요합니다")
        return result
    ...
    def optimize(
        ...
        *,
        ...
        if character is None:
            raise LoadoutOptimizationError(f"정적 데이터베이스에 캐릭터 {character_id}이(가) 없습니다")
        blueprint = self.static_dao.get_equipment_plan(character_id)
        if blueprint is None:
            raise LoadoutOptimizationError(f"캐릭터 {character_id}에게 공식 장비 청사진이 없습니다")
        if snapshot_id is None:
            raise LoadoutOptimizationError(
                "공식 장비 세팅 방안을 저장하려면 계산에 사용한 안정 가방 스냅샷을 명시적으로 지정해야 합니다"
            )
        pinned_snapshot_id = int(snapshot_id)
        if pinned_snapshot_id is None:
            raise LoadoutOptimizationError("아직 안정 가방 스냅샷이 없습니다")
        if self.user_dao.inventory_snapshot_summary(pinned_snapshot_id) is None:
            raise LoadoutOptimizationError(f"가방 스냅샷 {pinned_snapshot_id}이(가) 없습니다")
        ...
        if not core_candidates:
            raise LoadoutOptimizationError(
                f"가방에 공식 추천 코어 {blueprint['core_item_id']}이(가) 없습니다"
            )
        core = max(
            core_candidates,
            key=lambda item: (
        ...
        if core_template is None or not core_template.get("suit_id"):
            raise LoadoutOptimizationError("공식 추천 코어에 세트 ID가 없습니다")
        target_suit_id = str(core_template["suit_id"])
        suit = self.static_dao.get_suit(target_suit_id)
        if suit is None:
            raise LoadoutOptimizationError(f"정적 데이터베이스에 세트 {target_suit_id}이(가) 없습니다")
        required_geometries = {
        ...
        for cell in blueprint["cells"]:
            ...
            if template is None or template["kind"] != "module":
                raise LoadoutOptimizationError(f"공식 청사진 장비 템플릿이 잘못되었습니다: {template_id}")
        ...
        for slot in slots:
            ...
            if not candidates:
                suit_hint = f"그리고 suit_id={target_suit_id}" if slot["requires_suit"] else ""
                raise LoadoutOptimizationError(
                    f"위치 ({slot['row']}, {slot['column']})에 형태"
                    f"{slot['geometry_key']}{suit_hint}인 드라이브가 없습니다"
                )
            selected = max(
                candidates,
                key=lambda item: (
        ...
        if save:
            saved_plan_id = self.user_dao.save_loadout_plan(
                name=plan_name or f"{character_id} 공식 청사진 방안",
                character_id=character_id,
