...
_HIDDEN_FIELD_PARTS = (
    "경로", "SHA", "출처 행", "출처 파일", "파일 ID", "공식 버전", "payload",
)
_IDENTITY_LABEL_PARTS = (
    "정식 ID", "정식 인덱스", "Gameplay Tag", "정의 key", "곡선 정식 ID",
    "상수 ID", "사이클 유형", "Buff key",
)
_EFFECT_TYPE_LABELS = {
    "gameplay_effect": "정식 Gameplay Effect 정의",
    "gameplay_ability": "정식 캐릭터 스킬",
    "skill_damage": "정식 스킬 피해 항목",
    "buff": "정식 Buff 정의",
    "combat_effect": "전투 메커니즘 효과",
    "combat_curve": "전투 곡선",
    "combat_level_curve": "레벨 곡선",
    "reaction": "이능력 사이클 규칙",
    "combat_constant": "전투 상수",
    "gameplay_tag": "정식 Gameplay Tag",
    "roguelike_modifier": "게임플레이 속성 팩",
}
_VALUE_LABELS = {
    "RCIM_Constant": "상수 보간",
    "Infinite": "무한 지속",
    "Instant": "즉시 정산",
    "HasDuration": "유한 지속",
    "EGameplayEffectDurationType::HasDuration": "유한 지속",
    "EGameplayEffectDurationType::Infinite": "무한 지속",
    "EGameplayEffectDurationType::Instant": "즉시 정산",
    "None": "제공되지 않음",
    "—": "不可用",
    "buff": "Buff",
}
class StaticCatalogMechanicsService:
    ...
    def browse(
        ...
        *,
        ...
        if family is None:
            raise ValueError(f"알 수 없는 메커니즘 계열: {family_key!r}")
    ...
    def detail(self, record_id: str) -> MechanicsDetail:
        ...
        if kind == "formula":
            ...
            try:
                ...
                raise LookupError("공식이 존재하지 않습니다") from exc
            return self._details.formula_detail(formula)
        if kind == "model":
            raise LookupError("반사실 모델은 플레이어 도감에 표시하지 않습니다")
        if kind == "effect":
            entity_kind, separator, entity_key = key.partition(chr(31))
            if not separator:
                raise ValueError("효과 레코드 키 형식이 잘못되었습니다")
            try:
                raw = self._misc.detail(entity_kind, entity_key)
            except (KeyError, LookupError) as exc:
                raise LookupError("연관 메커니즘이 존재하지 않습니다") from exc
            if not isinstance(raw, CatalogDetail):
                raise LookupError("효과 상세 정보를 사용할 수 없습니다")
            return self._effect_detail(raw, record_id)
        raise ValueError(f"지원하지 않는 전투 메커니즘 레코드: {kind!r}")
    ...
    def _effect_card(
        ...
        labels = {
            "gameplay_ability": "정식 스킬",
            "skill_damage": "정식 피해 항목",
            "gameplay_effect": "GAMEPLAY EFFECT",
            "buff": "BUFF",
            "combat_effect": "메커니즘 효과",
        ...
        return MechanicsCard(
            ...
            family_key=family_key,
            card_kind="effect",
            eyebrow=labels.get(item.entity_kind, "공통 효과"),
            title=display_name,
    ...
    def _player_effect_subtitle(item: CatalogSearchItem) -> str:
        ...
        if StaticCatalogMechanicsService._hidden("요약", subtitle):
            return _EFFECT_TYPE_LABELS.get(item.entity_kind, "공통 전투 메커니즘")
        if re.search(r"[#_/]|\b(?:GA|GE|RCIM|RCCE)\b", subtitle):
            return _EFFECT_TYPE_LABELS.get(item.entity_kind, "공통 전투 메커니즘")
        return subtitle or _EFFECT_TYPE_LABELS.get(item.entity_kind, "공통 전투 메커니즘")
    ...
    def _effect_detail(self, raw: CatalogDetail, record_id: str) -> MechanicsDetail:
        ...
        if owner_link is not None:
            sections.append(PlayerSection(
                "소속 대상",
                (PlayerField("귀속", owner_label, "accent"),),
            ))
        ...
        display_name = self._effect_display_name(raw)
        badges = (_EFFECT_TYPE_LABELS.get(raw.entity_kind, "전투 메커니즘"),)
        notice = "수치 규칙은 아직 플레이어가 읽을 수 있는 설명으로 정리되지 않았습니다." if unstructured and not sections else ""
        return MechanicsDetail(
            record_id=record_id,
            card_kind="effect",
            title=display_name,
            subtitle=_EFFECT_TYPE_LABELS.get(raw.entity_kind, "공통 전투 메커니즘"),
    ...
    def _unreadable_raw(label: str, value: str) -> bool:
        ...
        if label_text.endswith("ID"):
            return True
        if label_text in {"반응 유형", "원소 1", "원소 2"}:
            return True
    ...
    def _technical_relation(label: str) -> bool:
        return any(token in label for token in ("리소스", "Calculation", "Blueprint", "Montage"))
    ...
    def _formula_links(properties: list[str]) -> list[tuple[str, CatalogLink]]:
        ...
        mapping = (
            ("finaldamageup", "independent_final_damage", "특수 최종 곱연산 구간 보기"),
            ("defignore", "defense", "방어 구간 보기"),
            ("damagepenetrate", "resistance", "저항 구간 보기"),
            ("crit", "critical", "치명타 구간 보기"),
            ("damageup", "damage_increase", "피해 증가 구간 보기"),
            ("atk", "panel_attribute", "패널 공식 보기"),
            ("hp", "panel_attribute", "패널 공식 보기"),
            ("def", "panel_attribute", "패널 공식 보기"),
        )
        rows = []
    ...
    def _mechanic_formula_links(
        ...
        if entity_kind not in {
            ...
            return []
        keys = (
            ("스킬 배율 보기", "skill_multiplier"),
            ("직접 피해 공식 보기", "direct_damage"),
        )
        return [
            (
                label,
