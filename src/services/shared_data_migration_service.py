...
def read_shape_bonus_catalog(database_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"이전 버전 정적 데이터베이스를 읽을 수 없습니다: {path}") from exc
    finally:
        if "connection" in locals():
            connection.close()
    if dataset is None or schema is None:
        raise RuntimeError(f"이전 버전 정적 데이터베이스에 데이터셋 정보가 없습니다: {path}")
    return {
...
def _matching_baseline(
    ...
    try:
        ...
        raise RuntimeError(f"이전 버전 추가 형태 마이그레이션 기준선을 읽을 수 없습니다: {baseline_path}") from exc
    baselines = payload.get("baselines") if isinstance(payload, dict) else None
    if not isinstance(baselines, list):
        raise RuntimeError(f"이전 버전 추가 형태 마이그레이션 기준선 형식이 잘못되었습니다: {baseline_path}")
    exact = next(
        (
            baseline
    ...
    if exact is not None:
        ...
        if len(compatible) != 1:
            raise RuntimeError(
                "이전 버전 정적 라이브러리의 데이터셋·구조 버전과 맞는 추가 형태 마이그레이션 기준선이 없습니다"
            )
    ...
    if source is None or not isinstance(source.get("records"), list):
        raise RuntimeError("추가 형태 마이그레이션 기준선이 참조하는 배포 기본값이 없습니다")
...
def migrate_legacy_static_shape_bonuses(
    *,
    ...
    with operation_scope(
        ...
        message="이전 버전 공용 추가 형태 오버라이드 마이그레이션",
    ) as span:
...
def _migrate_legacy_static_shape_bonuses(
    *,
    ...
    if unsupported_keys or unsupported_properties:
        raise RuntimeError(
            "이전 버전 추가 형태 차이를 새 버전 정적 라이브러리에 매핑할 수 없습니다:"
