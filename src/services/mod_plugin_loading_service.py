    ...
    def ensure_proxy_deployment_allowed(self) -> None:
        ...
        if snapshot.phase == "running":
            raise ModPluginLoadingError(
                "Mod Loader가 실행 중입니다. 먼저 Loader 방식으로 전환해 중지한 뒤 프록시 DLL을 배포하세요"
            )
    ...
    def start_loader(
        ...
        *,
        ...
        if current.phase != "stopped":
            raise ModPluginLoadingError(
                current.detail or f"Mod Loader의 현재 상태에서는 시작할 수 없습니다: {current.phase}"
            )
        try:
        ...
        try:
            ...
            if proxy_rollback_error is not None:
                rollback_messages.append(
                    "게임 디렉터리 프록시 DLL 롤백 실패:" + proxy_rollback_error
                )
            if rollback_error is not None:
                rollback_messages.append(
                    "Mod 작업 공간 레지스트리 롤백 실패:" + str(rollback_error)
                )
            if rollback_messages:
                raise ModPluginLoadingError(
                    f"{exc}; 추가 오류:" + "；".join(rollback_messages)
                ) from exc
            raise ModPluginLoadingError(str(exc)) from exc
    ...
    def _remove_proxy_for_loader(
        *,
        ...
        if not proxy_path.is_file():
            raise ModPluginLoadingError(
                "게임 디렉터리의 dwmapi.dll이 일반 파일이 아니어서 Loader를 안전하게 시작할 수 없습니다"
            )
        try:
            original_bytes = proxy_path.read_bytes()
        except OSError as exc:
            raise ModPluginLoadingError(
                "게임 디렉터리의 dwmapi.dll을 읽을 수 없습니다. 먼저 게임과 런처를 종료하세요"
            ) from exc
        ...
        try:
            ...
            raise ModPluginLoadingError(
                "패키지된 Mod 플러그인을 읽을 수 없어 Loader 시작을 취소했습니다"
            ) from exc
        ...
        if not known:
            ...
            try:
                ...
                if ModPluginLoadingService._file_sha256(backup_path) != proxy_sha256:
                    ...
                    raise ModPluginLoadingError(
                        "알 수 없는 dwmapi.dll의 백업 검증에 실패해 Loader 시작을 취소했습니다"
                    )
                if ModPluginLoadingService._file_sha256(proxy_path) != proxy_sha256:
                    raise ModPluginLoadingError(
                        "백업 중 게임 디렉터리의 dwmapi.dll이 변경되어 Loader 시작을 취소했습니다"
                    )
            except ModPluginLoadingError:
                raise
            except OSError as exc:
                raise ModPluginLoadingError(
                    "알 수 없는 dwmapi.dll을 현재 계정 저장 디렉터리에 백업할 수 없어 Loader 시작을 취소했습니다"
                ) from exc
        try:
            if ModPluginLoadingService._file_sha256(proxy_path) != proxy_sha256:
                raise ModPluginLoadingError(
                    "제거 전에 게임 디렉터리의 dwmapi.dll이 변경되어 Loader 시작을 취소했습니다"
                )
            proxy_path.unlink()
        except ModPluginLoadingError:
            raise
            ...
            raise ModPluginLoadingError(
                "게임 디렉터리의 dwmapi.dll을 제거할 수 없습니다. 먼저 게임과 런처를 종료하고 디렉터리 권한을 확인하세요"
            ) from exc
        ...
        try:
            if proxy_path.exists():
                if (
                    ...
                    return None
                return "대상 위치에 다른 dwmapi.dll이 이미 있어 해당 파일을 덮어쓰지 않았습니다"
