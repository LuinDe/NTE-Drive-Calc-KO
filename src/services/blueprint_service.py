...
def official_shape_matrix(shape: dict) -> list[list[int]]:
    ...
    if not cells:
        raise ValueError(
            f"공식 형태 {shape.get('shape_id') or '未知'}에 칸 데이터가 없습니다"
        )
...
def official_board(plan: dict) -> list[list[int]]:
    ...
    if playable != 20:
        raise ValueError(
            ...
            f"의 공식 보드는 20칸이어야 하지만 실제로는 {playable}칸입니다"
        )
    return board
    ...
    *,
    ...
    for character in static_dao.list_characters():
        ...
        results[role_name] = {
            ...
            "preferred_extra_label": preferred_label or "특정 선호 없음",
    ...
    for custom in custom_characters:
        ...
        results[role_name] = {
            ...
            "core_level": 0,
            "suit_name": str(suit.get("name_zh") or suit["suit_id"]),
            "preferred_extra_label": "특정 선호 없음",
            "blueprints": candidates,
            "is_custom": True,
        }
