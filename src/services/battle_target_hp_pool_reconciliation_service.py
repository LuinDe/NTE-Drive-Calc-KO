        ...
        *,
        ...
        for row in sorted(projected, key=_sequence, reverse=True):
            ...
            row["_calc_damage_correction_basis"] = (
                "완전한 히트별 축에서 두 대상의 wire ID가 같은 HP 풀을 공유합니다; 별칭 히트가 순서·"
                "피해량·HP 결손으로 유일하게 닫히며, 서버 잔차는 중복된 수명 주기 정산입니다."
            )
        ...
        return BattleTargetHpPoolReconciliation(
            ...
            evidence_basis=(
                "완전한 축, 같은 최대 HP, 그룹별 순차 피해량 닫힘 및 유일 후보; 원본 Core 행은 수정하지 않았습니다."
            ),
        )
