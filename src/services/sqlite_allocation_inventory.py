...
def legacy_shape_id(value: Any) -> str:
    ...
    if shape_id is None:
        raise AllocationInventoryProjectionError(f"알 수 없는 공식 드라이브 형태: {value!r}")
    return shape_id
    ...
    for stat in stats:
        ...
        if name is None:
            raise AllocationInventoryProjectionError(
                f"솔버가 아직 공식 속성 ID {property_id or '<empty>'}을(를) 지원하지 않습니다"
            )
...
class SqliteAllocationInventory:
    ...
    def _quality(value: Any) -> str:
        ...
        if quality is None:
            raise AllocationInventoryProjectionError(f"알 수 없는 장비 품질: {value!r}")
        return quality
    ...
    def build(self, snapshot_id: int | None = None) -> AllocationInventoryProjection:
        ...
        if snapshot_id is None:
            raise AllocationInventoryProjectionError(
                "장비 세팅 인벤토리 투영은 안정 가방 스냅샷을 명시적으로 지정해야 합니다"
            )
        ...
        try:
            ...
            raise AllocationInventoryProjectionError(
                f"안정 가방 스냅샷 {pinned_snapshot_id}을(를) 사용할 수 없습니다"
            ) from exc
        ...
        for item in snapshot_items:
            ...
            if kind == "module":
                ...
                raise AllocationInventoryProjectionError(f"알 수 없는 장비 유형: {kind!r}")
