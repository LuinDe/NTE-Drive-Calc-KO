    ...
    def _materialize_marginal_baseline(
        ...
        *,
        ...
        report_battle_analysis_progress(
            progress_callback,
            phase="compare",
            message="현재 적용 설정의 고정 축 기준을 구체화하는 중…",
        )
        ...
        *,
        ...
        if detail_level == "marginal":
            report_battle_analysis_progress(
                ...
                message="이번 후보가 공유하는 전투 리포트 사실을 고정하는 중…",
            )
            input_options = {
        ...
        if (
            ...
            if (
                ...
                report_battle_analysis_progress(
                    progress_callback,
                    phase="baseline",
                    message="현재 적용 설정의 고정 축 기준을 재구성하는 중…",
                )
            ...
            if baseline is not None:
                report_battle_analysis_progress(
                    progress_callback,
                    phase="compare",
                    message="초안과 현재 적용 설정의 차이를 집계하는 중…",
                )
                analysis = replace(
                    analysis,
        ...
        if (detail_level == "marginal" and analysis is not None
            ...
            report_battle_analysis_progress(progress_callback, phase="marginal_panel", message="캐릭터 동적 패널과 속성 한계 이득을 계산하는 중…")
            ...
            report_battle_analysis_progress(progress_callback, phase="marginal_panel", message="캐릭터 동적 패널과 속성 한계 이득 완료")
