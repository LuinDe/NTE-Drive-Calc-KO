...
class GamepadStateSyncService:
    ...
    def sync(
        ...
        logger.info(f"[상태 관리] 게임 내 동기화 완료: requested={len(state_changes)} applied={applied_count}")
        ...
        if mismatches:
            ...
            logger.warning(f"[상태 관리] 현재 상태 불일치로 건너뜀: indexes={indexes}")
        return summary
