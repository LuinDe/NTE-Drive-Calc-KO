...
class StaticCatalogForkService:
    ...
    def metadata(self) -> ForkCatalogMetadata:
        ...
        if not has_skill_tables:
            notes.append(
                "schema v30에는 아크 전용 스킬 표가 없습니다. 표시할 수 있는 스킬 레벨은 믹싱 1–5레벨 설명·파라미터·Buff입니다."
            )
        if payload_count == 0:
            notes.append(
                "source_row는 행 키와 SHA-256만 보관하며, 원본 payload는 배포 라이브러리에서 생략되었습니다."
            )
        notes.append(
            "combat_effect_definition / combat_effect_buff_link는 importer가 생성한 프로젝트 투영이며 독립된 공식 필드를 가장하지 않습니다."
        )
    ...
    def _critical_states(
        ...
        for level in (20, 30, 40, 50, 60, 70):
            ...
            if growth_row is None or pre_stage is None:
                continue
            for state, stage in (("돌파 전", pre_stage), ("돌파 후", pre_stage + 1)):
    ...
    def get_fork(self, fork_id: str) -> ForkCatalogDetail | None:
        ...
        for buff in buffs:
            relations.append(CatalogRelation(
                ...
                label=f"믹싱 {buff.refinement_level} · {buff.definition_id or buff.asset_path}",
        ...
        audit_notes = [
            "레벨 패널은 공식 modify pack 행을 표시하며, 중국어 설명에서 추가 수치를 추정하지 않습니다.",
            "임계 레벨의 “돌파 전/후”는 레벨 행과 공식 돌파 단계 행의 파생 조합이며, 원본 라이브러리에는 독립된 패널 행이 없습니다.",
            "아크 루트 버프를 Buff/GE 자산과 정확히 연결할 수 있습니다. 자산 경로가 정확히 일치할 때만 GA를 반환하며, 설명 텍스트로 스킬을 추측하지 않습니다.",
        ]
        if not ability_rows:
            audit_notes.append("schema v30에는 이 아크에서 GA로 이어지는 구조화된 정확한 관계가 보존되어 있지 않습니다.")
        if not relations_rows:
            audit_notes.append("이 아크에는 전용 캐릭터 ID나 육성 추천 관계가 없어 캐릭터 귀속이 미해석 상태로 유지됩니다.")
