...
class InventorySyncService:
    ...
    def __init__(
        ...
        *,
        ...
        if poll_seconds <= 0:
            raise ValueError("poll_seconds는 0보다 커야 합니다")
    ...
    def wait_for_equipment_snapshot(
        ...
        *,
        ...
        if not expected:
            raise ValueError("캐릭터 부분 재검토에는 장비 UID가 최소 하나 필요합니다")
        ...
        with self._state_condition:
            while True:
                ...
                if remaining <= 0:
                    raise TimeoutError("캐릭터 장착 부분 스냅샷 대기 시간 초과")
    ...
    def wait_for_action_inventory_snapshot(
        ...
        *,
        ...
        expected = frozenset((int(slot), int(serial)) for slot, serial in required_uids)
        if not expected:
            raise ValueError("상태 관리 스냅샷에는 장비 UID가 최소 하나 필요합니다")
        deadline = time.monotonic() + max(0.0, float(timeout))
        with self._state_condition:
            while True:
                ...
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    raise TimeoutError("모든 대상을 포함하는 상태 관리 스냅샷 대기 시간 초과")
                self._state_condition.wait(remaining)
    ...
    def _capture_scoped_equipment_snapshot(self, event: Mapping[str, Any]) -> None:
        ...
        with self._state_condition:
            ...
            self._state_condition.notify_all()
        logger.info(
            "스냅샷 가드 이벤트 진단: 선언 수={}, 고정 수={}, 알려진 수={}, 완전 표시={}, UID 집합 완전={}",
    ...
    def wait_for_phase(self, phase: SyncPhase, timeout: float = 10.0) -> InventorySyncState:
        ...
        with self._state_condition:
            while self._state.phase != phase:
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    raise TimeoutError(f"가방 동기화 상태 {phase} 대기 시간 초과")
                self._state_condition.wait(remaining)
            return self._state
