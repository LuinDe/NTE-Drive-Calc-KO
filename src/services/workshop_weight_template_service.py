...
def start_workshop_weight_template_refresh(
    ...
    def refresh() -> None:
        logger.info("공방 가중치 시작 업데이트를 시작했습니다")
        try:
            ...
            outcome = "업데이트됨" if result.updated else "이미 최신"
            logger.info(
                "공방 가중치 시작 업데이트 완료: 공개 캐릭터 수={}, {}",
            ...
            if result.character_ids is not None:
                ...
                if missing:
                    logger.info(
                        "공방 가중치에 {}이(가) 없어 이번에는 정적 기본 가중치를 계속 사용합니다",
                        "、".join(missing),
                    )
            ...
            logger.warning(
                "공방 가중치 시작 업데이트 실패({}), 로컬 템플릿 또는 배포 기준선을 계속 사용합니다",
                type(exc).__name__,
            )
            return
    ...
    def refresh(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(f"공방 가중치 API 요청 실패, HTTP {exc.code}") from exc
        except (urllib.error.URLError, TimeoutError, OSError) as exc:
            raise RuntimeError(f"공방 가중치 API 연결 실패: {exc}") from exc
        try:
            payload = json.loads(raw_payload)
        except (json.JSONDecodeError, UnicodeDecodeError) as exc:
            raise RuntimeError("공방 가중치 API 응답이 유효한 JSON이 아닙니다") from exc
        if not isinstance(payload, Mapping) or int(payload.get("code") or 0) != 200:
            message = str(payload.get("msg") or "알 수 없는 오류") if isinstance(payload, Mapping) else "형식 오류"
            raise RuntimeError(f"공방 가중치 API가 실패를 반환했습니다: {message}")
        records = payload.get("data")
        if not isinstance(records, list):
            raise RuntimeError("공방 가중치 API의 data가 캐릭터 배열이 아닙니다")
