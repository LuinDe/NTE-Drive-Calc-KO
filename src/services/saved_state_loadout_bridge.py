...
def character_ids_for_saved_role(
    ...
    if not isinstance(role, Mapping):
        raise SavedStateLoadoutError(f"캐릭터 [{role_name}]에 설정이 없어 공식 캐릭터 ID를 확정할 수 없습니다")
    ...
    if not character_ids:
        raise SavedStateLoadoutError(f"캐릭터 [{role_name}]에 유효한 공식 캐릭터 ID가 없습니다")
...
def resolve_character_id_for_saved_role(
    ...
    *,
    ...
    if selected_snapshot_id is None:
        raise SavedStateLoadoutError("사용자 데이터베이스에 아직 안정 가방 스냅샷이 없습니다")
    if user_dao.inventory_snapshot_summary(selected_snapshot_id) is None:
        raise SavedStateLoadoutError(
            f"지정한 안정 가방 스냅샷이 없습니다: {selected_snapshot_id}"
        )
    ...
    def resolve_instances(
        ...
        if len(instance_uids) == 1:
            ...
            if len(uids) == 1:
                return character_id
            raise SavedStateLoadoutError(
                f"캐릭터 [{role_name}]의 공식 ID {character_id}이(가) 여러 캐릭터 인스턴스 UID에 대응됩니다"
            )
        matched_text = "、".join(str(value) for value in sorted(instance_uids))
        raise SavedStateLoadoutError(
            f"캐릭터 [{role_name}]의 여러 후보 ID에 캐릭터 인스턴스가 동시에 있어({matched_text}) 자동으로 선택할 수 없습니다"
        )
    ...
    if len(mapped_candidates) > 1:
        ...
        raise SavedStateLoadoutError(
            f"캐릭터 [{role_name}]의 후보 ID에 캐릭터 인스턴스가 여러 개 저장되어 있습니다({matched_text}). 직접 선택하세요"
        )

    candidate_text = "、".join(str(value) for value in candidates)
    raise SavedStateLoadoutError(
        f"캐릭터 [{role_name}]에 공식 ID 후보가 여러 개 있지만({candidate_text}),"
        "현재·기록 안정 가방과 저장된 매핑 어디에도 판단에 사용할 캐릭터 인스턴스가 없습니다"
    )
...
def character_ids_for_static_role(
    ...
    if not raw_name:
        raise SavedStateLoadoutError("캐릭터 이름은 비워 둘 수 없습니다")
    ...
    if not ids:
        raise SavedStateLoadoutError(f"정적 데이터베이스에 캐릭터 [{raw_name}]의 공식 ID가 없습니다")
    return ids
...
def custom_character_id_for_role(
    ...
    if not matches:
        return None
    if len(matches) > 1:
        raise SavedStateLoadoutError(f"사용자 정의 캐릭터 [{raw_name}]에 중복 기록이 있습니다")
...
def resolve_character_id_for_static_role(
    ...
    *,
    ...
    if len(found) > 1:
        raise SavedStateLoadoutError(
            f"캐릭터 [{role_name}]의 여러 공식 ID가 고정 스냅샷에 동시에 있습니다. 직접 선택하세요"
        )
    for summary in user_dao.list_inventory_snapshots():
...
def _saved_uid(value: Any, *, expected_kind: str) -> tuple[int, int]:
    ...
    if match is None or match.group("kind") != expected_kind:
        raise SavedStateLoadoutError(f"잘못된 {expected_kind} UID: {value!r}")
...
def _shape_id(geometry: Any) -> str:
    ...
    if not name:
        raise SavedStateLoadoutError("가방 드라이브에 공식 geometry가 없습니다")
...
def _official_anchor(
    ...
    try:
        ...
        raise SavedStateLoadoutError("장비 세팅 보드 또는 공식 형태 좌표가 잘못되었습니다") from exc
    if not occupied or not offsets or len(occupied) != len(offsets):
        raise SavedStateLoadoutError("장비 세팅 보드의 점유 칸이 공식 형태 면적과 일치하지 않습니다")
    ...
    if len(matches) != 1:
        raise SavedStateLoadoutError(
            f"공식 장비 세팅 앵커를 유일하게 확정할 수 없습니다: occupied={sorted(occupied)}"
        )
    return matches[0]
...
class SavedStateLoadoutBridge:
    ...
    def prepare_role_plan(
        ...
        *,
        ...
        if snapshot_id is None:
            raise SavedStateLoadoutError(
                "장비 세팅 방안을 저장하려면 계산에 사용한 안정 가방 스냅샷을 명시적으로 지정해야 합니다"
            )
        ...
        for block in blocks:
            ...
            if not isinstance(drive, Mapping):
                raise SavedStateLoadoutError(
                    f"캐릭터 [{role_name}]의 보드 블록 {block.get('block_id')}에 대응하는 드라이브가 없습니다"
                )
            ...
            if item is None or item.get("kind") != "module":
                raise SavedStateLoadoutError(
                    f"캐릭터 [{role_name}]의 드라이브 UID ({slot}, {serial})이(가) 선택한 안정 가방에 없습니다"
                )
            official_shape_id = _shape_id(item.get("geometry"))
            shape = shapes.get(official_shape_id)
            if shape is None:
                raise SavedStateLoadoutError(f"정적 데이터베이스에 형태 {official_shape_id}이(가) 없습니다")
        ...
        if isinstance(tape, Mapping):
            ...
            if core_item is None or core_item.get("kind") != "core":
                raise SavedStateLoadoutError(
                    f"캐릭터 [{role_name}]의 코어 UID ({core_slot}, {core_serial})이(가) 선택한 안정 가방에 없습니다"
                )
            assignment = {
                "uid_serial": core_serial,
                "uid_slot": core_slot,
