        ...
        *,
        ...
        if _stage(character) >= 4:
            results.extend(_intervals(
                ...
                inference_basis=(
                    "정식 패시브가 BUFF_EVENT_TREATMENT를 감시하며, 출처 측 치료 이벤트마다 팀 전체"
                    "의 20초 방어 무시를 갱신합니다. HP가 가득 찼거나 유효 치료가 0이어도 치료 이벤트로 간주합니다."
                ),
            ))
        ...
        if base_attack is not None and e_events:
            ...
            results.extend(_intervals(
                ...
                inference_basis=(
                    "일로이 E 짧게 누르기의 정식 치료 이벤트는 팀 전체 고정 공격력도 함께 부여합니다. 수치:"
                    "min(출처 기본 공격력 × 20%, 500), 20초 지속."
                ),
            ))
        if (
            ...
            results.extend(_intervals(
                ...
                inference_basis=(
                    "일로이 5각성은 정식 치료 이벤트마다 팀 전체 고정 공격력을 갱신합니다. 수치:"
                    "min(출처 기본 공격력 × 15%, 500), 20초 지속."
                ),
            ))
        return tuple(sorted(
            results,
