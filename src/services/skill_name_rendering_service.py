...
_DAMAGE_TYPE_NAMES = {
    ...
    "TRUE": "진실 피해",
    "DarkFlame": "다크 플레임 피해",
    "DragonFlame": "드래곤 플레임 피해",
    "HolyFlame": "홀리 플레임 피해",
}

_BATTLE_CLASSIFICATION_NAMES = {
    "direct": "直伤",
    "direct_follow_up": "추가 직접 피해",
    "weave": "覆纹追加攻击",
    "topple": "倾陷伤害",
    "mechanic": "메커니즘 피해",
    "reaction": "사이클 / 특수 피해",
    "max_hp_reduction": "HP 상한 정산",
}
...
_ATTACK_TYPE_NAMES = {
    ...
    "follow-up": "추가 공격",
    "special damage": "特殊伤害",
    "passive damage": "패시브 피해",
    "awakening damage": "각성 피해",
    "other": "其他",
    "unknown": "알 수 없는 유형",
}
...
def _localized_attack_fallback(value: str | None) -> str:
    ...
    if not localized or localized in {"其他", "알 수 없는 유형"}:
        return ""
...
def render_battle_classification(value: str) -> str:
    stable = str(value or "").strip()
    return _BATTLE_CLASSIFICATION_NAMES.get(stable, stable or "알 수 없는 분류")
...
def render_battle_event_type(
    ...
    if attack and attack not in values and attack != "알 수 없는 유형":
...
class SkillNameRenderingService:
    ...
    def render_ability_name(
        ...
        *,
        ...
        return official_name or category or str(fallback or stable_id or "알 수 없는 스킬")
