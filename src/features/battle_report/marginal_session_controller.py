...
class BattleMarginalSessionController:
    ...
    def open(self) -> None:
        ...
        try:
            ...
            self._show_error("고정축 한계 이득 계산을 열 수 없습니다", error)
            return
    ...
    def recalculate(self, profiles: object) -> None:
        ...
        if not isinstance(profiles, list):
            self._show_error("한계 이득을 다시 계산할 수 없습니다", ValueError("캐릭터 후보 구성 형식이 잘못됨"))
            return
        try:
            ...
            self._show_error("한계 이득을 다시 계산할 수 없습니다", error)
            return
