...
class BattleBuildSnapshotController:
    ...
    def edit(self) -> None:
        ...
        try:
            ...
            self._show_error("이번 전투의 캐릭터 설정을 편집할 수 없습니다", error)
            return
        ...
        try:
            if action in {
                ...
                QMessageBox.information(
                    ...
                    "동기화 완료",
                    (
                        "현재 캐릭터 페이지의 육성과 현재 콘솔/드라이브를 이번 전투의 수정 사본으로 저장하고 활성화했습니다."
                        "캐릭터 페이지 데이터는 변경되지 않았습니다."
                        if include_equipment
                        else "현재 캐릭터 페이지의 육성을 이번 전투의 수정 사본으로 저장하고 활성화했습니다."
                        "사본에서 선택한 콘솔과 드라이브는 유지됩니다."
                    ),
                )
                return
            ...
            self._show_error("이번 전투 캐릭터 설정 저장 실패", error)
            return
        self._reload_report(record_id)
    ...
    def set_active(self, active: bool) -> None:
        ...
        try:
            ...
            self._show_error("이번 전투 캐릭터 설정 전환 실패", error)
            return
        self._reload_report(record_id)
        if not active:
