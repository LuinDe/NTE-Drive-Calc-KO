...
class BattleTargetConditionSelector(QGroupBox):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__("대상 및 외부 환경", parent)
        ...
        self.environment_combo.addItem("재료 / 육성 던전", "clone")
        ...
        top.addWidget(QLabel("마녀의 축복"), 0, 2)
    ...
    def _build_open_world_page(self) -> None:
        ...
        layout.addWidget(QLabel("대상 (다중 선택 가능)"), 0, 0)
    ...
    def set_catalog(self, catalog: dict[str, Any]) -> None:
        ...
        self.witch_combo.addItem("없음", None)
    ...
    def _clone_activity_changed(self) -> None:
        ...
        for difficulty in activity.get("difficulties") or ():
            ...
            label = f"난이도 {level or int(difficulty['difficulty_ordinal']) + 1}"
            if team_level:
                label += f"(추천 팀 Lv.{team_level})"
    ...
    def _clone_difficulty_changed(self) -> None:
        ...
        self.clone_targets_label.setText(
            "이 난이도의 대상:" + "、".join(target["name_zh"] for target in targets)
            if targets else "이 난이도에는 사용 가능한 몬스터 스폰 템플릿이 없습니다. 대상 매개변수를 직접 보충할 수 있습니다."
        )
        self._emit_preset()
    ...
    def _outer_half_changed(self) -> None:
        ...
        targets_text = (
            "이 구역의 대상:" + "、".join(target["name_zh"] for target in targets)
            if targets else "이 층에는 사용 가능한 몬스터 속성 팩이 없습니다."
        )
        targets_text += "\n완전한 전반·후반 전투 리포트를 어느 한 구역에든 저장하면 이 층의 두 구역이 자동으로 확정됩니다."
        buff = config.get("season_buff") or {}
        if buff:
            targets_text += f"\n시즌 버프: {buff['buff_name_zh']}——{buff['description_zh']}"
    ...
    def _feast_preset(self) -> dict[str, Any]:
        ...
        if difficulty:
            target = {
                ...
                "name_zh": f"쟁봉 연회·{stage.get('name_zh', '目标')}",
    ...
    def _environment_name(kind: str, ref: str) -> str:
        ...
        if kind == "outer_realm":
            ...
            if len(parts) >= 3 and parts[1].isdigit():
                return f"궤외지경 {int(parts[1])}층{'上半' if 'FirstHalf' in parts[2] else '下半'}"
    ...
    def _emit_preset(self) -> None:
        ...
        if selected_count == 0:
            self.summary_label.setText("대상을 하나 이상 선택하세요. 저장하기 전에는 히트 리플레이에 참여하지 않습니다.")
        else:
            self.summary_label.setText(
                f"대상 {selected_count}개를 선택했습니다. 각 히트는 targetId와 HP에 따라 각자의 속성에 매핑됩니다."
                "주요 대상은 기본 표시에만 사용됩니다."
            )
