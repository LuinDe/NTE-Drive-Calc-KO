...
def format_analysis_evidence(analysis: BattleAnalysisSnapshot) -> str:
    ...
    axis = "완전한 축" if analysis.axis_complete else "불완전한 축"
...
def format_time_stop_evidence(analysis: BattleAnalysisSnapshot) -> str:
    return {
        ...
        "inferred_q_action": (
            f"Q 동작 추정 시간 정지 {len(analysis.time_stop_intervals)}개 구간 (낮은 신뢰도)"
        ),
        "inferred_linko_e": (
            f"링코 E 추정 시간 정지 {len(analysis.time_stop_intervals)}개 구간 (낮은 신뢰도)"
        ),
        "inferred_q_and_linko_e": (
            f"Q / 링코 E 추정 시간 정지 {len(analysis.time_stop_intervals)}개 구간 (낮은 신뢰도)"
        ),
    }.get(getattr(analysis, "time_stop_source_kind", "none"), "시간 정지 미획득")
...
def build_timeline_layout(
    ...
    *,
    ...
    for device, label in (("mouse", "추정 마우스"), ("keyboard", "추정 키보드")):
        lane = TimelineLane(
            key=f"input:{device}",
            label=label,
            kind="input",
