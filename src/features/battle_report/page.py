...
class BattleReportPage(QWidget):
    ...
    def _build_report(self, game_ui_asset_root) -> None:
        ...
        control_card, control_layout = _section("전투 리포트 수집 작업 공간")
        status_row = QHBoxLayout()
        self.status_badge = QLabel()
        self.status_badge.setAlignment(Qt.AlignCenter)
        set_status_badge(self.status_badge, "시작 전", "neutral")
        self.status_detail = QLabel("아직 전투 리포트 수집을 시작하지 않았습니다.")
        ...
        self.export_button = QPushButton("전투 리포트 내보내기")
        ...
        actions = QHBoxLayout()
        self.capture_button = QPushButton("수집 시작")
        ...
        self.passthrough_toggle = QCheckBox("마우스 통과")
        self.passthrough_toggle.setChecked(True)
        self.passthrough_toggle.setToolTip("끄면 오버레이를 드래그할 수 있고, 켜면 마우스 조작이 게임으로 전달됩니다.")
        ...
        self.quality_label = QLabel("데이터 품질: 수집 대기 중")
    ...
    def update_state(self, state: BattleCaptureState) -> None:
        tones = {
            ...
            "stopping": ("종료 중", "warning"),
            "stopped": ("종료됨", "neutral"),
            "history": ("전투 리포트 기록", "neutral"),
            "error": ("수집 이상", "error"),
        }
        ...
        self.save_result_button.setText("수동 저장됨" if is_manual else "피해 결과 저장")
    ...
    def show_rerecord_hotkey_confirmation(
        ...
        self.status_detail.setText(
            f"{seconds:g}초 안에 {hotkey}을(를) 한 번 더 눌러 현재 전투 리포트를 버리고 다시 수집할지 확인하세요."
        )
    ...
    def _render_summary(self, summary: BattleSummary) -> None:
        ...
        self.quality_label.setText(
            f"{quality.source} · {quality.packet_count:,}패킷 · {quality.hit_count:,}히트"
        )
