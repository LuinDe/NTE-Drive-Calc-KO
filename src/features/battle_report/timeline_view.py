...
from src.i18n_display import tr as _ko_tr
...
class BattleUnifiedTimelineWidget(BattleTimelineRangeMixin, QWidget):
    ...
    def set_hit_heading(self, value: str) -> None:
        self._hit_heading = str(value).strip() or "정식 히트"
    ...
    def paintEvent(self, _event) -> None:
        ...
        if analysis is None or not analysis.timeline_hits:
            ...
            painter.drawText(self.rect(), Qt.AlignCenter, "현재 전투 리포트에 표시할 정식 히트 축이 없습니다")
            return
    ...
    def contextMenuEvent(self, event: QContextMenuEvent) -> None:
        if self._analysis is None or self._is_in_sticky_label(event.pos().x()):
            return
        menu = QMenu(self)
        set_start = menu.addAction("분석 시작으로 설정")
        set_end = menu.addAction("분석 종료로 설정")
    ...
    def _draw_avatar(
        ...
        if pixmap is not None and not pixmap.isNull():
            ...
            painter.setPen(QColor(theme_color("#f0f6fc")))
            painter.drawText(rect, Qt.AlignCenter, _ko_tr(character_name)[:1] or "?")
        painter.restore()
