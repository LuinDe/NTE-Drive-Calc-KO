...
class BattleReportController(
    ...
    def start(self, *, preserve_inventory_pause: bool = False) -> None:
        ...
        try:
            ...
            QMessageBox.warning(
                self._dialog_parent,
                "전투 리포트를 시작할 수 없음",
                f"패킷 캡처 설정 읽기 실패, 전투 리포트 수집을 시작하지 않았습니다: {error}",
            )
        ...
        if self._resume_inventory and not preserve_inventory_pause:
            try:
                ...
                QMessageBox.warning(
                    self._dialog_parent,
                    "전투 리포트를 시작할 수 없음",
                    f"가방 동기화 중지 실패, 전투 리포트 수집을 시작하지 않았습니다: {error}",
                )
                return
        ...
        self._page.clear_analysis("수집 중; 종료 후 정식 히트 기록을 저장하면 긴 페이지 분석을 생성합니다.")
    ...
    def reset_account_state(self) -> None:
        if self.is_running():
            raise RuntimeError("전투 리포트 수집 중에는 계정을 전환할 수 없습니다")
        self._operation_token += 1
        self._reset_capture_controls()
        self._invalidate_analysis_loading()
        self._service = None
    ...
    def _restore_last_history(self) -> None:
        ...
        state = BattleCaptureState(
            phase="history",
            message="마지막으로 저장한 전투 리포트를 복원했습니다.",
            running=False,
    ...
    def _save_current_result(self) -> None:
        ...
        try:
            ...
            self._show_history_error("피해 결과 저장 실패", error)
            return
        self._apply_retention_mutation(mutation, message="전투 리포트를 수동 저장했습니다.")
    ...
    def _show_history(self) -> None:
        ...
        try:
            ...
            self._show_history_error("전투 리포트 기록 읽기 실패", error)
            return
    ...
    def _show_transfer_dialog(self) -> None:
        if self.is_running():
            QMessageBox.information(
                self._dialog_parent,
                "전투 리포트 수집 중",
                "먼저 현재 수집을 종료한 뒤 전투 리포트 패키지를 내보내거나 읽으세요.",
            )
            return
        try:
            ...
            self._show_history_error("전투 리포트 패키지 페이지 열기 실패", error)
            return
    ...
    def _save_transfer_account_name(self, value: str) -> None:
        ...
        try:
            ...
            dialog.show_error(f"닉네임 저장 실패: {error}")
            return
        dialog.set_account_name(saved)
        QMessageBox.information(dialog, "계정 닉네임", "계정 닉네임을 저장했습니다.")
    ...
    def _export_selected_reports(self, report_ids: object) -> None:
        ...
        if dialog.has_unsaved_account_name():
            dialog.show_error("계정 닉네임에 저장하지 않은 변경이 있습니다. 먼저 “닉네임 저장”을 클릭하세요.")
            return
        selected = tuple(report_ids) if isinstance(report_ids, (tuple, list)) else ()
        if not selected:
            dialog.show_error("전투 리포트를 최소 한 개 선택하세요.")
            return
        dialog.clear_error()
        try:
            suggested = service.suggested_filename()
        except Exception as error:
            dialog.show_error(f"내보내기 준비 실패: {error}")
            return
        target, _selected_filter = QFileDialog.getSaveFileName(
            dialog,
            "전투 리포트 패키지 내보내기",
            suggested,
            "NTE 전투 리포트 패키지 (*.ntebr)",
        )
        if not target:
            return
        ...
        try:
            ...
            dialog.show_error(f"내보내기 실패: {error}")
        else:
            QMessageBox.information(
                dialog,
                "내보내기 완료",
                f"전투 리포트 {outcome.report_count}개를 내보냈습니다.\n저장 위치: {target}",
            )
        finally:
    ...
    def _import_report_bundle(self) -> None:
        ...
        try:
            ...
            dialog.show_error(f"전투 리포트 패키지 읽기 실패: {error}")
        else:
            QMessageBox.information(
                dialog,
                "읽기 완료",
                (
                    f"전투 리포트 {len(outcome.imported_record_ids)}개를 가져왔습니다;"
                    f"이미 있는 전투 리포트 {outcome.skipped_existing_count}개는 건너뛰었습니다."
                ),
            )
    ...
    def _view_history_record(self, battle_record_id: int) -> None:
        try:
            ...
            if stored is None:
                raise RuntimeError("선택한 전투 리포트가 더 이상 없습니다")
            history_service.update_page_state(
            ...
            self._show_history_error("전투 리포트 상세 읽기 실패", error)
            self._refresh_history_dialog()
            return
        state = BattleCaptureState(
            phase="history",
            message="전투 리포트 기록을 보는 중입니다.",
            running=False,
            summary=stored.summary,
            persistence_status="loaded_history",
            battle_record_id=stored.battle_record_id,
    ...
    def _toggle_history_retention(
        ...
        try:
            ...
            if current_kind == "manual":
                ...
                message = "전투 리포트의 수동 저장을 취소했습니다."
            else:
                mutation = history_service.save_record(battle_record_id)
                message = "전투 리포트를 수동 저장했습니다."
        except Exception as error:
            self._show_history_error("전투 리포트 저장 상태 갱신 실패", error)
            return
    ...
    def _delete_history_record(self, battle_record_id: int) -> None:
        answer = QMessageBox.question(
            ...
            "전투 리포트 삭제",
            "이 전투 리포트를 삭제하시겠습니까? 삭제 후에는 복원할 수 없습니다.",
        ...
        try:
            ...
            self._show_history_error("전투 리포트 삭제 실패", error)
            return
        if not deleted:
            self._refresh_history_dialog()
            return
    ...
    def _refresh_history_dialog(self) -> None:
        ...
        try:
            ...
            self._show_history_error("전투 리포트 기록 새로 고침 실패", error)
            return
        dialog.set_entries(entries)
