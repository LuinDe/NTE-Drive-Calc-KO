...
class BattleDamageCompositionPanel(QWidget):
    ...
    def _role_card(self, role: RoleDamageComposition) -> QFrame:
        ...
        header.addStretch()
        total = QLabel(
            f"총 피해  {_format_damage(role.total_damage)}  ·  "
            f"총 피해의 {role.share_percent:.1f}%"
        )
    ...
    def _system_card(self, composition: BattleDamageComposition) -> QFrame:
        ...
        description = QLabel("메커니즘은 식별했으나 캐릭터 피해 소유자가 없음")
        ...
        total = QLabel(
            ...
            f"구간 피해의 {composition.system_share_percent:.1f}%"
        )
        total.setStyleSheet(themed_style("font-size:12px;color:#8b949e"))
        header.addWidget(total)
        layout.addLayout(header)
        ...
        layout.addWidget(
            self._rows_scroll(
                ...
                empty_text="현재 범위에 시스템 또는 환경 피해가 없습니다",
            )
        )
        return card
    ...
    def _unattributed_card(self, composition: BattleDamageComposition) -> QFrame:
        ...
        total = QLabel(
            ...
            f"구간 피해의 {composition.other_share_percent:.1f}%"
        )
        total.setStyleSheet(themed_style("font-size:12px;color:#8b949e"))
        header.addWidget(total)
        layout.addLayout(header)
        ...
        layout.addWidget(
            self._rows_scroll(
                ...
                empty_text="현재 범위에 미귀속 피해가 없습니다",
            )
        )
        return card
    ...
    def _rows_scroll(
        ...
        *,
        ...
        if entries:
            ...
            placeholder = QLabel(empty_text or "피해 데이터 없음")
