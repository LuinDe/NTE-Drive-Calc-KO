...
def _quantification_tooltip(quantification) -> str:
    ...
    text = (
        f"변화 상태: {quantification_status_text(quantification.status)};"
        f"정량화율: {coverage}."
    )
...
def render_attribute_results(
    ...
    for row_index, result in enumerate(results):
        ...
        tooltip = (
            f"{result.assumption}\n"
            f"패널 연동 분모: {quantification_status_text(result.role_denominator_status)};"
            f"팀 분모: {quantification_status_text(result.team_denominator_status)}.\n"
