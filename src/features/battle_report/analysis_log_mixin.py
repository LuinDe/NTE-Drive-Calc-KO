...
class BattleAnalysisLogMixin:
    ...
    def _render_log(self) -> None:
        ...
        for row, hit in enumerate(page):
            ...
            replay_text = "—"
            crit_text = "리플레이 없음"
            replay_tooltip = "현재 히트에는 아직 공식 리플레이 결과가 없습니다."
            if replay is not None:
                ...
                replay_tooltip = "\n".join(
                    value
                    for value in (
                        f"관측: {replay.observed_damage:,.0f}",
                        (
                            "비치명 후보: —"
                            if replay.non_critical_damage is None
                            else f"비치명 후보: {replay.non_critical_damage:,.2f}"
                        ),
                        (
                            "치명 후보: —"
                            if replay.critical_damage is None
                            else f"치명 후보: {replay.critical_damage:,.2f}"
                        ),
                        details,
                        gaps,
                    )
            ...
            for column, value in enumerate(values):
                ...
                if column == 9:
                    ...
                    item.setToolTip(
                        "클릭하면 이 히트의 원본 필드, HP, 공식 인자, 신뢰도, 추정 버프를 확인할 수 있습니다."
                    )
                    font = item.font()
        ...
        self.log_page_label.setText(
            f"{self._log_page + 1} / {page_count} · {len(hits):,}건"
        )
