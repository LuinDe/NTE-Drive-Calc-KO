...
def build_config_page(window):
    ...
    window.config_role_search.setPlaceholderText("캐릭터 검색 (한글·병음 지원)...")
    ...
    reset_current_btn = QPushButton("현재 초기화")
...
def confirm_pending_config_changes(window, config_dir):
    ...
    ret = QMessageBox.question(
        window,
        "저장되지 않은 설정",
        "현재 계정의 스탯 가중치에 저장되지 않은 수정이 있습니다. 먼저 저장할까요?",
...
def _add_extra_shape_row(window, data, role_name, role_data, form_layout):
    ...
    value.setPlaceholderText("추가 형태 태그 선택")
    ownership = "“저장”을 클릭하면 현재 계정의 사용자 정의 캐릭터 데이터에 기록됩니다."
    value.setToolTip(f"추가 형태 태그 선택; {ownership}")
    value.currentTextChanged.connect(
        lambda text, rn=role_name: save_extra_shape_label(window, rn, text, data)
    )
    _field("추가 형태 태그", value, form_layout)
...
def _add_extra_shape_buff_row(
    ...
    ownership = "“저장”을 클릭하면 현재 계정의 사용자 정의 캐릭터 데이터에 기록됩니다."
    property_combo.setToolTip(f"추가 형태가 제공하는 속성 선택; {ownership}")
    ...
    value_spin.setToolTip(f"추가 형태 보너스 수치; {ownership}")
...
def _add_custom_target_suit_row(window, role_name, role_data, form_layout):
    ...
    combo.setToolTip("사용자 정의 캐릭터의 기본 계산 세트입니다. 캐릭터 관리에서 단일 계산에 한해 임시로 조정할 수 있습니다.")
...
def _add_custom_board(window, role_name, role_data, form_layout):
    ...
    def refresh_count() -> None:
        count = enabled_count()
        enabled_count_label.setText(f"활성 {count}/20")
...
def _populate_config_role_tab(window, data, role_name, tab_scroll, rebuild_all_tabs):
    ...
    if role_data.get("is_custom"):
        ...
        custom_row.addWidget(QLabel("사용자 정의 계산 가능 캐릭터: 배포 템플릿이 없습니다. 여기서 기본 계산 세트와 추가 형태를 설정하세요."))
    ...
    _add_role_weight_group(window, data, role_name, role_data, form_layout, rebuild_all_tabs, "카트리지 메인 스탯 가중치", "main_weights", "+ 메인 스탯 추가")
    _add_role_weight_group(window, data, role_name, role_data, form_layout, rebuild_all_tabs, "서브 스탯 가중치", "weights", "+ 서브 스탯 추가")
...
def add_weight(window, rn, data, cb, config_dir, weight_field="weights"):
    ...
    if not available:
        QMessageBox.information(window, "알림", "모든 스탯이 이미 추가되어 있습니다.")
        return
    label, accepted = QInputDialog.getItem(
        window, "스탯 추가", "스탯 선택:", [row[0] for row in available], 0, False,
    )
    if accepted:
