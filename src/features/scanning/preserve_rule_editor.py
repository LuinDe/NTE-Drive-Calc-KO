...
class TypeRangeDialog(QDialog):
    ...
    def _build_shape_section(self, selected: set[str]) -> QWidget:
        ...
        header = QHBoxLayout()
        header.addWidget(QLabel("드라이브 형태"))
    ...
    def _build_set_section(self, selected: set[str]) -> QWidget:
        ...
        header = QHBoxLayout()
        header.addWidget(QLabel("카트리지 세트"))
        select_all = QPushButton("전체 선택")
...
class PreserveRuleEditor(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        self.name_edit.setPlaceholderText("예: 치명타 이중 딜링 카트리지")
        form.addRow("규칙 이름", self.name_edit)
        type_widget, self._type_buttons = self._build_segment(
            (("카트리지", "tape"), ("드라이브", "drive")), self._item_type, self._change_item_type
        )
        form.addRow("장비 대상", type_widget)
        action_widget, self._action_buttons = self._build_segment(
            (("보존만", "keep"), ("바로 잠금", "lock")), self._action, self._change_action
        )
        form.addRow("적중 후 처리", action_widget)
        root.addLayout(form)

        self.main_group = QGroupBox("카트리지 메인 스탯 (하나라도 일치)")
        ...
        self.sub_group = QGroupBox("서브 스탯")
        ...
        tape_match_row.addWidget(QLabel("서브 스탯 일치"))
        ...
        tape_required_layout.addWidget(QLabel("필수 포함 (다중 선택 가능)"))
        ...
        drive_match_row.addWidget(QLabel("서브 스탯 일치"))
        ...
        drive_required_layout.addWidget(QLabel("필수 포함 (다중 선택 가능)"))
        ...
        advanced.addRow("품질 범위", self.quality_combo)
        self.range_summary = QLabel()
        range_row = QWidget()
        range_layout = QHBoxLayout(range_row)
        range_layout.setContentsMargins(0, 0, 0, 0)
        range_button = QPushButton("범위 선택")
        ...
        advanced.addRow("유형 범위", range_row)
    ...
    def _refresh_sub_match_hint(self) -> None:
        ...
        if (selected_count or required_count) and selected_count < match_count:
            hint.setText("일치 수보다 적을 수 없음")
        elif required_count >= match_count:
            hint.setText("필수 포함 수는 일치 수보다 적어야 함")
        else:
            hint.clear()
    ...
    def _refresh_range_summary(self) -> None:
        ...
        label = "카트리지 세트" if self._item_type == "tape" else "드라이브 형태"
    ...
    def _save_rule(self) -> None:
        ...
        if self._item_type == "drive" and not sub_stats:
            QMessageBox.warning(self, "규칙이 유효하지 않음", "드라이브 규칙은 서브 스탯을 최소 하나 선택해야 합니다.")
            return
        if self._item_type == "tape" and not (main_stats or sub_stats):
            QMessageBox.warning(self, "규칙이 유효하지 않음", "카트리지 규칙은 메인 스탯 또는 서브 스탯을 최소 하나 선택해야 합니다.")
            return
        if not set(required_sub_stats).issubset(sub_stats):
            QMessageBox.warning(self, "규칙이 유효하지 않음", "필수 포함 서브 스탯은 서브 스탯 일치 풀에도 포함되어야 합니다.")
            return
        ...
        if len(required_sub_stats) >= match_count:
            QMessageBox.warning(self, "규칙이 유효하지 않음", "필수 포함 서브 스탯 수는 일치 수보다 적어야 합니다.")
            return
        name = self.name_edit.text().strip() or ("카트리지 보존" if self._item_type == "tape" else "드라이브 보존")
        self._result_rule = {
