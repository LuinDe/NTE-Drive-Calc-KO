...
class ScreenshotCleanupPlan:
    ...
    def confirmation_text(self) -> str:
        prompt = (
            "증분 로직 설정에 따라 raw_drive_0001.png 한 장은 남겨야 하니 삭제하지 마세요.\n\n"
            f"다른 스캔 스크린샷 {self.scan_delete_count}개와"
            f"임시 감정 붙여넣기 스크린샷 {self.temp_delete_count}개를 삭제하며, 복구할 수 없습니다."
        )
        if self.baseline_missing:
            prompt += "\n\n주의: 비교용 스크린샷을 잃으면 전체 스캔을 다시 하거나, 완전 자동 증분 스캔을 사용하지 마세요."
        return prompt
...
def execute_screenshot_cleanup(plan: ScreenshotCleanupPlan) -> ScreenshotCleanupResult:
    ...
    for path in plan.delete_files:
        try:
            ...
            logger.warning(f"스크린샷 파일 삭제 실패: {path} ({exc})")
    return result
...
class ScanFileLifecycle:
    ...
    def same_equipment_by_ocr(self, left: Path, right: Path) -> bool:
        ...
        try:
            ...
            logger.info(
                f"증분 첫 이미지 OCR 데이터 비교: {'一致' if same else '不一致'} | {Path(left).name} vs {Path(right).name}"
            )
            if not same:
                logger.debug(f"probe={left_sig} baseline={right_sig}")
            return same
        except Exception as exc:
            logger.warning(f"증분 첫 이미지 OCR 데이터 비교 실패, probe를 유지한 채 일반 분석으로 진행합니다: {exc}")
            return False
    ...
    def prepare_incremental_parse(self, parse_scope: str) -> IncrementalParsePreparation:
        ...
        if imread_unicode(baseline) is None:
            result.baseline_missing = True
            logger.warning(f"증분 스캔 기준 이미지를 읽을 수 없습니다. 전체 스캔을 다시 하세요: {baseline}")
            return result
        ...
        if self.same_equipment_by_ocr(first_probe, baseline):
            ...
            logger.info(f"증분 첫 이미지가 raw_drive_0001과 일치하여 분석 완료 후 {first_probe.name}을(를) 삭제합니다")
        return result
    ...
    def move_to_failed(self, paths: Iterable[str | Path]) -> int:
        ...
        for src in paths:
            ...
            try:
                ...
                logger.error(f"실패 스크린샷 이동 실패: {src_path.name} | {exc}")
        return moved
    ...
    def delete_paths(self, paths: Iterable[str | Path]) -> int:
        ...
        for src in paths:
            ...
            try:
                ...
                logger.error(f"중복 스크린샷 삭제 실패: {src_path.name} | {exc}")
        return deleted
    ...
    def rename_incremental_successes(self, paths: Iterable[str | Path]) -> int:
        ...
        for src in paths:
            ...
            try:
                ...
                logger.error(f"증분 스크린샷 이름 바꾸기 실패: {src_path.name} | {exc}")
        return renamed
    ...
    def move_first_full_scan_to_tail(self) -> bool:
        ...
        logger.info(f"완전 자동 증분 첫 이미지 끼워넣기: 이전 raw_drive_0001을 {target.name}(으)로 이동했습니다")
        return True
