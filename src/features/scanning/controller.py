...
class ScanningController(QObject):
    ...
    def _reload_allocation_filter_settings(self) -> None:
        ...
        try:
            ...
            self._allocation_filter_settings = AllocationFilterSettings()
            logger.warning(f"계정 분배 필터 설정이 잘못되어 기본값을 사용했습니다: {exc}")
    ...
    def _update_allocation_filter_summary(self) -> None:
        ...
        if label is None:
            return
        quality_names = {"Blue": "蓝色", "Purple": "紫色", "Gold": "금색"}
        type_names = {"tape": "카트리지", "drive": "드라이브"}
        qualities = "、".join(
            quality_names[value]
        ...
        label.setText(f"필터 설정: 품질 {qualities}; 유형 {item_types}")
