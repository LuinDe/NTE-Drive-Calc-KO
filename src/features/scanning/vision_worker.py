    ...
    def run(self):
        ...
        try:
            ...
            if not os.path.exists(self.input_dir):
                self.error.emit(f"스크린샷 폴더 {self.input_dir}을(를) 찾을 수 없습니다")
                return
            ...
            if total == 0:
                self.error.emit("스크린샷 폴더가 비어 있어 처리할 이미지가 없습니다.")
                return
            ...
            for idx, filename in enumerate(image_files, 1):
                ...
                try:
                    ...
                    if not added:
                        ...
                        logger.info(f"증분 중복 스크린샷을 걸러냈습니다: {filename}")
                    else:
            ...
            if self._cancel_requested:
                logger.info("VisionWorkerThread: 분석이 취소되었습니다")
                del processor
            ...
            logger.info("VisionWorkerThread: processing_done 신호를 보내려 합니다")
            ...
            logger.error(f"VisionWorker가 SystemExit을 포착했습니다: {exc}")
            self.error.emit(f"시스템 비정상 종료: {exc}")
        except Exception as exc:
            import traceback as tb

            err_detail = f"{exc}\n\n{tb.format_exc()}"
            logger.error(f"VisionWorker 예외: {err_detail}")
