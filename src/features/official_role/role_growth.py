...
def _build_base_group(window, character_id: int, detail: dict, editor: dict) -> QGroupBox:
    ...
    likeability = QCheckBox(
        "호감도 10레벨" + (f"({'、'.join(likeability_parts)})" if likeability_parts else "")
    )
    ...
    if not likeability_properties:
        if str((detail.get("character") or {}).get("logical_character_key")) == "protagonist":
            likeability.setText("플레이어 캐릭터에는 호감도 시스템이 없습니다")
            likeability.setToolTip("제로는 플레이어 본인이므로 호감도 레벨과 10레벨 속성 보너스를 제공하지 않습니다.")
        else:
            likeability.setToolTip("현재 정적 데이터에 이 캐릭터의 호감도 10레벨 속성이 없습니다.")
...
def _build_awakening_group(
    ...
    def refresh_resonance() -> None:
        ...
        for effect, threshold, label in resonance_labels:
            ...
            state = "활성화됨" if active else f"비활성 (각성 {threshold}개 필요)"
            label.setText(
...
def _damage_multiplier_row(
    ...
    return (
        str(damage.get("display_name_zh") or damage.get("damage_id") or "배율 항목"),
        scaling_name,
...
def _show_skill_detail(
    ...
    dialog.setWindowTitle(f"{_skill_name(skill)} - 스킬 배율 상세")
    dialog_layout = QVBoxLayout(dialog)
    summary = QLabel(
        f"기본 레벨 {base_level}"
        + (f" + 각성 {delta} = 적용 레벨 {effective_level}" if delta else "")
    )
    summary.setStyleSheet("font-weight:bold;color:#58a6ff;")
    dialog_layout.addWidget(summary)
    table = QTableWidget(len(rows), 4)
    table.setHorizontalHeaderLabels(("배율 항목", "배율 속성", "피해 유형", "현재 배율"))
    ...
    if not rows:
        dialog_layout.addWidget(QLabel("현재 정적 데이터에 표시할 스킬 배율이 없습니다."))
    ...
    def refresh_effective_levels() -> None:
        ...
        for skill in detail.get("skills") or ():
            ...
            effective_labels[skill_id].setText(
                f"적용 {effective_level}레벨 (3각성 +1)"
                if delta
                else f"적용 {effective_level}레벨"
            )
...
def _build_fork_group(
    ...
    *,
    ...
    identity.addWidget(QLabel("레벨:"))
    identity.addWidget(fork_level)
    fork_breakthrough_label = QLabel("돌파:")
    ...
    identity.addWidget(QLabel("재련:"))
    identity.addWidget(refinement)
    margin_label = QLabel("직접 피해 이득: --")
    ...
    base_label = QLabel("기본 보너스 (주황색은 재련 무조건 상시):")
    ...
    def refresh_fork_summary() -> None:
        ...
        if not stats:
            stats_layout.addWidget(QLabel("아크 미장착"))
        for property_id, value in stats.items():
            ...
            if permanent_value is not None:
                property_label += f"(상시 {_display_property_value(detail, property_id, permanent_value)})"
        ...
        if current and baseline and baseline["damage"] > 0:
            ...
            margin_label.setText(f"직접 피해 이득: {gain:+.2f}%")
        else:
            margin_label.setText("직접 피해 이득: --")
        ...
        if star:
            ...
            effect_text.setText("공식 재련 설명이 없습니다.")
