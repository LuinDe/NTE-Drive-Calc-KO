...
def _build_equipment_cards_group(
    ...
    *,
    ...
    group = QGroupBox(f"카트리지 / 드라이브 상세 ({item_count}개)")
    ...
    if context_key == "theory":
                ...
                WarehouseResultCard(
                    {
                        ...
                        "display_name": str(
                            ...
                            or ("카트리지" if kind == "core" else "드라이브")
                        ),
        ...
        if not theory_items:
            grid.addWidget(QLabel("공식 방안에 카트리지나 드라이브가 없습니다."), 0, 0)
    else:
        if not items:
            grid.addWidget(QLabel("카트리지나 드라이브가 없습니다."), 0, 0)
...
def _aggregate_equipment_stats(detail: dict, context_key: str) -> list[tuple[str, str]]:
    if context_key == "theory":
        return [
            (_attribute_name(detail, property_id), "목표 스탯")
...
def _build_drive_summary_group(
    ...
    *,
    ...
    def refresh_summary() -> None:
        ...
        count_label.setText(f"장착된 드라이브: {len(modules)}    콘솔: {'已装配' if cores else '未装配'}")
        ...
        if getattr(window, "scoring_engine", None) is None or not score_items:
            ...
            score_label.setText(
                f"장비 세팅 점수: {total_score:.1f} · {loadout_total_grade(total_score)}"
            )
        gain = calculate_official_role_equipment_gain(calculation_detail, context_key)
        if gain:
            margin_label.setText(f"직접 피해 이득: {gain['gain_percent']:+.2f}%")
        else:
            margin_label.setText("직접 피해 이득: --")
        rows = _aggregate_equipment_stats(calculation_detail, context_key)
        if not rows:
            summary_layout.addWidget(QLabel("(드라이브/콘솔이 없습니다. 먼저 가방을 동기화하거나 장비 세팅 방안을 저장하세요)"))
        else:
            info_group = QGroupBox("요약 속성 (실시간 계산)")
