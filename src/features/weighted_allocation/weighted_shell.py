...
def build_weighted_allocation_page(window) -> QWidget:
    ...
    layout.addWidget(QLabel("캐릭터를 선택하고 우선순위를 조정한 뒤 통합 세팅을 시작하세요."))

    selector_card = window._card("캐릭터 우선순위")
    ...
    actions = QHBoxLayout()
    window.weighted_run_button = QPushButton("계산 시작")
    window.weighted_run_button.setObjectName("btnPrimary")
    window.weighted_save_button = QPushButton("방안 저장")
    window.weighted_save_button.setEnabled(False)
    window.weighted_one_key_button = QPushButton("원클릭 장착")
    window.weighted_one_key_button.setObjectName("btnPrimary")
    window.weighted_one_key_button.setEnabled(False)
    window.weighted_one_key_button.setToolTip("설정의 장착 실행 방식에 따라 현재 통합 방안을 장착")
    window.weighted_automatic_button = QPushButton("자동 장착")
    window.weighted_automatic_button.setObjectName("btnPrimary")
    window.weighted_automatic_button.setEnabled(False)
    window.weighted_automatic_button.setToolTip("게임 내 조작을 모사해 현재 통합 방안을 단계별로 장착")
...
def _hide_legacy_selector_controls(selector: RoleSelector) -> None:
    for button in selector.findChildren(QPushButton):
        if button.text() in {"복원", "저장", "불러오기", "초기화"}:
            button.hide()
        elif button.text() == "?":
            button.hide()
    for label in selector.findChildren(QLabel):
        if "초기화는 현재 화면에만 적용" in label.text():
            label.setText(
                "클릭해 캐릭터를 선택하고 순서를 조정하세요. “=”는 같은 등급의 공동 분배를 뜻합니다."
                "캐릭터 관리 설정은 계산 시 현재 계정에 자동 저장됩니다."
            )
...
def refresh_weighted_allocation_page(window) -> None:
    ...
    try:
        ...
        window.weighted_status_label.setText(f"캐릭터 목록을 읽을 수 없습니다: {exc}")
