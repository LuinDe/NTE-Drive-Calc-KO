...
def _load_weighted_persistence(window, database_path: Path) -> None:
    ...
    if persistence.restore_request is None:
        if weights_changed:
            window.weighted_status_label.setText("계정 스탯 가중치를 갱신했습니다. 캐릭터와 카트리지 선택은 유지되니 다시 계산하세요.")
        elif persistence.profile_version is None:
            window.weighted_status_label.setText("캐릭터를 선택하고 우선순위를 설정하세요.")
        else:
            window.weighted_status_label.setText(
                f"카트리지 선호도(v{persistence.profile_version})를 자동으로 읽었습니다. 이 버전에 완전히 대응하는 저장 방안을 찾지 못했습니다."
            )
        return
    token = object()
    window._weighted_restore_token = token
    window.weighted_status_label.setText(f"카트리지 선호도(v{persistence.profile_version})를 자동으로 읽었으며 저장 방안을 복원하는 중…")
    worker = WorkerThread(
...
def _on_weighted_restore_done(
    ...
    window.weighted_status_label.setText(
        f"저장 방안을 자동으로 읽었습니다: 캐릭터 {len(preview.result.unified.selected)}명, 카트리지 선호도 v{persistence.profile_version}."
    )
...
def _on_weighted_restore_error(
    ...
    window._weighted_allocation_restore_worker = None
    window.weighted_status_label.setText(
        f"카트리지 선호도(v{persistence.profile_version})를 자동으로 읽었지만 저장 방안을 안전하게 복원할 수 없습니다: {error}"
    )
...
def _mark_weighted_preferences_dirty(window) -> None:
    ...
    if hasattr(window, "weighted_status_label"):
        window.weighted_status_label.setText("설정이 수정되었습니다. 다시 계산하세요.")
...
def _selection_rows(window) -> list[dict[str, Any]]:
    ...
    def row_for(name: str, ordinal: int) -> dict[str, Any]:
        ...
        if (
            ...
            raise ValueError(f"[{name}]의 세트가 비어 있습니다. 먼저 캐릭터 관리에서 세트를 선택하세요.")
...
def _build_single_select_row(selector: RoleSelector, title: str, choices: list[str], selected: list[str]) -> QWidget:
    ...
    def refresh_summary() -> None:
        text = selected[0] if selected else "선택 안 함"
...
def _show_empty_curtain_preferences(window, role_name: str) -> None:
    ...
    dialog.setWindowTitle(f"{role_name} · 관리")
    ...
    config_box = QGroupBox("카트리지 설정")
    config_layout = QVBoxLayout(config_box)
    suit_row = QHBoxLayout()
    suit_row.addWidget(QLabel("세트:"))
    ...
    stat_help.clicked.connect(
        lambda: selector._show_help("스탯 필터와 우선순위 설명", WEIGHTED_STAT_FILTER_HELP)
    )
    ...
    other_box = QGroupBox("기타 설정")
    other_layout = QVBoxLayout(other_box)
    effect_row = QHBoxLayout()
    effect_row.addWidget(QLabel("세트 효과:"))
    effect_combo = QComboBox()
    effect_combo.addItem("4피스", FOUR_PIECE)
    effect_combo.addItem("2피스", TWO_PIECE)
    effect_combo.addItem("효과 없음", NO_EFFECT)
    ...
    effect_help.clicked.connect(
        lambda: selector._show_help("세트 효과 설명", WEIGHTED_SET_EFFECT_HELP)
    )
    effect_row.addWidget(effect_help)
    other_layout.addLayout(effect_row)

    crit_row = QHBoxLayout()
    crit_row.addWidget(QLabel("치명타 확률 최소값:"))
    ...
    crit_help.clicked.connect(
        lambda: selector._show_help("치명타 확률 최소값", WEIGHTED_CRIT_THRESHOLD_HELP)
    )
...
def _current_snapshot_and_profile(window) -> tuple[int, int, int]:
    ...
    if not rows:
        raise ValueError("먼저 캐릭터를 최소 한 명 선택하세요.")
    with UserDataDao(weighted_allocation_dependencies(window).user_database_path) as dao:
        ...
        if snapshot is None:
            raise ValueError("사용 가능한 완전한 가방 스냅샷이 없습니다. 먼저 가방 동기화를 완료하세요.")
