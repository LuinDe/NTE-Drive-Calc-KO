    ...
    def __init__(
        ...
        *,
        ...
        for option in options:
            ...
            name = getattr(window, "_weighted_role_names", {}).get(option.character_id, "캐릭터")
            placeholder_layout.addWidget(QLabel(f"{name}의 결과를 준비하는 중…"))
...
def _role_option_card(
    ...
    if changed_comparisons:
        ...
        diff_button.setToolTip("저장된 장비 세팅 슬롯을 선택해 이번 결과의 신규·기존 장비와 속성 변화를 확인합니다")
    ...
    role_header.addWidget(_result_badge("점수", f"{option.score:.1f}", grade_color))
    role_header.addWidget(_result_badge("등급", grade, grade_color))
    equip_button = QPushButton("장착")
    ...
    if option.generated_board:
        layout.addWidget(_section_label("퍼즐 청사진:"))
    ...
    if equipment_assignments:
        ...
        layout.addWidget(_section_label(f"카트리지 / 드라이브 ({len(equipment_assignments)}개):"))
