...
def restore_weighted_allocation_preview(
    ...
    if set(selected) != set(expected):
        raise RuntimeError("저장된 방안이 현재 고정 스냅샷의 사용 가능 캐릭터와 일치하지 않습니다.")
    updated_options = []
    ...
    unified = replace(
        ...
        explanation=tuple(preview.result.unified.explanation) + ("현재 공식 청사진 기준으로 저장 방안을 복원했습니다",),
    )
    return replace(
        preview,
...
def _restore_saved_option(context, option, signature: WeightedSavedPlanSignature):
    ...
    if role is None:
        raise RuntimeError(f"캐릭터 {signature.character_id}에 현재 공식 청사진이 없습니다.")
    official_cells = {
    ...
    for saved in signature.assignments:
        ...
        if (
            ...
            raise RuntimeError(
                f"캐릭터 {signature.character_id}의 수동 교체 방안에 검증 가능한 장비나 점수가 없습니다."
            )
        ...
        if saved.kind == "core":
            ...
            if shape is None or saved.target_row is None or saved.target_column is None:
                raise RuntimeError(f"캐릭터 {signature.character_id}의 수동 드라이브에 공식 좌표가 없습니다.")
            ...
            if (
                ...
                raise RuntimeError(
                    f"캐릭터 {signature.character_id}의 수동 교체 배치가 현재 공식 청사진과 맞지 않습니다."
                )
        ...
        restored.append(AllocationAssignment(
            ...
            contributions=(),
            compatibility=("저장된 방안", "현재 공식 청사진 검증"),
    ...
    if occupied_cells != official_cells:
        raise RuntimeError(
            f"캐릭터 {signature.character_id}의 수동 교체 배치가 현재 공식 청사진을 완전히 덮지 않습니다."
        )
...
def run_weighted_allocation(request: WeightedAllocationRequest) -> WeightedAllocationPreview:
    ...
    if not request.user_database_path.is_file():
        raise RuntimeError("사용 가능한 계정 가방 데이터베이스가 없습니다. 먼저 가방 동기화를 완료하세요.")
    if not 1 <= int(request.top_k) <= 20:
        raise ValueError("Top-K는 1에서 20 사이여야 합니다.")
...
def save_weighted_allocation_preview(
    ...
    *,
    ...
    if not result.unified.selected:
        raise RuntimeError("저장할 통합 분배 방안이 없습니다.")
...
def replace_weighted_allocation_assignment(
    ...
    *,
    ...
    if candidate is None:
        raise RuntimeError(f"교체 장비 UID {new_uid}이(가) 계산에 고정된 가방 스냅샷에 없습니다.")
    target_owner = next(
        (
            option
            for option in preview.result.unified.selected
    ...
    if target_owner is None:
        raise RuntimeError(f"현재 임시 방안에 교체 대상 UID {old_uid}이(가) 없습니다.")
    if displaced_owner is target_owner:
        raise RuntimeError("같은 캐릭터의 현재 방안에 있는 다른 장비로 중복해서 자리를 차지할 수 없습니다.")
    target_role = next(
        (role for role in preview.context.roles if role.character_id == target_owner.character_id),
        None,
    )
    if target_role is None:
        raise RuntimeError(f"현재 방안에 캐릭터 {target_owner.character_id}의 점수 설정이 없습니다.")
        ...
        for ordinal, assignment in enumerate(option.assignments):
            ...
            if assignment.uid == new_uid:
                ...
                updated_assignments.append(replace(
                    ...
                    score=0.0,
                    contributions=(),
                    compatibility=("다른 캐릭터 장비 대여 자리 표시", "가방 후보 풀에 속하지 않음"),
                    virtual=True,
                    grid_count=(
    ...
    if changed_count != 1:
        raise RuntimeError(
            f"현재 방안에는 교체 대상 UID {old_uid}이(가) 정확히 하나 있어야 하지만 실제로는 {changed_count}개입니다."
        )
    if displaced_owner is not None and displaced_count != 1:
        raise RuntimeError(
            f"현재 임시 방안에서 대여된 장비 UID {new_uid}을(를) 유일하게 찾을 수 없습니다."
        )
    unified = replace(
        preview.result.unified,
        total_score=sum(float(option.score) for option in updated_options),
        ...
        explanation=tuple(preview.result.unified.explanation) + (
            "사용자 수동 최적화 교체. 다른 캐릭터 장비를 대여하면 원래 슬롯에는 가상 자리 표시를 사용",
        ),
    )
    return replace(
        preview,
