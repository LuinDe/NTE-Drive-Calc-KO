...
class RewindExecutionUiMixin:
    ...
    def _configure_rewind(self) -> None:
        ...
        if self._rewind_options.drive_customization == "apply_plan" and len(self._saved_rewind_shape_ids) != 8:
            self._start_rewind_button.setText("먼저 8슬롯 방안을 저장하세요")
            return
    ...
    def _start_rewind_execution(self) -> None:
        ...
        if active_owner not in (None, self._rewind_hotkey_owner):
            QMessageBox.information(
                self,
                "되감기 진행",
                "현재 전역 중지 키를 다른 작업이 사용 중입니다. 먼저 그 작업을 중지한 뒤 되감기를 진행하세요.",
            )
            return
        ...
        self._start_rewind_button.setText("되감기 중…")
    ...
    def _on_rewind_complete(self, report: Any) -> None:
        ...
        if quality_remaining:
            ...
            remaining_text = "、".join(
                f"{quality_labels.get(quality, quality)} {remaining} 남음"
                for quality, remaining in quality_remaining
            )
        else:
            remaining_text = f"남은 수량 {getattr(report, 'remaining_currency', 0)}"
        self._start_rewind_button.setText(f"되감기 {draws}회 완료, {remaining_text}")
