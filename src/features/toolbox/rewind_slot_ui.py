...
class RewindSlotUiMixin:
    ...
    def _initialize_rewind_slots(
        ...
        self._render_rewind_slots(
            "저장된 방안" if self._slots_complete() else "사용자 정의 방안",
            "마지막으로 저장한 8슬롯 방안을 불러왔습니다." if self._slots_complete() else "빈 슬롯을 클릭해 12종 드라이브 중 하나씩 추가하세요.",
        )
    ...
    def _render_rewind_slots(self, title: str, detail: str) -> None:
        ...
        clear_button = QPushButton("후보 비우기")
        ...
        self._save_plan_button.setText("방안 저장" if complete else "8슬롯을 채운 뒤 저장")
    ...
    def _populate_rewind_slot_grid(self, grid: QGridLayout) -> None:
        ...
        for index, recommendation in enumerate(self._editable_slots):
            ...
            slot_label = QLabel(f"{index + 1}번 슬롯")
            ...
            if recommendation is None:
                ...
                shape_label = QLabel(f"{recommendation.shape.cell_count}형 드라이브")
                ...
                probability_label = QLabel(
                    f"점수 부족 {recommendation.quality_gap:g} ·"
                    f"보유 {recommendation.owned_count} · 확률 {probability:.0%}"
                )
    ...
    def _edit_rewind_slot(self, slot_index: int) -> None:
        ...
        self._render_rewind_slots("사용자 정의 방안", "슬롯을 클릭하면 형태를 하나씩 계속 조정할 수 있습니다.")
    ...
    def _clear_rewind_slots(self) -> None:
        ...
        self._render_rewind_slots(
            "사용자 정의 방안",
            "현재 페이지 후보를 비웠습니다. 저장된 방안은 그대로이며 다시 열면 다시 불러옵니다.",
        )
