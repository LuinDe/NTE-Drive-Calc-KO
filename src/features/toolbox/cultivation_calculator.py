...
class CultivationCalculatorDialog(QDialog):
    ...
    def _build(self) -> None:
        ...
        note = QLabel(
            "캐릭터 페이지에 저장된 육성 상태로 미리 채웁니다. 정식 재료만 합산하며 가방 인벤토리를 차감하거나 체력을 추정하지는 않습니다.",
            self,
        )
        ...
        configuration_caption = QLabel("육성 목표", input_body)
        ...
        self._role = QPushButton("캐릭터 선택", controls)
        ...
        grid.addWidget(QLabel("캐릭터", controls), 0, 0)
        grid.addWidget(self._role, 0, 1, 1, 2)
        self._character_toggle = _participation_toggle(controls, "캐릭터 육성")
        ...
        grid.addWidget(QLabel("현재 레벨", controls), 1, 0)
        grid.addWidget(self._current_level, 1, 1)
        grid.addWidget(self._current_stage, 1, 2)
        grid.addWidget(QLabel("목표 레벨", controls), 2, 0)
        ...
        self._fork = QPushButton("아크 선택", self._fork_controls)
        ...
        fork_grid.addWidget(QLabel("아크", self._fork_controls), 0, 0)
        fork_grid.addWidget(self._fork, 0, 1, 1, 2)
        self._fork_toggle = _participation_toggle(self._fork_controls, "아크 육성")
        ...
        fork_grid.addWidget(QLabel("현재 레벨", self._fork_controls), 1, 0)
        fork_grid.addWidget(self._fork_current_level, 1, 1)
        fork_grid.addWidget(self._fork_current_stage, 1, 2)
        fork_grid.addWidget(QLabel("목표 레벨", self._fork_controls), 2, 0)
        ...
        skill_caption = QLabel("스킬 목표", input_body)
        ...
        self._skills_toggle = _participation_toggle(input_body, "스킬 목표")
        ...
        calculate = QPushButton("필요 재료 계산", input_body)
        ...
        self._copy_button = buttons.addButton("목록 복사", QDialogButtonBox.ActionRole)
    ...
    def _rebuild_skills(self, seed: CultivationSeed) -> None:
        ...
        if not seed.skills:
            self._skills_layout.addRow(QLabel("현재 정식 정적 라이브러리에서 업그레이드 가능한 스킬을 제공하지 않습니다.", self._skills_box))
            return
        for skill in seed.skills:
            ...
            row_layout.addWidget(QLabel("현재", row))
            row_layout.addWidget(current)
            row_layout.addWidget(QLabel("대상", row))
    ...
    def _calculate(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "육성 계산기", str(exc))
            return
        except Exception as exc:
            QMessageBox.warning(self, "육성 계산기", f"재료 계산 실패: {exc}")
            return
    ...
    def _render_plan(self, plan: CultivationPlan) -> None:
        ...
        summary = QLabel(
            "재료 데이터 완전" if complete else "재료 데이터가 불완전합니다. 아래는 식별된 재료입니다",
            self._result_body,
        )
