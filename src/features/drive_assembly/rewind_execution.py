...
def execute_rewind_request(
    ...
    *,
    ...
    if stop_checker():
        raise RuntimeError("되감기 실행이 중지되었습니다")
    _image, rect = capture()
    ...
    for quality in selected_qualities:
        ...
        if currency is None:
            raise RuntimeError(f"{quality_difficulty}의 되감기 화폐 수량을 인식하지 못했습니다")
        ...
        if uses_custom_price:
            ...
            if custom_cost is None or custom_cost <= 0:
                raise RuntimeError(f"{quality_difficulty}의 맞춤 10연 가격을 인식하지 못했습니다")
...
def _validate_rewind_request(request: RewindExecutionRequest) -> None:
    ...
    if request.drive_customization == "apply_plan" and len(request.shape_ids) != 8:
        raise ValueError("되감기 방안을 적용하려면 드라이브 후보가 정확히 8개여야 합니다")
...
def _shape_control_name(shape_id: str) -> str:
    ...
    if normalized not in {
        ...
        raise ValueError(f"매핑되지 않은 되감기 드라이브 형태: {shape_id}")
    return normalized
