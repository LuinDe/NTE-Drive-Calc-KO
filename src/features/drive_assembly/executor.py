    ...
    for action in sequence:
        ...
        if _execute_one_action(action, mouse, should_stop=should_stop, runtime_state=runtime_state):
            ...
            if action.get("duplicate_status_filter"):
                logger.info(
                    "중복 장비 상태 필터 실행됨 |"
                    f"캐릭터={role_name or '未指定'} | 블록={action.get('block_id', '未指定')} |"
                    f"그룹={action.get('duplicate_group_id') or '未分组'} | 상태={action.get('name')}"
                )
            ...
            if on_action_executed is not None:
                try:
                    ...
                    logger.warning(f"장착 동작 기록 콜백 실패 | {_action_diagnostic(action)} | 원인={exc}")
        else:
            report.skipped_actions.append(dict(action))
            logger.warning(
                "장착 동작 건너뜀 |"
                f"캐릭터={role_name or '未指定'} | {_action_diagnostic(action)} |"
                "원인=지원하지 않는 동작이거나 사전 검사 실패"
            )
    return report
...
def execute_role_assembly_plan(
    ...
    logger.info(
        "캐릭터 장착 시작 |"
        f"캐릭터={role_name or '未指定'} | 카트리지={plan.get('tape_count', 0)} |"
        f"드라이브={plan.get('drive_count', 0)} | 최상위 동작={[action.get('name') for action in plan.get('actions', [])]}"
    )
    ...
    logger.info(
        "캐릭터 장착 종료 |"
        f"캐릭터={role_name or '未指定'} | 실행됨={report.executed_actions} |"
        f"건너뜀={[action.get('name', '未命名') for action in report.skipped_actions]}"
    )
    return report
...
def execute_role_traversal_assembly_plan(
    ...
    logger.info(
        "장착 순회 실행 시작 |"
        f"계획 캐릭터={[step.get('role_name') for step in traversal_plan.get('plans', [])]} |"
        f"누락={report.missing_roles} | 미인식={report.unrecognized_roles} | 중복 캐릭터={report.duplicate_roles}"
    )
    for step in traversal_plan.get("plans", []):
        logger.info(
            "캐릭터 경로 실행 |"
            f"캐릭터={step.get('role_name')} | 시작 인덱스={step.get('start_roster_index')} |"
            f"대상 인덱스={step.get('roster_index')} |"
            f"내비게이션={[action.get('gamepad_button') or action.get('gamepad_stick') or action.get('name') for action in step.get('action_sequence', []) if not _is_role_blueprint_assembly_action(action)]}"
        )
        ...
        for action in step.get("action_sequence", []):
            ...
            if role_plan is None:
                if role_name:
                    report.skipped_roles.append(role_name)
                logger.warning(f"캐릭터 장착 건너뜀 | 캐릭터={role_name or '未指定'} | 원인=대응하는 장착 계획 없음")
                continue
            role_report = execute_role_assembly_plan(
                role_plan,
                backend=mouse,
            ...
            if role_verifier is not None:
                ...
                if verification and not verification.get("ok", True):
                    ...
                    logger.warning(f"캐릭터 장착 검증 실패 | 캐릭터={role_name} | 상세={verification}")
            report.role_reports.append(role_report)
        if pending_actions:
            action_report = execute_action_sequence(
                pending_actions,
    ...
    logger.info(
        "장착 순회 실행 종료 |"
        f"내비게이션 동작={report.navigation_actions} | 캐릭터 보고={[(item.role_name, item.executed_actions) for item in report.role_reports]} |"
...
def _action_diagnostic(action: dict[str, Any]) -> str:
    ...
    fields = [f"동작={action.get('name', '未命名')}"]
...
def _retry_unselected_quality(
    ...
    if capture is None:
        logger.warning(f"드라이브 모듈 {action.get('block_id')} 검증 불가 | 원인=백엔드가 스크린샷을 지원하지 않음")
        return False
    try:
        image = capture()
    except Exception as exc:
        logger.warning(f"드라이브 모듈 {action.get('block_id')} 검증 불가 | 원인=스크린샷 실패 | 예외={exc!r}")
        return False
...
def _retry_missing_drive_block(
    ...
    if not target or not retry_from or not retry_to:
        logger.warning(f"드라이브 모듈 {action.get('block_id')} 검증 불가 | 원인=재시도 좌표 없음 | target={target} from={retry_from} to={retry_to}")
        return False
...
def _capture_drive_target_baseline(
    ...
    if capture is None or not target:
        logger.warning(
            f"드라이브 모듈 {action.get('block_id')} 기준선 미수집 |"
            f"원인={'后端不支持截图' if capture is None else '缺少目标坐标'} | target={target}"
        )
        return False
    try:
        image = capture()
    except Exception as exc:
        logger.warning(f"드라이브 모듈 {action.get('block_id')} 기준선 미수집 | 원인=스크린샷 실패 | 예외={exc!r}")
        return False
    sample = _drive_target_sample(image, _point(target), int(action.get("sample_radius") or 12))
    if sample is None:
        logger.warning(f"드라이브 모듈 {action.get('block_id')} 기준선 미수집 | 원인=대상 샘플 무효 | target={target}")
        return False
