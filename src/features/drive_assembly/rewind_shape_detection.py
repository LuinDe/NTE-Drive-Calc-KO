...
def detect_rewind_shape_positions(
    ...
    if array.ndim != 3 or array.shape[2] < 3:
        raise RuntimeError("되감기 선택 페이지 스크린샷 형식이 잘못되었습니다")
    if len(candidate_centers) != len(_SHAPE_NAMES):
        raise RuntimeError(
            f"되감기 선택 페이지 후보 슬롯은 {len(_SHAPE_NAMES)}개여야 하지만 실제로는 {len(candidate_centers)}개입니다"
        )
    ...
    if weak:
        details = ", ".join(
            f"슬롯{index + 1}:{name}={score:.2f}" for index, name, score in weak
        )
        raise RuntimeError(f"되감기 선택 페이지 형태 인식 신뢰도가 부족합니다 ({details}; 총점 {total_score:.2f})")
...
def _best_unique_assignment(score_rows: Sequence[Sequence[float]]) -> tuple[tuple[int, ...], float]:
    ...
    if complete is None:
        raise RuntimeError("되감기 선택 페이지에서 12종 드라이브 형태가 완전히 매칭되지 않았습니다")
