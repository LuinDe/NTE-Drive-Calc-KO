...
_STATE_LABELS = {
    "normal": "일반 레벨",
    "breakthrough_before": "돌파 전",
    "breakthrough_after": "돌파 후",
    "max_level": "만렙",
}
...
def _source_label(source: CatalogSource | None) -> str:
    if source is None:
        return "—"
    location = source.relative_path or source.row_key or "경로 없음"
...
class CharacterDetailPanel(QWidget):
    ...
    def _build(self) -> None:
        ...
        self.tabs.addTab(self.overview_scroll, "개요 및 육성")

        self.growth_tab = self._build_growth_tab()
        self.tabs.addTab(self.growth_tab, "1–80레벨 패널")
    ...
    def set_detail(self, detail: CharacterDetail | None) -> None:
        ...
        if detail is None:
            self.title_label.setText("캐릭터를 선택해 자료를 확인하세요")
            self.dataset_label.setText("읽기 전용 배포 정적 라이브러리")
            self.overview_layout.addWidget(self._empty_label("캐릭터 상세 정보가 없습니다"))
            self.overview_layout.addStretch()
        else:
            ...
            self.dataset_label.setText(
                f"{dataset.dataset_id} · schema {dataset.schema_version} · importer {dataset.importer_version} · 읽기 전용"
            )
    ...
    def _render_overview(self, detail: CharacterDetail) -> None:
        ...
        identity = self._section("캐릭터 식별 정보", "character")
        identity.layout().addLayout(self._copy_row("정식 character_id", str(character.character_id)))
        identity.layout().addLayout(self._copy_row("중국어 이름", character.name_zh))
        identity.layout().addLayout(self._copy_row("속성 유형", f"{character.element_label} · {_text(character.element_type)}"))
        identity.layout().addLayout(self._copy_row("진영/그룹", _text(character.group_type)))
        identity.layout().addLayout(self._copy_row("캐릭터 리소스 경로", _text(character.actor_path)))
        identity.layout().addLayout(self._copy_row("논리 캐릭터 키", _text(character.logical_character_key)))
        identity.layout().addLayout(self._copy_row("필드 출처", _source_label(character.source)))
        self.overview_layout.addWidget(identity)

        breakthrough = self._section("레벨 곡선 및 돌파 단계", "character_panel_growth")
        if detail.breakthroughs:
            for stage in detail.breakthroughs:
                label = QLabel(
                    f"{stage.level}레벨 · 단계 {stage.stage}　"
                    f"돌파 전 HP/ATK/DEF {_number(stage.before.hp_base)} / {_number(stage.before.atk_base)} / {_number(stage.before.def_base)}　→　"
                    f"돌파 후 {_number(stage.after.hp_base)} / {_number(stage.after.atk_base)} / {_number(stage.after.def_base)}"
                )
            ...
            breakthrough.layout().addWidget(self._empty_label("이 캐릭터에는 정식 레벨/돌파 패널이 없습니다"))
    ...
    def _likeability_section(self, detail: CharacterDetail) -> QFrame:
        section = self._section("호감도", "character_likeability_bonus")
        bonus = detail.likeability
        if bonus is None:
            section.layout().addWidget(self._empty_label("정적 라이브러리에 이 캐릭터의 호감도 정식 속성 수정 데이터가 없습니다"))
            return section
        section.layout().addLayout(self._copy_row("적용 레벨", str(bonus.required_level)))
        section.layout().addLayout(self._copy_row("수정 ID", bonus.modify_data_id))
    ...
    def _awakening_section(self, detail: CharacterDetail) -> QFrame:
        section = self._section("각성 및 구조화 효과", "character_awaken_effect")
        if not detail.awakenings:
            section.layout().addWidget(self._empty_label("정적 라이브러리에 이 캐릭터의 각성 행이 없습니다"))
            return section
        for effect in detail.awakenings:
            title = effect.title_zh or effect.effect_id
            summary = QLabel(
                f"{effect.ordinal + 1}. {title}　[{effect.awaken_type}]　"
                f"구조 필드 {len(effect.structured_effects)} · GE {len(effect.gameplay_effect_ids)}"
            )
            ...
            section.layout().addLayout(self._copy_row("정식 effect_id", effect.effect_id))
            ...
            if effect.gameplay_effect_ids:
                section.layout().addLayout(self._copy_row("정식 GE", "\n".join(effect.gameplay_effect_ids)))
        return section

    def _skill_section(self, detail: CharacterDetail) -> QFrame:
        section = self._section("스킬, 레벨 및 업그레이드 소모", "character_skill / gameplay_ability_catalog")
        if not detail.skills:
            section.layout().addWidget(self._empty_label("정적 라이브러리에 이 캐릭터의 스킬 목록이 없습니다"))
            return section
        for skill in detail.skills:
            section.layout().addLayout(self._copy_row(
                skill.name_zh or skill.skill_id,
                f"{skill.skill_id} · {skill.ability_type} · 레벨 소모 기록 {len(skill.levels)}건",
            ))
            for path in (skill.gameplay_tag, skill.gameplay_ability_path, skill.gameplay_effect_path):
                if path:
                    section.layout().addLayout(self._copy_row("정식 식별자/경로", path))
            for level in skill.levels:
                costs = "、".join(
                    f"{item.item_id} × {_number(item.quantity)}" for item in level.costs
                ) or "소모 항목 없음"
                section.layout().addLayout(self._copy_row(
                    f"레벨 {level.level} (돌파 {level.required_breakthrough_stage} / 각성 {level.required_awaken_level})",
                    costs,
                ))
        return section

    def _graduation_section(self, detail: CharacterDetail) -> QFrame:
        section = self._section("육성 가이드, 졸업 템플릿 및 전용 무기", "character_cultivation_* / character_graduation_template")
        guide = detail.cultivation
        if guide is not None:
            section.layout().addLayout(self._copy_row("육성 점수", f"S {guide.s_score} · A {guide.a_score}"))
            for fork_id, name, description in guide.fork_recommendations:
                section.layout().addLayout(self._copy_row(f"추천 아크 · {name}", fork_id))
                if description:
        ...
        if graduation is None:
            section.layout().addWidget(self._empty_label("정적 라이브러리에 이 캐릭터의 졸업 템플릿이 없습니다"))
            return section
        section.layout().addLayout(self._copy_row(
            "졸업 아크/전용 무기",
        ...
        for path in graduation.fork_paths:
            section.layout().addLayout(self._copy_row("전용 무기 리소스 경로", path))
        return section

    def _gap_section(self, detail: CharacterDetail) -> QFrame:
        section = self._section("데이터 가용성", "schema / manifest")
    ...
    def set_growth_page(self, page: GrowthPage) -> None:
        ...
        self.growth_hint.setText("공식 기본 패널. 경계 레벨의 돌파 전후는 별도 행입니다")
        self._update_growth_controls()
    ...
    def set_combat_page(self, page: CombatLinkPage) -> None:
        ...
        self.combat_hint.setText("정식 캐릭터 바인딩, GA 리소스, 이벤트→GE/Buff, 그리고 캐릭터 소속 Buff")
        self._update_combat_controls()
