...
class ForkOwnedResources:
    ...
    def close_all(self) -> None:
        ...
        if failures:
            raise ExceptionGroup("아크 도감 리소스 닫기 실패", failures)
...
class ForkGalleryCard(QFrame):
    ...
    def _build(
        ...
        badges.addStretch(1)
        campaign_title = campaign.title.display_name if campaign else "첫 출시"
        limited = QLabel(
...
class ForkCatalogPage(QWidget):
    ...
    def _build_gallery(self) -> QWidget:
        ...
        title = QLabel("아크 도감", hero)
        title.setStyleSheet(themed_style(
            "color:#f0f6fc;font-size:25px;font-weight:900"
        ))
        subtitle = QLabel(
            "한정 특집을 우선 표시합니다. 카드를 클릭하면 레벨 패널, 믹싱 스킬, 특수 효과 관계, 전체 육성 경로를 확인할 수 있습니다.",
            hero,
        )
        ...
        self.search_edit.setPlaceholderText("아크 이름 또는 ID 검색")
        self.search_edit.textChanged.connect(self.set_search_query)
        toolbar.addWidget(self.search_edit, 1)
        self.filter_toggle = QPushButton("필터  ▾", host)
    ...
    def _build_filter_panel(self, parent: QWidget) -> QFrame:
        ...
        quality_label = QLabel("품질", panel)
        type_label = QLabel("유형", panel)
    ...
    def _render_gallery(self) -> None:
        ...
        self._visible = tuple(
            ...
            if not query or _ko_contains(item.name_zh, query) or query in item.fork_id.casefold()
        )
        ...
        self.filter_toggle.setText(
            f"필터{' · ' + str(active_count) if active_count else ''}  "
        ...
        if limited:
            self._create_section("한정 특집 · 최신순", limited)
        if regular:
            self._create_section("첫 출시 아크 · 품질 / 유형 / 이름", regular)
        if not self._visible:
            empty = QLabel("조건에 맞는 아크를 찾지 못했습니다", self._gallery_host)
    ...
    def catalog_back_label(self) -> str | None:
        return "아크 목록" if self._stack.currentIndex() == 1 else None
