...
class FeastEncounterView(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        difficulty_label = QLabel("난이도", self.controls)
        ...
        self.condition_summary = QLabel("도전 조건 미적용", self.controls)
        ...
        self.conditions_toggle.setText("도전 조건")
        ...
        self.blessing_toggle.setText("마녀의 축복")
        ...
        blessing_label = QLabel("전투 전 축복", self.blessing_host)
        ...
        self.blessing_summary = QLabel("마녀의 축복 미선택", self.blessing_host)
    ...
    def set_stage(
        ...
        *,
        ...
        self.heading.setText(
            f"쟁봉 연회 · {setup.period_label} · 도전 {setup.challenge_ordinal}"
        )
        ...
        self.blessing_combo.addItem("선택 안 함", "")
    ...
    def _toggle_options(self, expanded: bool) -> None:
        ...
        self.conditions_toggle.setText("조건 접기" if expanded else "도전 조건")

    def _toggle_blessing(self, expanded: bool) -> None:
        self.blessing_host.setVisible(expanded)
        self.blessing_toggle.setText("축복 접기" if expanded else "마녀의 축복")
    ...
    def _refresh_blessing(self) -> None:
        ...
        if not key or self._blessing_loader is None:
            self.blessing_summary.setText("마녀의 축복 미선택")
            return
        detail = self._blessing_loader(key)
        values = detail.sections[0].values if detail and detail.sections else ()
        if not values:
            self.blessing_summary.setText("축복 효과 미제공")
            return
        value = values[0]
        label = value.display_label or "축복 효과"
        effect = value.display_value or "효과 설명 미제공"
    ...
    def _refresh_detail(self) -> None:
        ...
        self.condition_summary.setText(
            f"{len(selected)}개 적용됨"
            if selected else (self._setup.condition_note or "도전 조건 미적용")
        )
        detail = self._loader(
        ...
        self.detail.set_detail(
            ...
            context=MonsterContext(
                ...
                scene=(
                    ...
                    f"도전 {self._setup.challenge_ordinal} · {self._setup.title}"
                ),
