...
class CharacterDetailView(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self.art = QLabel("일러스트 미제공", self.art_panel)
        ...
        copy = QVBoxLayout()
        copy.setSpacing(4)
        self.eyebrow = QLabel("캐릭터 도감", self.identity_panel)
        self.eyebrow.setStyleSheet(themed_style(
            "color:#58a6ff;font-size:10px;font-weight:900"
        ))
        self.name = QLabel("캐릭터 선택", self.identity_panel)
        ...
        self.release_line = QLabel("출시 정보 미제공", self.identity_panel)
        self.release_line.setStyleSheet(themed_style(
            "color:#8b949e;font-size:10px;font-weight:700"
        ))
        badges = QHBoxLayout()
        self.element_badge = self._badge("속성 미제공", "#58a6ff")
        self.availability_badge = self._badge("획득 정보 미제공", "#3fb950")
        ...
        self.cultivation_tabs.addTab(self.growth_view, "레벨 육성")
        self.cultivation_tabs.addTab(self.skill_training_view, "스킬 육성")
        ...
        self.tabs.addTab(self.overview_host, "캐릭터 프로필")
        self.tabs.addTab(self.skill_view, "스킬")
        self.tabs.addTab(self.cultivation_tabs, "육성")
        self.tabs.addTab(self.awakening_view, "각성")
        self.tabs.addTab(self.affinity_host, "호감도")
        self.tabs.addTab(self.route_view, "청사진 및 졸업")
    ...
    def set_data(
        ...
        if release is not None and release.release_date:
            ...
            self.release_line.setText("출시 정보 미제공")
        self.art.clear()
    ...
    def _render_overview(
        ...
        summary = QLabel(
            f"스킬 {detail.character.skill_count}개  ·  "
            f"각성 {detail.character.awakening_count}개  ·  "
            + ("졸업 템플릿 있음" if detail.character.has_graduation else "졸업 템플릿 없음"),
        )
        ...
        for level in (1, 20, 40, 60, 70, 80):
            ...
            panel_grid.addWidget(self._detail_tile(
                f"Lv.{level}",
                f"HP {_number(point.hp_base)}  ·  공격력 {_number(point.atk_base)}  ·  방어력 {_number(point.def_base)}",
        ...
        if not growth.items:
            panel.layout().addWidget(self._muted("현재 정식 데이터에 레벨 패널이 없습니다"))
    ...
    def _render_affinity(self, detail: CharacterDetail) -> None:
        ...
        if bonus is None:
            self.affinity_layout.addWidget(self._muted("현재 정식 데이터에 호감도 속성이 없습니다"))
        else:
            hero = self._panel(f"호감도 Lv.{bonus.required_level} 보상")
