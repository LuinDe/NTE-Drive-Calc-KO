...
class ShapeGlyph(QWidget):
    ...
    def paintEvent(self, _event) -> None:  # noqa: N802 - Qt override
        ...
        if not self.shape.cells:
            ...
            painter.drawText(self.rect(), Qt.AlignCenter, "형태 없음")
            return
...
class CurvePlot(QLabel):
    ...
    def set_curve(self, curve: AttributeCurve | None) -> None:
        if curve is None or not curve.points:
            self.setText("현재 표시할 강화 곡선이 없습니다")
            return
...
class ExpandableCard(QFrame):
    ...
    def _refresh_toggle_text(self) -> None:
        text = (
            f"접기  {self._title}"
            if self._expanded
            else f"펼치기  {self._title}  ·  {self._summary}"
        )
...
class EquipmentGalleryCard(QFrame):
    ...
    def __init__(self, record: EquipmentRecord, *, owned_count: int | None, shape_name: str, presentation: EquipmentPresentation,
        ...
        line = (
            f"{quality_name} 콘솔 · 최대 Lv.{record.max_level} · 서브 속성 {record.sub_count}개"
            if record.kind == "core"
            else f"{quality_name} 드라이브 · {record.area}칸 · {shape_name}"
        )
        layout.addWidget(_text(line, self, muted=True))
        ownership = (
            "인벤토리를 사용할 수 없음"
            if owned_count is None
            else "미보유" if owned_count == 0 else f"보유 {owned_count}개"
        )
...
class EquipmentDetailView(QScrollArea):
    ...
    def clear_account_projection(self) -> None:
        ...
        self._reset()
        panel, panel_layout = _card(self.host, "현재 계정 인벤토리")
        panel_layout.addWidget(_text(
            "인벤토리를 현재 사용할 수 없습니다. 새로 고침한 뒤 다시 확인하세요.",
            panel,
            muted=True,
        ))
    ...
    def show_equipment(
        ...
        kind = "콘솔 / 카트리지" if record.kind == "core" else "드라이브"
        copy.addWidget(_text(f"{kind}  ·  {quality_label}  ·  최대 Lv.{record.max_level}", hero))
        description = f"소속 세트  {record.suit_name}" if record.kind == "core" else f"{self._controller.shape_name(record.shape_id)}  ·  {record.area}칸 차지"
        copy.addWidget(_text(description, hero, muted=True))
        row.addLayout(copy, 1)
        self.layout.addWidget(hero)
        strength, strength_layout = _card(self.host, "레벨 / 강화 곡선")
        strength_layout.addWidget(_text(
            f"강화 상한 Lv.{record.max_level}  ·  기본 속성 {record.main_count}개  ·  랜덤 서브 속성 {record.sub_count}개",
            strength,
        ))
        ...
        def select_curve(index: int) -> None:
            ...
            if isinstance(curve, AttributeCurve):
                ...
                value.setText(f"만렙 {curve.label}  {number}{'%' if curve.show_percent else ''}")
            else:
                value.setText("현재 장비에는 표시할 강화 곡선이 없습니다")
        selector.currentIndexChanged.connect(select_curve)
        select_curve(selector.currentIndex())
        self.layout.addWidget(strength)
        rules = ExpandableCard("스탯 생성 규칙", f"기본 속성 {record.main_count}개 · 서브 속성 최대 {record.sub_count}개", self.host)
        rules.body_layout.addWidget(_text(
            "속성 후보는 장비 종류, 드라이브 면적, 품질에 따라 달라집니다. 위쪽에서 각 정식 강화 곡선을 전환할 수 있습니다.", rules.body,
        ))
    ...
    def _add_strength_experience(
        ...
        if experience is None:
            layout.addWidget(_text("강화 경험치는 아직 제공되지 않습니다.", strength, muted=True))
            return
        summary = ExpandableCard(
            "강화 경험치",
            f"{len(experience.levels)}개 레벨 · 누적 {experience.total:,}",
            strength,
        )
        running_total = 0
        for start in range(0, len(experience.levels), 5):
            ...
            summary.body_layout.addWidget(_text(
                f"Lv.{first_level}–{last_level}  필요 {segment_exp:,}  ·  누적 {running_total:,}",
                summary.body,
            ))
    ...
    def _add_owned_instances(self, record: EquipmentRecord) -> None:
        ...
        if projection is None:
            panel, panel_layout = _card(self.host, "내 동일 장비")
            panel_layout.addWidget(_text("현재 계정 인벤토리를 지금 사용할 수 없습니다. 새로고침 후 다시 확인하세요.", panel, muted=True))
            self.layout.addWidget(panel)
            return
        expanded = ExpandableCard("내 동일 장비", f"안정 창고에 {len(items)}개 있음", self.host)
        if not items:
            expanded.body_layout.addWidget(_text("현재 안정 창고에 이 장비가 없습니다.", expanded.body, muted=True))
        for number, item in enumerate(items, 1):
            main_name, main_value = item.main_stats[0] if item.main_stats else ("", None)
            card = self._presentation.equipment_card(
                f"{number}번째 · {'等级未知' if not item.level_known else f'Lv.{item.level}'}",
    ...
    def show_suit(self, suit: SuitRecord) -> None:
        ...
        hero_layout.addWidget(_text(
            f"콘솔 세트  ·  드라이브 형태 {len(suit.required_shape_ids)}종 해금  ·  "
    ...
    def _add_suit_sections(self, suit: SuitRecord) -> None:
        shapes, shapes_layout = _card(self.host, "장착 가능 드라이브 형태")
        ...
        for shape_id in suit.required_shape_ids:
            ...
            if shape is not None:
                ...
                tile_layout.addWidget(_text(f"{shape.name} · {shape.area}칸", tile, muted=True))
            else:
                tile_layout.addWidget(_text("형태 관계 정보 없음", tile, muted=True))
            shape_row.addWidget(tile)
        shape_row.addStretch(1)
        shapes_layout.addLayout(shape_row)
        self.layout.addWidget(shapes)
        effects, effects_layout = _card(self.host, "세트 효과")
        for effect in suit.effects:
            summary = effect.description.split(".", 1)[0]
            if len(summary) > 34:
                summary = summary[:34] + "…"
            panel = ExpandableCard(f"{effect.required_count}피스", summary, effects)
            ...
            for property_id, modifier_value in effect.modifiers:
                ...
                panel.body_layout.addWidget(_text(
                    f"고정 보너스  {label} +{value:g}{'%' if percent else ''}", panel.body, muted=True,
                ))
            if effect.has_conditional_effect:
                panel.body_layout.addWidget(_text(
                    "조건, 지속, 중첩 규칙은 현재 읽을 수 있는 설명에 따라 이 카드 안에 표시됩니다.",
                    panel.body,
                    muted=True,
                ))
    ...
    def _add_graduations(self, suit_id: str) -> None:
        ...
        related = ExpandableCard("추천 캐릭터", f"졸업 템플릿 {len(links)}개가 이 세트를 사용합니다", self.host)
        if not links:
            related.body_layout.addWidget(_text("현재 연결된 졸업 템플릿이 없습니다.", related.body, muted=True))
        for link in links:
            label = self._controller.property_info(link.main_property_id)[0]
            related.body_layout.addWidget(_text(
                f"{link.character_name}  ·  추천 메인 속성 {label}  ·  드라이브 총 면적 {link.drive_area}", related.body,
            ))
    ...
    def show_shape(self, shape: ShapeRecord) -> None:
        ...
        row.addWidget(_text(
            f"{shape.area}칸 차지  ·  {self._owned_summary(self._controller.owned_count(shape_id=shape.shape_id), '该形状驱动')}",
            hero,
        ), 1)
        ...
        relation, relation_layout = _card(self.host, "이 형태를 지원하는 콘솔 세트")
    ...
    def _owned_summary(count: int | None, noun: str) -> str:
        if count is None:
            return "현재 계정 인벤토리를 지금 사용할 수 없음"
        return f"현재 {noun} {count}개 보유"
class EquipmentCatalogPage(QWidget):
    ...
    def _build_gallery(self) -> QWidget:
        ...
        hero, hero_layout = _card(page, "콘솔 및 드라이브")
        hero_layout.addWidget(_text(
            "정식 장비 도감, 강화 곡선, 세트 조합을 둘러봅니다. 현재 계정을 연결하면 실제로 보유한 장비도 확인할 수 있습니다.", hero, muted=True,
        ))
        categories = QHBoxLayout()
        self.category_group = QButtonGroup(page)
        self.category_group.setExclusive(True)
        specs = (
            ("콘솔 / 카트리지", "core", len(self._catalog_equipment("core"))),
            ("드라이브", "module", len(self._catalog_equipment("module"))),
            ("세트", "suit", len(self._controller.archive.suits)),
            ("형태", "shape", len(self._controller.archive.shapes)),
        )
        ...
        self.ownership.addItem("전체 도감", "all")
        self.ownership.addItem("보유 항목만 보기", "owned")
    ...
    def _refresh_cards(self) -> None:
        ...
        if self.category_key in {"core", "module"}:
            for record in self._catalog_equipment(self.category_key):
                ...
                if (query and not _ko_contains(haystack, query)) or (quality != "all" and record.quality != quality):
                    continue
            ...
            for suit in sorted(self._controller.archive.suits, key=lambda row: official_suit_number(row.suit_id)):
                ...
                if (query and not _ko_contains(suit.name, query)) or (owned_only and not owned):
                    continue
            ...
            for shape in sorted(self._controller.archive.shapes, key=lambda row: (row.area, row.cells)):
                ...
                if (query and not _ko_contains(shape.name, query)) or (owned_only and not owned):
                    continue
    ...
    def _suit_card(self, suit: SuitRecord, owned: int | None) -> QFrame:
        ...
        layout.addWidget(_text(
            f"드라이브 형태 {len(suit.required_shape_ids)}종 해금  ·  {self._ownership_text(owned)}",
            card, muted=True,
        ))
        button = QPushButton("세트 효과 보기", card)
    ...
    def _shape_card(self, shape: ShapeRecord, owned: int | None) -> QFrame:
        ...
        layout.addWidget(_text(
            f"{shape.name} · {shape.area}칸 · {self._ownership_text(owned)}",
            card, muted=True,
        ))
        button = QPushButton("지원 세트 보기", card)
        button.setObjectName("btnAction")
    ...
    def catalog_back_label(self) -> str | None:
        return "콘솔·드라이브 목록" if self.stack.currentWidget() is self.detail else None
    ...
    def _ownership_text(count: int | None) -> str:
        if count is None:
            return "인벤토리를 사용할 수 없음"
        return "미보유" if count == 0 else f"{count}개 보유"
    def resizeEvent(self, event) -> None:  # noqa: N802 - Qt override
        super().resizeEvent(event)
