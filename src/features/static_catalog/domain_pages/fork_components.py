...
_EVENT_LABELS = {
    "BUFF_EVENT_BEGIN": "효과 시작",
    "BUFF_EVENT_BESKILL_HIT_BEFORE_CALC": "스킬에 피격되기 전",
    "BUFF_EVENT_CHANGE_ROLE_INT": "교대 등장",
    "BUFF_EVENT_CHANGE_ROLE_OUT": "교대 퇴장",
    "BUFF_EVENT_CHANGE_ROLE_OUT_BEGIN": "퇴장 시작",
    ...
    "BUFF_EVENT_QTE_BEGIN": "지원 스킬 발동",
    "BUFF_EVENT_QTE_BEGIN_ALL_PLAYER": "팀이 지원 스킬 발동",
    "BUFF_EVENT_Q_SKILL_BEGIN": "종결 스킬 사용",
    "BUFF_EVENT_SHIELDHEALTHSTART": "보호막 획득",
    "BUFF_EVENT_SKILL_AFTER_HIT": "스킬 히트 후",
    "BUFF_EVENT_SKILL_HIT_BEFORE_CALC": "스킬 히트 전",
    "BUFF_EVENT_SKILL_REALFINISH": "스킬 사용 종료",
    "BUFF_EVENT_SKILL_REALFINISH_ALL_PLAYER": "팀 스킬 사용 종료",
    "BUFF_EVENT_TREATMENT": "치료 수행",
}
...
def present_effects(
    ...
    timing_parts = []
    if applied:
        timing_parts.append(f"{'、'.join(applied)} 시 적용")
    if removed:
        timing_parts.append(f"{'、'.join(removed)} 시 종료")
    result = (
        f"{'、'.join(properties)}에 영향을 주며, 구체적인 수치는 현재 믹싱 스킬 설명을 따릅니다"
        if properties else "효과 내용은 현재 믹싱 스킬 설명을 따릅니다"
    )
    stacking = f"최대 {max(stack_limits)}중첩" if stack_limits else None
    condition = "스킬 설명에 명시된 공격 또는 상태에서만 적용" if has_condition else None
    return ForkEffectPresentation(
        timing="；".join(timing_parts) or "스킬 설명에 명시된 조건에 따라 적용",
        result=result,
        stacking=stacking,
        condition=condition,
    )
...
def add_effect_tiles(
    ...
    rows = [
        ("적용 시점", presentation.timing),
        ("효과 내용", presentation.result),
    ]
    if presentation.stacking:
        rows.append(("중첩 규칙", presentation.stacking))
    if presentation.condition:
        rows.append(("적용 조건", presentation.condition))
