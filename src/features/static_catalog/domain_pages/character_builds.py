...
class CharacterBuildView(QWidget):
    ...
    def _plan_card(self, detail: CharacterDetail) -> QFrame:
        card = self._panel("캐릭터 청사진 · 정식 5×5")
        plan = detail.equipment_plan
        if plan is None:
            card.layout().addWidget(self._muted("현재 정식 데이터에 캐릭터 청사진이 없습니다"))
            return card
        summary = QHBoxLayout()
        ...
        summary.addWidget(self._chip(f"드라이브 템플릿 · Lv.{plan.module_level}"))
        ...
        modules = "、".join(
            f"{module.display_name or '名称暂未提供'} {module.grid_count}칸"
            for module in plan.modules
        )
        card.layout().addWidget(self._info("드라이브 구성", modules))
        names = self._property_names(plan.core_attributes)
        card.layout().addWidget(self._info(
            "콘솔 메인 속성", "、".join(names) or "名称暂未提供",
        ))
        recommended = self._property_names(plan.recommended_attributes)
        card.layout().addWidget(self._info(
            "정식 추천 속성", "、".join(recommended) or "名称暂未提供",
        ))
        return card

    def _cultivation_card(self, detail: CharacterDetail) -> QFrame:
        card = self._panel("정식 육성 방향")
        guide = detail.cultivation
        if guide is None:
            card.layout().addWidget(self._muted("현재 정식 데이터에 육성 방향이 없습니다"))
            return card
        attributes = tuple(
            (
                display_name
        ...
        card.layout().addWidget(self._info(
            "육성 속성", "、".join(attributes) or "名称暂未提供",
        ))
        if not guide.fork_recommendations:
            card.layout().addWidget(self._muted("현재 정식 데이터에 추천 아크가 없습니다"))
        for fork_id, formal_name, description in guide.fork_recommendations:
            ...
            note = QLabel(description or "정식 추천 아크", card)
    ...
    def _shape_card(self, detail: CharacterDetail) -> QFrame:
        card = self._panel("정식 추가 형태 및 보너스")
        bonus = detail.shape_bonus
        if bonus is None:
            card.layout().addWidget(self._muted("현재 정식 데이터에 추가 형태가 없습니다"))
            return card
        card.layout().addWidget(self._chip(f"{bonus.shape_grid_count}칸 추가 형태"))
    ...
    def _graduation_card(self, detail: CharacterDetail) -> QFrame:
        ...
        card.layout().addWidget(self._info(
            "콘솔 메인 속성", "、".join(core_stats) or "名称暂未提供",
        ))
        card.layout().addWidget(self._info(
            "드라이브 템플릿", "、".join(drive_stats) or "名称暂未提供",
        ))
        card.layout().addWidget(self._info(
            "보드 목표", f"{graduation.drive_area}칸 · 추가 형태 {graduation.extra_shape_count}개",
        ))
        return card

    def _weights_card(self, detail: CharacterDetail) -> QFrame:
        card = self._panel("추천 가중치")
        card.layout().addWidget(self._derived_badge())
        weights = detail.recommended_weights
        if weights is None or not weights.properties:
            card.layout().addWidget(self._muted("현재 프로젝트에 파생 추천 가중치가 없습니다"))
            return card
        for prop, weight, main_weight in weights.properties:
            card.layout().addWidget(self._info(
                self._property_name(prop),
                f"종합 {_number(weight * 100)}% · 메인 스탯 {_number(main_weight * 100)}%",
            ))
        return card

    def _stages_card(self, detail: CharacterDetail) -> QFrame:
        card = self._panel("단계별 육성 루트")
        guide = detail.cultivation
        if guide is None or not guide.stages:
            card.layout().addWidget(self._muted("현재 정식 데이터에 단계별 육성 루트가 없습니다"))
            return card
        ...
        for stage in guide.stages:
            ...
            card.layout().addWidget(self._info(
                f"단계 {stage.ordinal + 1}",
                f"캐릭터 Lv.{stage.character_level} · 아크 Lv.{stage.fork_level} · "
                f"콘솔 Lv.{stage.core_level} · 드라이브 Lv.{stage.equipment_level}\n{skills}",
            ))
        return card
...
class CharacterAwakeningView(QWidget):
    ...
    def _card(
        ...
        description = QLabel(
            _plain(awakening.description_zh) or "현재 정식 데이터에 설명이 없습니다",
            card,
        )
        ...
        for bonus in awakening.skill_level_bonuses:
            ...
            card.layout().addWidget(CharacterBuildView._chip(
                f"스킬 레벨 보너스 · {name} {bonus.level_delta:+d}",
            ))
        return card
...
def _awakening_stage(awakening: AwakeningEffect) -> str:
    if awakening.awaken_type == "Awaken_Resonance":
        return "3각성" if awakening.ordinal == 6 else "6각성"
    names = ("1각성", "2각성", "3각성", "4각성", "5각성", "6각성")
    return names[awakening.ordinal] if 0 <= awakening.ordinal < len(names) else "각성"
