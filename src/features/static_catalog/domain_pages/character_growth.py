...
class CharacterGrowthView(QWidget):
    def __init__(
        ...
        *,
        ...
        layout.setSpacing(5)
        title = QLabel("레벨 계획", calculator)
        title.setStyleSheet(themed_style(
            "color:#f0f6fc;font-size:15px;font-weight:900"
        ))
        hint = QLabel(
            "레벨과 돌파 상태를 선택하면 정식 경험치 책, 돌파 재료, 방스(方斯)를 바로 합산합니다. 활력은 환산하지 않습니다.",
            calculator,
        )
        ...
        controls.addWidget(QLabel("캐릭터 레벨", calculator))
        ...
        self.include_breakthroughs = QCheckBox("중간 돌파 포함", calculator)
        ...
        self.preview_level = self._metric("목표 패널", "—")
        ...
        section_title = QLabel("돌파 마일스톤", self)
    ...
    def _milestone(self, stage: BreakthroughStage) -> QFrame:
        ...
        title = QLabel(f"Lv.{stage.level} · 돌파 {stage.stage}", card)
        title.setStyleSheet(themed_style(
            "color:#58a6ff;font-size:13px;font-weight:900"
        ))
        delta = QLabel(
            f"HP +{_number(stage.after.hp_base - stage.before.hp_base)}   "
            f"공격력 +{_number(stage.after.atk_base - stage.before.atk_base)}   "
            f"방어력 +{_number(stage.after.def_base - stage.before.def_base)}",
            card,
        )
    ...
    def _refresh_preview(self) -> None:
        if not self._points:
            self._set_metric(self.preview_level, "현재 정식 데이터에 없음")
        ...
        if point is None:
            self._set_metric(self.preview_level, f"Lv.{target} · 정식 패널 없음")
            return
        state = "돌파 후" if point.state == "breakthrough_after" else "돌파 전"
    ...
    def _refresh_materials(self) -> None:
        ...
        if detail is None or detail.progression is None:
            self.progression_result.setText(
                "캐릭터 육성의 정식 상위 데이터 출처는 확인되었지만, 아직 현재 배포 정적 라이브러리에 포함되지 않았습니다."
            )
            return
        ...
        lines = [f"레벨업 경험치 · {projection.required_experience:,}"]
        if projection.experience_books:
            book_line = self._format_requirements(projection.experience_books)
            overflow = (
                f"(초과 경험치 {projection.experience_overflow:,})"
                if projection.experience_overflow else "(초과 없음)"
            )
            lines.append(f"경험치 책 · {book_line} {overflow}")
        if self.include_breakthroughs.isChecked():
            lines.append(
                "돌파 재료 · " + (
                    self._format_requirements(projection.breakthrough_materials)
                    if projection.breakthrough_materials else "추가 재료 없음"
                )
            )
        else:
            lines.append("돌파 재료 · 중간 돌파 미포함")
        if projection.additional_costs:
            lines.append("추가 소모 · " + self._format_requirements(
                projection.additional_costs
            ))
        if projection.gaps:
            lines.append("일부 정식 수량이 아직 제공되지 않아 위 내용은 알려진 항목의 합계일 뿐입니다.")
        self.progression_result.setText("\n".join(lines))
