...
class CombatMechanicsCatalogPage(QWidget):
    ...
    def _build_gallery_page(self) -> QWidget:
        ...
        self.gallery_title = QLabel("전투 메커니즘 도감", control)
        self.gallery_title.setStyleSheet(themed_style(
            "color:#f0f6fc;font-size:21px;font-weight:900;"
        ))
        self.gallery_subtitle = QLabel("피해가 어떻게 계산되는지 공식으로 설명합니다", control)
    ...
    def _refresh_gallery(self) -> None:
        ...
        self.result_count.setText(f"{len(self._cards)}개")
        self._card_widgets = []
        _clear_layout(self.gallery_grid)
        if not self._cards:
            empty = QLabel("일치하는 피해 공식이 없습니다.", self.gallery_host)
    ...
    def resizeEvent(self, event: QResizeEvent) -> None:  # noqa: N802 - Qt override
        ...
        self.search.setPlaceholderText("공식 검색" if narrow else "공식 또는 명칭(한글/중국어) 검색")
