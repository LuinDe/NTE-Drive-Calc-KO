...
class ForkProfileView(QWidget):
    ...
    def _build(self) -> None:
        ...
        self.tabs.addTab(detail_scroll, "상세")
        self.tabs.addTab(cultivation_scroll, "육성")
    ...
    def _build_hero(self) -> QFrame:
        ...
        self.name = QLabel("아크 선택", hero)
        self.name.setStyleSheet(themed_style(
            "color:#f0f6fc;font-size:23px;font-weight:900"
        ))
        badges = QHBoxLayout()
        self.quality_badge = self._badge("품질 미제공", "#d29922")
        self.type_badge = self._badge("유형 미제공", "#58a6ff")
        self.release_badge = self._badge("첫 출시 아크", "#8b949e")
        badges.addWidget(self.quality_badge)
        badges.addWidget(self.type_badge)
        badges.addWidget(self.release_badge)
        badges.addStretch(1)
        self.owner_label = QLabel("소속 캐릭터 · 아직 불러오지 않음", hero)
    ...
    def _build_detail_tab(self) -> None:
        controls = self._panel("레벨·믹싱")
        hint = QLabel(
            "1–80레벨을 선택합니다. 20/30/40/50/60/70레벨은 돌파 전과 돌파 후를 모두 유지합니다.",
            controls,
        )
        ...
        level_row.addWidget(QLabel("레벨", controls))
        ...
        refinement_caption = QLabel("믹싱 노드", controls)
        ...
        self.panel_host = self._panel("패널")
        self.panel_grid = QGridLayout()
        self.panel_host.layout().addLayout(self.panel_grid)
        self._detail_layout.addWidget(self.panel_host)
        self.refinement_host = self._panel("아크 스킬")
        self.refinement_title = QLabel("믹싱 1", self.refinement_host)
        ...
        self.effects_host = self._panel("효과 설명")
        ...
        self.characters_host = self._panel("소속 캐릭터·적합 캐릭터")
    ...
    def _build_cultivation_tab(self) -> None:
        current = self._panel("현재 레벨 육성 정보")
        self.current_level_cost = QLabel("아크를 아직 선택하지 않음", current)
        ...
        self.breakthrough_host = self._panel("돌파 경로 · 소모")
        ...
        self.refinement_cost_host = self._panel("믹싱 1–5레벨 소모")
    ...
    def _sync_stage_buttons(self) -> None:
        ...
        for index, row in enumerate(choices):
            ...
            state = "돌파 전" if index == 0 and len(choices) == 2 else "돌파 후"
            if len(choices) == 1:
                state = f"{stage}단계"
            button = self._node_button(f"{state} · 상한 {cap}")
    ...
    def _refresh_selection(self) -> None:
        ...
        stage_text = f"돌파 {self._stage}단계" if self._stage is not None else "돌파 단계 미제공"
        exp_text = f"레벨업 경험치 {growth.need_exp}" if growth is not None else "레벨업 경험치 미제공"
        ...
        self.current_level_cost.setText(
            ...
            "레벨별 레벨업 소모: 현재 정식 데이터가 제공되지 않으며, 0으로 대체 표시하지 않습니다."
        )
        self._refresh_refinement()
        self._refresh_node_states()
    ...
    def _render_panel(self, detail: ForkCatalogDetail) -> None:
        ...
        if not self._panel_values:
            self.panel_grid.addWidget(self._gap("패널 · 현재 정식 데이터 미제공"), 0, 0)
            return
        names: dict[str, str] = {}
        percent: set[str] = set()
    ...
    def _refresh_refinement(self) -> None:
        ...
        self.refinement_title.setText(f"믹싱 {self._refinement}")
        if refinement is None:
            self.refinement_description.setText("아크 스킬 · 현재 정식 데이터 미제공")
        else:
            title = refinement.title_zh or "스킬 제목 미제공"
    ...
    def _render_effects(
        ...
        if presentation is None:
            message = (
                "현재 믹싱에는 추가 효과 설명이 없습니다"
                if refinement is not None else "현재 믹싱 스킬은 아직 제공되지 않습니다"
            )
    ...
    def _render_characters(self) -> None:
        ...
        if owner_names:
            self.owner_label.setText(f"전용 아크 · {'、'.join(owner_names)}")
        elif recommended_names:
            self.owner_label.setText(f"육성 추천 · {'、'.join(recommended_names)}")
        else:
            self.owner_label.setText("범용 아크 · 전용 캐릭터 없음")
        compatible = tuple(
    ...
    def _render_cultivation_routes(self) -> None:
        ...
        for stage in detail.breakthroughs:
            card = self._panel(f"{stage.stage}단계 · 상한 Lv.{stage.max_fork_level}")
            ...
            card.layout().addWidget(self._info_row("돌파 패널", modifier_text))
            self._breakthrough_cards.append(card)
        for refinement in detail.refinement_levels:
            card = self._panel(f"{'★' * refinement.level} · 믹싱 {refinement.level}")
            costs = self._item_names.present_raw(refinement.need_gold_raw)
            card.layout().addWidget(self._info_row(
                "소모",
                self._item_names.player_text(costs) or "아직 제공되지 않음",
            ))
            card.layout().addWidget(self._info_row(
                "스킬",
                refinement.title_zh or "현재 정식 데이터에 없음",
            ))
