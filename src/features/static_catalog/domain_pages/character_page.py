...
class CharacterCatalogPage(QWidget):
    ...
    def _build_gallery(self) -> QWidget:
        ...
        self.filter_summary = QLabel("전체 캐릭터 · 전체 속성 · 전체 품질 · 전체 획득 방식", filters)
        ...
        self.filter_toggle = QPushButton("필터 펼치기  ▾", filters)
        ...
        acquisition_options = [("전체", "all", True, "")]
        ...
        self.empty_label = QLabel("일치하는 캐릭터가 없습니다", self.gallery_host)
    ...
    def _apply_filters(self) -> None:
        ...
        self.result_count.setText(f"캐릭터 {len(visible)}명")
        self.filter_summary.setText(" · ".join((
            self._checked_text(self.availability_group, "전체 캐릭터"),
            self._checked_text(self.element_group, "전체 속성"),
            self._checked_text(self.quality_group, "전체 품질"),
            self._checked_text(self.acquisition_group, "전체 획득 방식"),
        )))
    ...
    def _matches(
        ...
        *,
        ...
        if (
            query
            and not _ko_contains(summary.name_zh, query)
