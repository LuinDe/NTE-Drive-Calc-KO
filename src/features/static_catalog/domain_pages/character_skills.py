...
def _multiplier_lines(skill: CharacterSkill, level: int) -> tuple[tuple[str, str], ...]:
    ...
    for hint in skill.level_hints:
        ...
        label = _plain(hint.description_zh) or f"배율 구간 {len(lines) + 1}"
        if bases != ("攻击",):
            label += " · " + "/".join(bases)
...
class _SkillRowHeader(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.setAccessibleName("스킬 상세 펼치기/접기")
        self.setToolTip("행 전체를 클릭해 펼치거나 접기")
...
class SkillActionCard(QFrame):
    ...
    def __init__(self, action: CharacterActionCard, parent: QWidget | None = None) -> None:
        ...
        if action.skill is not None:
            ...
            self.level.hide()
            status = (
                f"돌파 {action.passive.unlock_stage} 해금"
                if action.passive is not None else "레벨 미제공"
            )
    ...
    def _toggle_text(self, expanded: bool) -> str:
        noun = "설명" if self.action.passive is not None else "배율"
    ...
    def _render_drawer(self) -> None:
        ...
        if skill is None or level is None:
            self.drawer_layout.addWidget(self._muted("현재 정식 데이터에 레벨별 배율이 없습니다"))
            return
        title = QLabel(f"현재 레벨 배율 · Lv.{level}", self.drawer)
        ...
        if not lines:
            self.drawer_layout.addWidget(self._muted("현재 정식 데이터에 배율 상세가 없습니다"))
            return
    ...
    def _render_passive(self, passive: CharacterPassive) -> None:
        title = QLabel(f"돌파 {passive.unlock_stage} 해금", self.drawer)
        title.setStyleSheet(themed_style(
            "color:#58a6ff;font-size:14px;font-weight:900"
        ))
        self.drawer_layout.addWidget(title)
        ...
        for item in descriptions:
            ...
            rendered = True
        if not rendered:
            self.drawer_layout.addWidget(self._muted("현재 정식 데이터에 패시브 설명이 없습니다"))
...
class CharacterSkillTrainingView(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        controls.addWidget(QLabel("스킬", self))
    ...
    def _refresh_costs(self) -> None:
        ...
        if action is None or action.skill is None or not self.start.count():
            self.costs.addWidget(self._muted("현재 정식 데이터에 스킬 육성 기록이 없습니다"))
            self._set_summary("현재 정식 데이터에 스킬 육성 기록이 없습니다.", available=False)
            return
        ...
        for cost_level in range(start, end):
            ...
            text = "、".join(
                f"{projection.display_name} × " + (
                    _number(item.quantity) if not item.hidden_amount else "수량 미제공"
                )
                for item, projection in zip(level.costs, projections)
            ) or "소모 미제공"
            label = QLabel(f"Lv.{cost_level + 1} 승급    {text}", self)
            label.setWordWrap(True)
            label.setStyleSheet(themed_style(
