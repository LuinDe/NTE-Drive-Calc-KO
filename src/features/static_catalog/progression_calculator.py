...
class _Disclosure(QFrame):
    ...
    def __init__(
        ...
        *,
        ...
        root.setSpacing(5)
        toggle = QPushButton("자세한 정보  ▾", self)
        ...
        def toggle_content(expanded: bool) -> None:
            ...
            toggle.setText("자세한 정보  ▴" if expanded else "자세한 정보  ▾")
...
class ProgressionCalculatorDialog(QDialog):
    ...
    def _build(self) -> None:
        ...
        self.title = QLabel("육성 재료와 활력", hero)
        self.title.setObjectName("progressionTitle")
        subtitle = QLabel(
            "헌터 레벨과 보유 재료를 편집하고, 정식 던전 산출량 기준으로 최소 활력을 계산합니다.",
            hero,
        )
        ...
        hunter_label = QLabel("헌터 레벨", level_card)
        ...
        identification_label = QLabel("적용 감별 레벨", level_card)
        ...
        materials_heading = QLabel("재료 목록", scroll_host)
        ...
        self.calculate_button = QPushButton("최소 활력 계산", self)
    ...
    def _refresh_identification(self) -> None:
        ...
        self.identification_level.addItem(
            f"감별 {projection.native_level} (기본)",
            projection.native_level,
        )
        if projection.native_level >= 3:
            self.identification_level.addItem(
                f"감별 {projection.native_level - 1} (한 단계 낮춤)",
    ...
    def _render_materials(self, session: ProgressionCalculatorSession) -> None:
        ...
        if not session.materials:
            empty = QLabel("현재 정식 데이터에 계산 가능한 재료 요구량이 없습니다.", self)
    ...
    def _calculate(self) -> None:
        ...
        if session is None:
            self.validation.setText("먼저 캐릭터 또는 아크 육성 페이지에서 계산기를 여세요.")
            return
        owned = {
        ...
        self.validation.setText("최소 활력을 계산하는 중…")
    ...
    def _calculation_failed(self, token: object) -> None:
        ...
        self._calculation_token = None
        self._set_calculation_busy(False)
        self.validation.setText("육성 계산에 실패했습니다. 잠시 후 다시 시도하세요.")
    ...
    def _set_calculation_busy(self, busy: bool) -> None:
        ...
        self.calculate_button.setText("계산 중…" if busy else "최소 활력 계산")

    def _render_result(self, result: ProgressionStaminaResult) -> None:
        self._clear_layout(self.result_layout)
        labels = {
            StaminaPlanStatus.COMPLETE: "정확한 결과",
            StaminaPlanStatus.PARTIAL: "일부 계산 가능",
            StaminaPlanStatus.UNAVAILABLE: "현재 계산 불가",
        }
        ...
        if result.total_stamina is not None:
            total = QLabel(f"최소 활력  {result.total_stamina}", self.result_host)
        elif result.known_stamina > 0:
            total = QLabel(
                f"확인된 일부 활력  {result.known_stamina} · 전체 총량 알 수 없음",
                self.result_host,
            )
        else:
            total = QLabel("전체 활력을 현재 알 수 없음", self.result_host)
        total.setObjectName("progressionResultTotal")
        self.result_layout.addWidget(total)
        for run in result.runs:
            row = QLabel(
                f"{run.label}  × {run.runs}회  ·  활력 {run.total_stamina}",
                self.result_host,
            )
