...
class StaticCatalogMonsterProvider:
    ...
    domain = CatalogDomain(
        key="monsters",
        label="몬스터 및 콘텐츠",
        description="도감, 템플릿 프로필, 쟁봉(争锋), 궤외(轨外), 던전, 이상 현상 사냥과 고위험 의뢰",
        order=30,
    )
    ...
    def close(self) -> None:
        if not self._closed:
            ...
            if errors:
                raise RuntimeError(
                    f"몬스터 자료 제공자를 닫는 중 구성 요소 {len(errors)}개가 실패했습니다"
                ) from errors[0]
    ...
    def _validate_release(self, release: StaticCatalogRelease) -> None:
        if self._closed:
            raise RuntimeError("몬스터 자료 어댑터가 닫혔습니다")
