...
class StaticCatalogMiscProvider:
    ...
    def __init__(
        ...
        *,
        ...
        if domain_key not in _DOMAIN_ORDER:
            raise ValueError(f"지원하지 않는 기타 자료 도메인: {domain_key!r}")
    ...
    def relations(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("이 자료는 해당 페이징 관계를 지원하지 않습니다")
    ...
    def _validate_release(self, release: StaticCatalogRelease) -> None:
        if self._closed:
            raise RuntimeError("기타 자료 어댑터가 닫혔습니다")
        ...
        if release.source_payloads_omitted != metadata.source_payloads_omitted:
            raise RuntimeError("자료실 요청 중에 출처 payload 배포 정책이 변경되었습니다")
    ...
    def _detail(self, detail: MiscDetail) -> CatalogDetail:
        ...
        if not sections:
            sections.append(
                CatalogSection("관련 자료", fields=(), references=references)
            )
    ...
    def _relation_reference(
        ...
        if relation_kind == "references":
            if (
                values.get("target_available") != "예"
                or values.get("catalog_detail_available") != "예"
            ):
                return None
            ...
            if values.get("target_available") != "예":
                return None
            target_kind, target_key, label = (
                "gameplay_effect", values.get("effect_id"), "GE 보기"
            )
        elif relation_kind == "ability_montages":
            if values.get("target_available") != "예":
                return None
            target_kind, target_key, label = (
                "montage", values.get("montage_asset_path"), "Montage 보기"
            )
        else:
            return None
    ...
    def _reference(relation: object) -> CatalogReference | None:
        ...
        if not target_domain or not target_key:
            if target_kind == "fork" and target_key:
                return CatalogReference(
                    label=str(getattr(relation, "label", "아크 보기")),
                    domain_key="fork",
                    record_id=target_key,
                )
            return None
        return CatalogReference(
            label=str(getattr(relation, "label", "연관 자료 보기")),
            domain_key=target_domain,
            record_id=encode_typed_record_id(target_kind, target_key),
        )
    ...
    def _source_detail(
        ...
        fields = (
            ...
            CatalogField(
                "원본 payload",
                "보존됨" if trace.payload_present else "배포 패키지에서 제공하지 않음",
        ...
        return CatalogDetail(
            ...
            sections=(CatalogSection("출처 근거", fields),),
