...
MENU_GROUPS = (
    ("collection", "캐릭터와 육성", "조작 가능한 대상에서 시작해 성장, 스킬, 연관 메커니즘을 단계별로 살펴봅니다."),
    ("combat", "장비와 적", "게임 도감 기준으로 콘솔, 드라이브, 몬스터, 플레이 환경을 정리합니다."),
    ("mechanics", "전투 메커니즘", "공통 전투 규칙과 피해 공식을 확인합니다."),
)


MENU_ENTRIES = (
    CatalogMenuEntry(
        "character", "collection", "캐릭터 도감", "CHARACTER ARCHIVE",
        "일러스트, 아바타, 식별 정보, 육성, AEQR 및 캐릭터 전용 동작.", "#58a6ff", "캐",
        "character", ("1036",),
    ),
    CatalogMenuEntry(
        "fork", "collection", "아크 도감", "FORK ARCHIVE",
        "레벨 돌파, 패널, 믹싱 스킬, Buff 및 적합 캐릭터.", "#a371f7", "아",
        "fork", ("fork_DemonBlade",),
    ),
    CatalogMenuEntry(
        "equipment", "combat", "콘솔 및 드라이브", "EQUIPMENT CODEX",
        "세트, 형태, 메인·서브 스탯, 강화 곡선, 졸업 조합.", "#f0883e", "장",
        "equipment", ("Attack_orange",),
    ),
    CatalogMenuEntry(
        "monsters", "combat", "몬스터 및 콘텐츠", "ENCOUNTER ARCHIVE",
        "오픈 월드, 쟁봉, 궤외, 던전, Boss 및 적 속성.", "#f85149", "적",
...
class CatalogMenuCard(QFrame):
    ...
    def _build(self, art: QPixmap | None) -> None:
        ...
        state = QLabel("들어가기  ›" if self._available else "현재 배포에서 사용할 수 없음", self)
...
class StaticCatalogMenu(QWidget):
    ...
    def _hero(self) -> QWidget:
        ...
        title = QLabel("게임 자료실", hero)
        title.setStyleSheet(themed_style("color:#f0f6fc;font-size:21px;font-weight:900"))
        subtitle = QLabel("자료 영역을 선택하세요. 각 항목은 해당 영역과 관련된 정식 데이터와 관계만 표시합니다.", hero)
