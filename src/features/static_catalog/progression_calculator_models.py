...
UNKNOWN_NAME = "名称暂未提供"
CALLBACK_ERROR_TEXT = "결과는 계산되었지만 원래 페이지로 돌아가지 못했습니다. 닫은 뒤 다시 여세요."
...
class ProgressionMaterialInput:
    ...
    def requirement_text(self) -> str:
        if self.required_quantity is not None:
            return f"필요 {self.required_quantity}"
        if self.known_quantity > 0:
            return f"전체 필요량 알 수 없음 · 최소 {self.known_quantity} 확인됨"
        return "필요량 알 수 없음"
    ...
    def more_info(self) -> tuple[tuple[str, str], ...]:
        ...
        if requested:
            rows.append(("요청 재료 ID", requested))
        if self.canonical_id and (
            len(self.requested_ids) > 1
            or self.canonical_id not in self.requested_ids
        ):
            rows.append(("정규 재료 ID", self.canonical_id))
        if self.text_table and self.text_key:
            rows.append(("현지화 인덱스", f"{self.text_table} / {self.text_key}"))
        if self.resolved_locale:
            rows.append(("현지화 언어", self.resolved_locale))
        return tuple(rows)
...
class ProgressionCalculatorSession:
    ...
    def more_info(self) -> tuple[tuple[str, str], ...]:
        ...
        for index, gap in enumerate(self.upstream_gaps, start=1):
            ...
            rows.append((f"업스트림 미완료 항목 {index}", " · ".join(values)))
        return tuple(rows)
...
class ProgressionCalculatorOrchestrator:
    ...
    def prepare(
        ...
        raise TypeError("공용 육성 계산기는 캐릭터 딕셔너리 또는 ForkProgressionRequest만 받습니다")
    ...
    def _prepare_character(
        ...
        if kind not in {"character_level", "skill"}:
            raise ValueError(f"지원하지 않는 캐릭터 육성 요청: {kind or '<empty>'}")
        character_id = str(request.get("character_id") or "").strip()
        if not character_id:
            raise ValueError("캐릭터 육성 요청에 character_id가 없습니다")
        entity_id = character_id
        if kind == "skill":
            skill_id = str(request.get("skill_id") or "").strip()
            if not skill_id:
                raise ValueError("스킬 육성 요청에 skill_id가 없습니다")
        ...
        return ProgressionCalculatorSession(
            ...
            title="스킬 육성" if kind == "skill" else "캐릭터 레벨 육성",
            materials=materials,
    ...
    def _materials(
        ...
        *,
        ...
        for raw in requirements:
            ...
            if not item_id:
                raise ValueError("재료 요구 항목에 안정적인 item_id가 없습니다")
...
def _status(value: object, *, default: StaminaPlanStatus | None) -> StaminaPlanStatus:
    ...
    try:
        ...
        raise ValueError(f"잘못된 재료 요구 상태: {value!r}") from exc
...
def _tuple_value(value: object, label: str) -> tuple[object, ...]:
    ...
    if isinstance(value, (str, bytes, Mapping)):
        raise TypeError(f"{label}은(는) DTO 시퀀스여야 합니다")
    try:
        return tuple(value)  # type: ignore[arg-type] # validated generic DTO iterable
    except TypeError as exc:
        raise TypeError(f"{label}은(는) DTO 시퀀스여야 합니다") from exc
...
def _gaps(value: object) -> tuple[ProgressionUpstreamGap, ...]:
    ...
    for row in rows:
        ...
        if not code:
            raise ValueError("업스트림 미완료 항목에 안정적인 원인 코드가 없습니다")
...
def _nonnegative_int(value: object, label: str) -> int:
    if isinstance(value, bool):
        raise ValueError(f"{label}은(는) 음수가 아닌 정수여야 합니다")
    try:
        parsed = int(str(value).strip())
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{label}은(는) 음수가 아닌 정수여야 합니다") from exc
