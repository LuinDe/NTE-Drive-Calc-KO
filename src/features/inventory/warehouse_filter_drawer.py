...
class WarehouseFilterDrawer(QFrame):
    ...
    def __init__(self, parent: QWidget) -> None:
        ...
        close_button.setToolTip("닫고 이번 수정을 적용하지 않음")
        ...
        self.card_tab = QPushButton("카트리지")
        self.drive_tab = QPushButton("드라이브 모듈")
    ...
    def _rebuild(self) -> None:
        ...
        if self._page_kind == "core":
            ...
            for size, title in ((2, "II형 드라이브"), (3, "III형 드라이브"), (4, "IV형 드라이브")):
                options = tuple(
                    option
        ...
        self._add_section("품질", self._catalog.qualities, "qualities", self._draft.qualities)
        if self._page_kind == "core":
            self._add_section(
                "메인 속성",
        ...
        self.summary.setText(f"조건 {self._draft.active_group_count}개")
