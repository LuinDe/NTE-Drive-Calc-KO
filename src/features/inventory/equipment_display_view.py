...
def _page_equipment(self):
    ...
    sh = QHBoxLayout()
    sh.addWidget(QLabel("검색"))
    self.equip_search = QLineEdit()
    self.equip_search.setPlaceholderText("캐릭터 이름 검색 (한글·병음 지원)...")
    ...
    self.equip_import_all_btn = QPushButton("원클릭 가져오기")
    self.equip_import_all_btn.setObjectName("btnPrimary")
    self.equip_import_all_btn.setToolTip("완전하고 잠기지 않은 게임 내 방안을 모두 가져오기")
...
def _queue_equipment_render(self, eq):
    ...
    empty_text = (
        ...
        else "저장된 장비 세팅이 없습니다. 먼저 분배를 실행하고 저장하세요."
    )
    if isinstance(self, QWidget):
        show_equipment_master_detail(
            ...
            roles,
            empty_message=(
                empty_text or "현재 게임 스냅샷에 표시할 캐릭터 장비가 없습니다."
            ),
        )
        return

    if not roles:
        ph = QLabel(empty_text or "현재 게임 스냅샷에 표시할 캐릭터 장비가 없습니다.")
...
def _on_sqlite_equipment_display_error(self, token, error):
    if token is not getattr(self, "_equip_load_token", None):
        return
    logger.error(f"SQLite 장비 세팅 표시 새로 고침 실패: {error}")
...
def _on_game_equipment_display_loaded(self, token, result):
    ...
    if projection is None:
        _on_sqlite_equipment_display_error(self, token, "게임 내 방안 투영 결과가 잘못되었습니다")
        return
    if projection.supported:
        ...
        self.equip_import_all_btn.setToolTip(
            f"완전하고 잠기지 않은 게임 내 방안 {ready_count}개를 모두 가져오기"
        )
        self._game_loadout_message = (
            "현재 게임 스냅샷에 장착된 드라이브나 카트리지가 없습니다."
    ...
    try:
        ...
        logger.warning(f"게임 장비 세팅 점수 투영 실패: {exc}")
...
def _refresh_equip(self, *, restore_role_name=None):
    ...
    if getattr(self, "_equipment_mode", "saved") == "game":
        try:
            ...
            logger.error(f"게임 내 장비 세팅 표시 새로 고침 실패: {exc}")
    ...
    try:
        ...
        logger.error(f"SQLite 장비 세팅 표시 새로 고침 실패: {exc}")
        eq = {}
