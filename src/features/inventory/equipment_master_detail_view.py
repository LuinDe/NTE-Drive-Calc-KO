...
def _role_badges(state: dict[str, Any]) -> list[tuple[str, str, str, str]]:
    ...
    if state.get("_game_mode"):
        if state.get("_game_existing_plan_locked"):
            badges.append(("", "대응하는 계산 세팅이 잠겨 있음", "#30363d", "lock"))
        if state.get("_game_imported"):
            badges.append(("✓", "현재 게임 세팅을 가져옴", "#238636", "imported"))
        elif not state.get("_game_importable"):
            badges.append(("!", str(state.get("_game_reason") or "게임 세팅이 불완전함"), "#9e6a03", "incomplete"))
        else:
            tip = str(state.get("_game_reason") or "계산 세팅으로 가져올 수 있음")
            if state.get("_game_existing_plan_locked"):
                tip += "(먼저 계산 세팅 잠금을 해제해야 함)"
            badges.append(("↓", tip, "#1f6feb", "importable"))
        return badges
    if state.get("_allocation_locked"):
        badges.append(("", "계산 세팅이 잠겨 있음", "#30363d", "lock"))
    if (state.get(ROLE_LAST_DIFF) or {}).get(DIFF_CHANGED):
        badges.append(("Δ", "이전 계산 세팅과 변동이 있음", "#1f6feb", "changed"))
    return badges
...
def _apply_role_button_status(
    ...
    status_text = "、".join(badge[1] for badge in badges) or "특별한 상태 없음"
    ...
    *,
    ...
    for role_name, state in roles:
        ...
        if slot_id is not None:
            ...
            manage_button.setToolTip("이 캐릭터의 장비 세팅 슬롯 관리")
