...
def _set_allocation_lock_button_state(button: QPushButton, locked: bool) -> None:
    ...
    button.setAccessibleName("세팅 잠금 해제" if locked else "세팅 잠금")
    button.setToolTip(
        "현재 방안이 잠겨 있음: 이 장비는 다른 캐릭터의 계산이나 교체 후보에 포함되지 않습니다"
        if locked
        else "현재 방안이 잠기지 않음: 클릭하면 이 방안과 장비를 유지합니다"
    )
...
def _render_equip_role(self, role_name, rd, *, target_layout=None):
    ...
    if rd.get("_empty_slot"):
        ...
        layout.addWidget(QLabel("이 장비 세팅 슬롯은 비어 있습니다. 계산 저장 시 선택하면 방안이 기록됩니다."))
    ...
    total_layout.addWidget(QLabel("총평"))
    ...
    if is_game_mode:
        import_btn = QPushButton("가져옴" if rd.get("_game_imported") else "가져오기")
        ...
        if rd.get("_game_existing_plan_locked"):
            import_btn.setToolTip("기존 계산기 방안이 잠겨 있습니다. 먼저 잠금을 해제하세요")
        elif not rd.get("_game_importable"):
            import_btn.setToolTip(str(rd.get("_game_reason") or "현재 방안이 불완전함"))
    ...
    if bp:
        ...
        if is_game_mode:
            ...
            comparison_selector.setToolTip("속성 요약에 사용할 계산 세팅 슬롯 선택")
            ...
            if comparison_slots:
                ...
                comparison_selector.addItem("비교 방안 없음")
    ...
    if tape_data:
        ...
        gl.addWidget(presentation.section_label("카트리지:"))
    ...
    if drives:
        gl.addWidget(presentation.section_label(f"드라이브 ({len(drives)}개):"))
        for d in drives:
            d_q = d.get(EQUIP_QUALITY, "Gold")
