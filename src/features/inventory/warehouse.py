...
def _shape_label(geometry: Any) -> str:
    ...
    return _SHAPE_LABELS.get(value, str(geometry or "알 수 없는 형태"))
...
def _drive_type_label(shape: str) -> str:
    ...
    return {"2": "II형 드라이브", "3": "III형 드라이브", "4": "IV형 드라이브"}.get(number, "드라이브")
...
def _stat_view(stat: Mapping[str, Any], *, main: bool = False) -> dict[str, Any]:
    ...
    label = _localized(stat.get("names"), _STAT_LABELS.get(property_id, property_id or "알 수 없는 속성"))
...
def warehouse_item_view(
    ...
    *,
    ...
    item_name = _localized(row.get("names"), str(row.get("item_id") or "알 수 없는 장비"))
    suit_name = _display_suit_name(_localized(row.get("suit_names"), str(row.get("suit_id") or "미식별 세트")))
    ...
    kind_label = "드라이브" if kind == "module" else "카트리지" if kind == "core" else kind or "未知"
    suit_id = str(row.get("suit_id") or "")
    shape_id = str(row.get("geometry") or "")
    shape = _shape_label(shape_id) if kind == "module" else "카트리지"
    title = _drive_type_label(shape) if kind == "module" else "카트리지"
...
def _state_tags(row: Mapping[str, Any]) -> list[tuple[str, str]]:
    ...
    if not row.get("state_known", True):
        return [("상태 알 수 없음", "#8b949e")]
    if row.get("discarded"):
        tags.append(("폐기됨", "#f85149"))
    if row.get("locked"):
        tags.append(("잠김", "#d29922"))
    if row.get("equipped"):
        tags.append(("장착됨", "#58a6ff"))
    return tags
...
def warehouse_item_with_state(item: Mapping[str, Any], target_state: str) -> dict[str, Any]:
    ...
    if target_state not in {"normal", "locked", "discarded"}:
        raise ValueError(f"알 수 없는 창고 상태: {target_state}")
    if not item.get("state_known", True):
        raise ValueError("현재 인벤토리 출처에서는 잠금·폐기 상태를 읽거나 수정할 수 없습니다")
    updated = dict(item)
...
def warehouse_type_options(items: Iterable[Mapping[str, Any]], category: str = "all") -> list[tuple[str, str]]:
    ...
    for item in items:
        ...
        label = str(item.get("title") if kind == "module" else item.get("suit_name") or "알 수 없는 유형")
        options[key] = label
...
class WarehouseCardDelegate(QStyledItemDelegate):
    ...
    def _stat_row(painter: QPainter, rect: QRect, stat: Mapping[str, Any], *, main: bool = False) -> None:
        ...
        comparison_background = str(stat.get("comparison_background") or "")
        if comparison_background:
            background = QColor(comparison_background)
    ...
    def paint(self, painter: QPainter, option, index) -> None:
        ...
        if item.get("equipped"):
            ...
            if not avatar.isNull():
                ...
                self._text(painter, avatar_rect, "장착됨", theme_color("#58a6ff"), 8, bold=True)
        content_top = top + 70
        stats = [*(item.get("main_stats") or []), *(item.get("sub_stats") or [])]
        if not stats:
            self._text(painter, QRect(left, content_top + 7, width, 18), "스탯 데이터 없음", theme_color("#6e7681"), 10)
        else:
