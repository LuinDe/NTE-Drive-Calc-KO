...
def _sqlite_plan_display_state(
    ...
    *,
    ...
    for assignment in plan["assignments"]:
        ...
        if item is None:
            message = (
                f"방안 #{plan.get('plan_id')}의 장비 UID가"
                f"({assignment.get('uid_slot')}, {assignment.get('uid_serial')}) "
                f"출처 스냅샷 {snapshot_id}에 없습니다"
            )
            logger.error("저장된 방안 호환성 오류: {}", message)
        ...
        if unknown_properties:
            message = (
                f"방안 #{plan.get('plan_id')}의 출처 스냅샷에 현재 정적 데이터에 정의되지 않은 속성 ID가 있습니다:"
        ...
        if item["kind"] == "core":
            ...
            if suit_id not in suit_names:
                message = f"방안 #{plan.get('plan_id')}의 코어 세트 {suit_id or '<empty>'}이(가) 현재 정적 데이터에 없습니다"
                logger.error("저장된 방안 호환성 오류: {}", message)
                raise RuntimeError(message)
        ...
        if official_shape not in shape_cells:
            message = f"방안 #{plan.get('plan_id')}의 드라이브 형태 {geometry or '<empty>'}이(가) 현재 정적 데이터에 없습니다"
            logger.error("저장된 방안 호환성 오류: {}", message)
            raise RuntimeError(message)
        drives.append(
            {
