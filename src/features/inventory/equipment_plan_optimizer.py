...
def _replacement_assignments(plan, old_uid, replacement):
    ...
    if not replaced:
        raise ValueError("현재 장비가 변경되었습니다. 장비 세팅 페이지를 새로 고친 뒤 다시 시도하세요.")
    return assignments
...
def _sqlite_replacement_candidates(
    ...
    *,
    ...
    with UserDataDao(database_path) as user_dao, StaticGameDataDao() as static_dao:
        ...
        if plan is None:
            raise ValueError("해당 캐릭터의 저장된 방안을 찾지 못했습니다")
        if plan.get("allocation_locked"):
            raise ValueError("잠긴 방안은 교체 최적화를 할 수 없습니다. 먼저 잠금을 해제하세요")
        ...
        if current is None:
            raise ValueError("현재 장비가 이 방안에 바인딩된 가방 스냅샷에 없습니다")
...
def _open_official_saved_plan_optimizer(
    ...
    *,
    ...
    if not isinstance(plan, dict):
        return False
    if plan.get("allocation_locked"):
        QMessageBox.information(window, "교체 최적화", "현재 방안이 잠겨 있습니다. 먼저 장비 세팅 페이지에서 잠금을 해제하세요.")
        return True
...
def _optimize_saved_equipment(
    ...
    *,
    ...
    if (
        ...
        if not is_custom_role:
            try:
                ...
                QMessageBox.warning(self, "교체 최적화", f"공식 캐릭터 상세를 읽을 수 없습니다: {exc}")
                return
            QMessageBox.warning(self, "교체 최적화", "현재 방안을 공식 캐릭터 상세에서 찾을 수 없습니다. 다시 계산하고 저장한 뒤 다시 시도하세요.")
            return
    ...
    try:
        ...
        QMessageBox.warning(self, "교체 최적화", str(exc))
        return
    ...
    if item_kind == "drive":
        ...
        title = f"교체 최적화 - {current.get(EQUIP_SHAPE_ID) or '驱动'}"
    else:
        score = lambda item: float(
        ...
        title = f"{core_term} 교체 - {current.get(EQUIP_SET_NAME) or core_term}"
    ...
    if not ranked:
        QMessageBox.information(self, "교체 최적화", "현재 스냅샷에 교체할 수 있는 같은 종류의 장비가 없습니다.")
        return
    ...
    role_header = QLabel(f"장착 캐릭터: {role_name}")
    ...
    summary_text = (
        "사용자 정의 캐릭터는 현재 스탯과 카트리지 메인 속성 가중치 점수로만 정렬됩니다"
        if is_custom_role
        else (
        f"현재 직접 피해 이득: {current_margin:+.2f}% (후보는 직접 피해 이득순 정렬)"
        if rank_by_damage and current_margin is not None
        else "후보는 현재 스탯 세팅 가중치 점수순으로 정렬됩니다"
        )
    )
    summary = QLabel(
        f"{summary_text}; 같은 {('形状' if item_kind == 'drive' else '套装')}의 후보 장비만 표시하며,"
        "이 방안의 다른 선택된 장비는 사용하지 않습니다."
    )
    summary.setWordWrap(True)
    summary.setStyleSheet(themed_style("color:#8b949e"))
    layout.addWidget(summary)
    current_group = QGroupBox("현재 드라이브" if item_kind == "drive" else f"현재 {core_term}")
    ...
    candidates_group = QGroupBox(f"교체 가능 {'驱动' if item_kind == 'drive' else core_term} ({len(ranked)}개)")
    ...
    for candidate_margin, candidate_score, candidate in ranked:
        ...
        def apply_replacement(_checked=False, selected=candidate, selected_score=candidate_score):
            try:
                ...
                if callable(replacement_persister):
                    ...
                    with UserDataDao(database_path) as dao:
                        ...
                        save_kwargs = {
                            "name": str(plan.get("name") or f"최적화 방안: {role_name}"),
                ...
                QMessageBox.warning(dialog, "교체 실패", str(exc))
                return
            dialog.accept()
            ...
            QMessageBox.information(self, "교체 최적화", "새 장비 세팅 방안으로 저장했습니다.")
        ...
        if rank_by_damage:
            margin = QLabel(f"직접 피해 이득: {candidate_margin:+.2f}%")
        ...
        if used_by:
            user_label = QLabel(f"사용자: {', '.join(used_by)}")
