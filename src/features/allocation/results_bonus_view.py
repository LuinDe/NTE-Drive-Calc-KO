...
def _make_bonus_mode_switch(self, default_mode, on_change, mode_defs=None):
    ...
    mode_defs = list(mode_defs or (("equipment", "콘솔 속성 요약"), ("character", "캐릭터 속성 요약")))
...
def _bonus_comparison_widget(
    ...
    old_title = old_title or ("이전" if compact else "이전 방안")
    new_title = new_title or ("새" if compact else "새 방안")
    old_empty = "저장된 장비 세팅 없음" if not has_old else ("속성 변화 없음" if compact else "요약할 속성이 없습니다")
    ...
    new_column = self._bonus_comparison_column(
        ...
        aligned,
        "new",
        "속성 변화 없음" if compact and not aligned else "요약할 속성이 없습니다",
        priority_stats=priority_stats,
        role_name=role_name,
        mode=mode,
        colored_stats=colored_stats,
...
def _show_bonus_comparison_dialog(
    ...
    dlg.setWindowTitle(f"{role_name} {self._bonus_summary_mode_label(mode)} 비교")
