...
def _plan_diff_text(self, role_name, diff):
    ...
    if not removed and not added:
        return "이번 세팅은 저장된 방안과 장비 변동이 없습니다."
...
def _diff_snapshot_from_source(self, role_name, source):
    ...
    if item_type == "tape":
        snapshot[EQUIP_SET_NAME] = _diff_value(source, EQUIP_SET_NAME, "") or "카트리지"
...
def _diff_item_card(self, role_name, item, is_new=False):
    ...
    item_type = item.get(EQUIP_TYPE, "drive")
    if item_type == "tape":
        label = item.get(EQUIP_SET_NAME) or "카트리지"
        main_stat = item.get(EQUIP_MAIN_STATS, "")
        shape_id = None
    else:
...
def _append_equipment_swap_frame(body_layout, role_name, old_item, new_item, diff_item_card):
    ...
    if old_item is not None:
        ...
        pair_layout.addWidget(QLabel("  (해제 불필요)"))
    ...
    if new_item is not None:
        ...
        pair_layout.addWidget(QLabel("  (장착 불필요)"))
...
def _build_plan_diff_dialog(self, role_name, diff):
    ...
    if not removed and not added:
        ...
        if removed_tape or added_tape:
            pair_index += 1
            body_layout.addWidget(section_label(f"변동 {pair_index}: 카트리지"))
        ...
        for old_d, new_d in drive_pairs:
            ...
            title = (
                f"변동 {pair_index}: {old_sid} → {new_sid}" if old_sid != new_sid else f"변동 {pair_index}: {old_sid}"
            )
        ...
        for old_d in unmatched_old:
            ...
            body_layout.addWidget(
                section_label(
                    f"변동 {pair_index}: {old_d.get(EQUIP_SHAPE_ID) or '驱动'}"
                )
            )
            _append_equipment_swap_frame(
                body_layout,
        ...
        for new_d in unmatched_new:
            ...
            body_layout.addWidget(
                section_label(
                    f"변동 {pair_index}: {new_d.get(EQUIP_SHAPE_ID) or '驱动'}"
                )
            )
            _append_equipment_swap_frame(
                body_layout,
...
def _persist_saved_equipment_sync(self):
    ...
    if callable(refresh):
        try:
            ...
            logger.debug(f"장비 세팅 페이지 새로 고침 시 선택적 UI 상태가 없어 무시했습니다: {exc}")
