...
class RoleSelectorPreferencesMixin:
    ...
    def _manage_role_preferences(self, name):
        ...
        weapon_row.addWidget(QLabel("아크:"))
        ...
        template_box = QGroupBox("직접 선택 설정")
        ...
        help_btn.clicked.connect(lambda: self._show_help("스탯 직접 선택 설명", STAT_PRIORITY_HELP))

        stat_option_row = QHBoxLayout()
        stat_option_row.setContentsMargins(0, 4, 0, 0)
        stat_option_row.setSpacing(16)
        stat_equal = QCheckBox("서브 스탯 우선순위 동일")
        ...
        crit_threshold_label = QLabel("치명타 확률 최소값")
        ...
        crit_threshold_help.clicked.connect(
            lambda: self._show_help("치명타 확률 최소값", CRIT_THRESHOLD_HELP)
        )
        ...
        effect_help_btn.clicked.connect(lambda: self._show_help("세트 효과 설명", SET_EFFECT_HELP))
        ...
        crit_row.addWidget(QLabel("치명타 확률 상한:"))
        ...
        crit_cap_help.clicked.connect(
            lambda: self._show_help("치명타 확률 상한", CRIT_RATE_CAP_HELP)
        )
    ...
    def save_priority_config(self, show_message: bool = True):
        ...
        if show_message:
            QMessageBox.information(self, "저장 완료", "현재 캐릭터 우선순위 방안을 저장했습니다. 언제든 불러올 수 있습니다.")
    ...
    def _load_priority_config_from(self, path: Path):
        ...
        try:
            ...
            QMessageBox.warning(self, "우선순위 복원", f"우선순위 설정 불러오기 실패: {exc}")
