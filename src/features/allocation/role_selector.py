...
class RoleSelector(RoleSelectorPreferencesMixin, QWidget):
    ...
    def _build(self):
        ...
        self.search.setPlaceholderText("캐릭터 검색 (한글·병음 지원)...")
        self.search.setClearButtonEnabled(True)
        self.search.textChanged.connect(self._filter)
        search_row.addWidget(self.search, 1)

        primary_reset_btn = QPushButton("초기화")
        ...
        primary_restore_btn = QPushButton("복원")
        ...
        primary_save_btn = QPushButton("저장")
        ...
        primary_load_btn = QPushButton("불러오기")
        ...
        help_btn.clicked.connect(lambda: self._show_help("우선순위 저장 설명", PRIORITY_SAVE_HELP))
    ...
    def _render_priority_row(self):
        ...
        for visible_pos, index in enumerate(visible_indexes):
            ...
            name_btn.setToolTip("클릭하면 현재 우선순위에서 제외, 뒤로 드래그하면 대상의 같은 등급 그룹에 맞춰 우선순위 조정")
            ...
            if index < len(self.selected) - 1 and (not query or index + 1 in visible_indexes):
                ...
                link_btn.setToolTip(">: 엄격 우선, >>: 배치 경계, =: 같은 배치 동급. 클릭하면 순환 전환됩니다.")
