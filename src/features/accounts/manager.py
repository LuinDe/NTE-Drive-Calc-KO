...
class AccountManager:
    ...
    def read_index(self) -> dict:
        if self.accounts_index_file.exists():
            try:
                ...
                logger.warning(f"계정 인덱스 읽기 실패, 기본 계정 설정을 사용합니다: {exc}")
        return {"active_account_id": "default", "accounts": [{"id": "default", "name": "기본 계정"}]}
    ...
    def account_meta(self, account_id: str | None = None) -> dict:
        ...
        for account in data.get("accounts", []):
            if account.get("id") == target:
                return account
        return data.get("accounts", [{"id": "default", "name": "기본 계정"}])[0]
    ...
    def initialize(self) -> AccountState:
        ...
        if not normalized:
            normalized = [{"id": "default", "name": "기본 계정"}]
...
def import_account_data(
    ...
    with zipfile.ZipFile(source_zip, "r") as zf:
        try:
            ...
            raise ValueError("계정 가져오기 파일에 유효한 manifest.json이 없습니다") from error
        if not isinstance(manifest, dict) or manifest.get("format") != "nte-account-export":
            raise ValueError("계정 가져오기 파일 형식이 지원되지 않습니다")
        if manifest.get("version") != TRANSFER_FORMAT_VERSION:
            raise ValueError("계정 가져오기 파일 버전이 지원되지 않습니다")
        account = manifest.get("account")
        if not isinstance(account, dict):
            raise ValueError("계정 가져오기 파일에 계정 정보가 없습니다")
        ...
        for member in zf.infolist():
            ...
            if name in member_names:
                raise ValueError(f"계정 가져오기 파일에 중복 항목이 있습니다: {name}")
        ...
        if f"account/{USER_DATABASE_FILENAME}" not in member_names:
            raise ValueError("계정 가져오기 파일에 사용자 데이터가 없습니다")
...
def show_account_manager_dialog(parent, style_sheet: str, manager: AccountManager, active_account_id: str, switch_account_callback, refresh_account_combo_callback) -> None:
    ...
    row = QHBoxLayout()
    row.addWidget(QLabel("계정"))
    combo = QComboBox()
    row.addWidget(combo, 1)
    layout.addLayout(row)

    name_edit = QLineEdit()
    name_edit.setPlaceholderText("계정 이름")
    ...
    def delete_account():
        ...
        if len(data.get("accounts", [])) <= 1:
            QMessageBox.information(dialog, "계정 삭제", "계정은 최소 하나 남겨야 합니다.")
            return
        account = manager.account_meta(account_id)
        ret = QMessageBox.question(
            dialog,
            "계정 삭제",
            f"계정 「{account.get('name', account_id)}」과(와) 해당 인벤토리·장비 세팅·스크린샷 등 데이터를 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.",
    ...
    def export_account():
        ...
        path, _ = QFileDialog.getSaveFileName(dialog, "계정 데이터 내보내기", default_name, "NTE Account Export (*.zip)")
        if not path:
            return
        try:
            export_account_data(manager, account_id, Path(path))
        except Exception as exc:
            QMessageBox.critical(dialog, "계정 데이터 내보내기", f"내보내기 실패: {exc}")
            return
        QMessageBox.information(dialog, "계정 데이터 내보내기", "현재 계정 데이터를 내보냈습니다.")

    def import_account():
        path, _ = QFileDialog.getOpenFileName(dialog, "계정 데이터 가져오기", "", "NTE Account Export (*.zip)")
        if not path:
            return
        ...
        try:
            ...
            QMessageBox.critical(dialog, "계정 데이터 가져오기", f"가져오기 실패: {exc}")
            return
        ...
        refresh(imported_id)
        refresh_account_combo_callback()
        QMessageBox.information(dialog, "계정 데이터 가져오기", "계정 데이터를 가져왔습니다.")
