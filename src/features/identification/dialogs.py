...
def choose_identify_image_options(self, path: Path, *, parent=None):
    ...
    drive_rb = QRadioButton("드라이브")
    tape_rb = QRadioButton("카트리지")
    drive_rb.setChecked(True)
    type_group.addButton(drive_rb, 0)
    type_group.addButton(tape_rb, 1)
    type_row.addWidget(drive_rb)
    type_row.addWidget(tape_rb)
    type_row.addStretch()
    ...
    drive_layout.addWidget(QLabel("드라이브 형태"))
    ...
    def set_selected_shape(shape_id):
        ...
        for btn, sid in shape_buttons:
            if sid == shape_id:
                btn.setStyleSheet(
                    themed_style(
                        "QPushButton{border:2px solid #2f81f7;background:#10243f;color:#f0f6fc;border-radius:6px;padding:4px}"
                    )
                )
            else:
                btn.setStyleSheet(
                    themed_style(
                        "QPushButton{border:1px solid #30363d;background:#161b22;color:#c9d1d9;border-radius:6px;padding:4px}"
                    )
                )
    ...
    set_combo.addItem("자동 인식", AUTO_CHOICE)
    ...
    def sync_rows():
        ...
        for i in range(tape_row.count()):
            ...
            if w:
                w.setVisible(is_tape)

    drive_rb.toggled.connect(sync_rows)
    tape_rb.toggled.connect(sync_rows)
    sync_rows()
    ...
    if tape_rb.isChecked():
        set_name = (
            None
            if set_combo.currentData() == AUTO_CHOICE
            else self._combo_data_or_resolved_text(set_combo, self.all_set_names)
        )
        main_stat = (
            None
            if main_combo.currentData() == AUTO_CHOICE
            else self._combo_data_or_resolved_text(main_combo, main_pool)
        )
        return {
            "type": "tape",
            "set_name": set_name,
...
def confirm_identify_tape_main_stats(self, items, *, parent=None):
    ...
    main_pool = self._get_tape_main_stats_pool()
    dlg = QDialog(parent)
    dlg.setWindowTitle("카트리지 메인 스탯 확인")
    dlg.setMinimumSize(720, 420)
    dlg.setStyleSheet(current_style_sheet())
    layout = QVBoxLayout(dlg)
    layout.setSpacing(10)
    hint = QLabel("인식된 카트리지마다 메인 스탯을 확인하세요. 중국어 또는 병음으로 직접 입력해 검색할 수 있습니다.")
    ...
    for idx, item in enumerate(tapes, 1):
        ...
        label = QLabel(f"카트리지 {idx}  {item.set_name or ''}  {summary}")
        ...
        added = set()
        if current:
            combo.addItem(str(current), str(current))
            added.add(str(current))
        for stat_name in main_pool:
