...
class IdentificationController(IdentificationManualParsingMixin, QObject):
    ...
    def _on_capture_stop_hotkey(self) -> None:
        ...
        logger.warning(
            "중지 단축키 {}를 받았습니다. 연속 스크린샷 감정은 완료 단축키 {}로 돌아가세요.",
            configuration.stop,
    ...
    def _refresh_identify_options(self):
        ...
        for sid in sorted(
            ...
            self.ident_shape_combo.addItem(f"{sid} ({self._shape_areas.get(sid, 0)}칸)", sid)
    ...
    def _clear_identify_input(self):
        ...
        self.ident_summary.setText("장비 데이터 입력 대기 중")
    ...
    def _start_identify_capture_mode(self):
        ...
        QMessageBox.information(
            self._dialog_parent,
            "스크린샷 감정",
            f"OK를 클릭한 뒤 게임으로 돌아가세요.\n\n{hotkeys.capture}로 연속 스크린샷을 찍고, {hotkeys.finish}로 완료해 감정 페이지로 돌아옵니다.",
        )
        ...
        self.ident_summary.setText(
            f"스크린샷 감정 시작됨: {hotkeys.capture} 스크린샷, {hotkeys.finish} 완료"
        )
    ...
    def _capture_identify_foreground(self):
        ...
        try:
            ...
            logger.success(f"감정 스크린샷 성공: {path.name}")
            self.identify_capture_signal.emit(str(path))
        except Exception as e:
            logger.error(f"감정 스크린샷 실패: {e}")
            log_event(
                "ERROR",
    ...
    def _finish_identify_capture_mode(self):
        ...
        self.ident_summary.setText(f"감정 스크린샷 {count}장을 완료했습니다. 감정 시작을 클릭해 계속하세요.")
    ...
    def _identify_from_image_path(self):
        ...
        self._set_identify_busy(True, "이미지 분석 중...")
        worker = WorkerThread(
    ...
    def _on_identify_items_loaded(self, items):
        ...
        if not items:
            ...
            QMessageBox.warning(
                self._dialog_parent,
                "감정",
                "이미지에서 감정할 수 있는 드라이브나 카트리지를 인식하지 못했습니다.",
            )
            return
        if not self._confirm_identify_tape_main_stats(items):
            _identification_event(
                ...
                "사용자가 카트리지 메인 스탯 확인을 취소했습니다",
                item_count=len(items),
            )
            self.ident_summary.setText("감정을 취소했습니다")
            return
    ...
    def _start_identify_items(self, items):
        self._set_identify_busy(True, "캐릭터 청사진을 매칭하고 점수를 계산하는 중...")
        worker = WorkerThread(
