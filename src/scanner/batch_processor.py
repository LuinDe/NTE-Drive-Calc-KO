...
class BatchProcessor:
    ...
    def __init__(
        ...
        logger.info("=" * 60)
        logger.info("오프라인 일괄 처리 파이프라인 시작")
        logger.info("=" * 60)
    ...
    def process_all(self, filter_adjacent_duplicates: bool = False):
        if not os.path.exists(self.input_dir):
            raise InventoryEmptyError(f"스크린샷 폴더 {self.input_dir}을(를) 찾을 수 없습니다. 먼저 스캔을 실행하세요!")
        ...
        if total_files == 0:
            logger.info("수신함이 비어 있어 처리할 새 스크린샷이 없습니다.")
            return

        logger.info(f"\n처리되지 않은 스크린샷 {total_files}장을 발견해 분석을 시작합니다...\n")
        ...
        for idx, filename in enumerate(image_files, 1):
            ...
            try:
                ...
                logger.info(f"[{idx:04d}/{total_files:04d}] 분석: {cost:.2f}s | {filename}")
                if not added:
                    duplicate_count += 1
                    logger.info("      > 증분 중복 스크린샷을 걸러냈습니다\n")
                    continue

                if item_obj.item_type == "drive":
                    logger.info(f"      > [드라이브] | 형태: {item_obj.shape_id.ljust(8)} | 품질: {item_obj.quality}")
                else:
                    logger.info(
                        f"      > [카트리지] | 세트: {getattr(item_obj, 'set_name', '未知').ljust(8)} | 품질: {item_obj.quality}"
                    )

                logger.info(f"      > 메인 스탯: {item_obj.main_stats}")
                logger.info(f"      > 서브 스탯: {item_obj.sub_stats}\n")
                success_count += 1

            except Exception as e:
                logger.error(f"[{idx:04d}/{total_files:04d}] 분석 실패: {filename} | 오류: {str(e)}\n")

        cost_time = time.time() - start_time
        avg_time = cost_time / total_files if total_files else 0
        logger.success("=" * 60)
        if filter_adjacent_duplicates:
            logger.success(f"분석 완료. 이번에 장비 {success_count}개를 등록했고 연속 촬영 중복 의심 {duplicate_count}개를 걸러냈습니다.")
        else:
            logger.success(f"분석 완료. 이번에 장비 {success_count}개를 등록했습니다.")
        logger.success(f"총 소요 시간: {cost_time:.2f}초 (평균 {avg_time:.2f}초/장)")
        logger.success("=" * 60)
