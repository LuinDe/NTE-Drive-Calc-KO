...
ZERO_LEVEL_DETAIL_PAGE_RULES: tuple[GamepadVisionRule, ...] = (
    GamepadVisionRule("강화 버튼 흰색 비율", (0.69, 0.88, 0.98, 0.985), "white", "gt", 0.16, 185),
    GamepadVisionRule("빠른 추가 슬롯 회색 비율", (0.68, 0.70, 0.99, 0.88), "gray", "gt", 0.22),
    GamepadVisionRule("빠른 추가 슬롯 비강조", (0.68, 0.70, 0.99, 0.88), "white", "lt", 0.25, 185),
    GamepadVisionRule("상세 오른쪽 열이 가방 흰 배경이 아님", (0.68, 0.40, 0.97, 0.90), "white", "lt", 0.20, 185),
)

COMMON_DETAIL_PAGE_RULES: tuple[GamepadVisionRule, ...] = (
    GamepadVisionRule("상세 속성 영역 회색 비율", (0.68, 0.20, 0.98, 0.55), "gray", "gt", 0.25),
    GamepadVisionRule("상세 오른쪽 열이 가방 흰 배경이 아님", (0.68, 0.40, 0.97, 0.90), "white", "lt", 0.20, 185),
)
...
class GamepadVisionProfile:
    ...
    inventory_first_item_rules: tuple[GamepadVisionRule, ...] = (
        GamepadVisionRule("첫 칸 분홍색 선택 테두리", (0.04, 0.135, 0.155, 0.34), "pink", "gt", 0.008),
        GamepadVisionRule("오른쪽 속성 흰 배경", (0.68, 0.40, 0.97, 0.90), "white", "gt", 0.12, 165),
        GamepadVisionRule("상단 내비게이션 회색 비율", (0.15, 0.025, 0.58, 0.105), "gray", "gt", 0.08),
    )
...
def _format_vigem_error(exc: Exception) -> str:
    ...
    return (
        "ViGEmBus 가상 게임패드 드라이버가 준비되지 않아 전체 스캔을 시작할 수 없습니다.\n\n"
        "다음 순서로 처리하세요:\n"
        "1. 먼저 PC를 재시작한 뒤 이 프로그램을 다시 여세요.\n"
        "2. 그래도 오류가 나면 시작 메뉴의 NTE Drive Calc -> Install ViGEmBus Driver로 드라이버를 다시 설치/복구하세요.\n"
        "3. 복구 후 PC를 다시 재시작하세요.\n\n"
        f"원본 오류: {raw}"
    )
...
class GamepadScanner:
    ...
    def __init__(self, output_dir="scanned_images"):
        ...
        logger.info("가상 Xbox 360 게임패드에 연결하는 중...")
        try:
    ...
    def close(self):
        ...
        try:
            try:
                ...
                logger.debug(f"가상 게임패드 초기화 실패, 해제를 계속합니다: {exc}")
        finally:
            self.gamepad = None
            self._buttons = None
            del gamepad
            gc.collect()
            logger.info("가상 Xbox 360 게임패드를 해제했습니다")
    ...
    def _clear_output_images(self):
        ...
        for name in os.listdir(self.output_dir):
            ...
            if os.path.isfile(path) and os.path.splitext(name)[1].lower() in image_exts:
                ...
                removed += 1
        if removed:
            logger.info(f"전체 스캔 전에 이전 스크린샷 {removed}장을 정리했습니다.")
    ...
    def _commit_temp_output(self):
        ...
        logger.success(f"전체 스캔 스크린샷을 루트 디렉터리에 기록했습니다. 총 {moved}장.")
    ...
    def capture_panel(self, sct, counter):
        ...
        logger.info(f"[{counter:04d}] 캡처 성공")
        return filename
    ...
    def _activate_and_focus_last_inventory_equipment(self) -> None:
        ...
        profile = self._profile()
        logger.info("상태 동기화 전에 왼쪽 스틱 아래를 두 번 보냅니다: 게임패드를 활성화하고 마지막 줄 마지막 항목으로 이동.")
        for _ in range(2):
            self.push_left_joystick(
                0.0,
                -1.0,
    ...
    def _last_inventory_position(self, total_drives: int) -> tuple[int, int]:
        ...
        if total < 1:
            raise ValueError("전체 스캔 인벤토리 수량은 0보다 커야 합니다.")
    ...
    def _looks_like_detail_page(self, image: np.ndarray) -> tuple[bool, str]:
        ...
        for index, rules in enumerate(self._profile().vision.detail_page_rule_groups, start=1):
            ...
            group_reason = f"상세 규칙 그룹{index}: {reason}"
            if matched:
    ...
    def _wait_for_page_state(self, predicate, timeout_seconds: float, label: str) -> bool:
        ...
        stable = 0
        last_reason = "유효한 스크린샷을 얻지 못했습니다"
        with mss.MSS() as sct:
            while time.monotonic() < deadline and not self._stopped:
                ...
                if matched:
                    ...
                    if stable >= profile.page_state_stable_frames:
                        logger.info(f"페이지 인식 성공: {label} | {last_reason}")
                        return True
                else:
                    stable = 0
                time.sleep(profile.page_state_poll_seconds)
        logger.warning(f"페이지 인식 시간 초과: {label} | {last_reason}")
        return False
    ...
    def _refresh_to_first_item(self):
        logger.info("상세 페이지 복귀 위치 지정 전송: D -> Y -> 상세 페이지 대기 -> B -> 가방 첫 페이지 첫 칸 대기")
        self._apply_moves(["D"])
        time.sleep(self._profile().detail_refresh_move_settle_seconds)
        self._press_y()
        if not self._wait_for_detail_page():
            raise RuntimeError("Y를 누른 뒤 강화 상세 페이지를 감지하지 못해 잘못된 조작을 막기 위해 스캔 후 관리를 중지했습니다.")
        time.sleep(self._profile().page_state_detected_settle_seconds)
        self._press_b()
        if not self._wait_for_inventory_first_item():
            raise RuntimeError("B로 돌아간 뒤 가방 첫 페이지 첫 칸을 감지하지 못해 잘못된 조작을 막기 위해 스캔 후 관리를 중지했습니다.")
        time.sleep(self._profile().page_state_detected_settle_seconds)
    ...
    def sync_equipment_states(self, total_drives: int, state_changes: list[dict], action_mode: str = "default") -> int:
        ...
        try:
            ...
            logger.warning(
                f"장비 상태 {len(changes)}개를 역순으로 동기화할 준비가 되었습니다. 마지막 줄 마지막 항목에 위치했으니 게임 가방 화면을 그대로 두세요."
            )
            for change in changes:
                if self._stopped:
                    logger.warning(f"상태 동기화가 중지되었습니다. 이번에 대상 {applied}/{len(changes)}개를 처리했습니다.")
                    break
                ...
                logger.info(
                    f"상태 동기화 준비: raw_drive_{index:04d}"
                ...
                if changed:
                    applied += 1
                    logger.info(
                        f"상태 동기화 완료: raw_drive_{index:04d}"
                        f"{change.get('current_state')} -> {change.get('target_state')}"
                    )
            return applied
        finally:
    ...
    def start_scan(self, total_drives=None, on_capture=None, commit_on_complete=True):
        ...
        profile = self._profile()
        logger.warning("\n" + "=" * 50)
        logger.warning(f"가상 게임패드가 준비되었습니다. {profile.takeover_countdown_seconds:g}초 후 제어를 넘겨받으니 게임으로 돌아가세요")
        logger.warning("지금 첫 줄 첫 번째 드라이브/카트리지가 선택되어 있는지 확인하세요")
        logger.warning("=" * 50)
        time.sleep(profile.takeover_countdown_seconds)

        if total_drives is None:
            raise ValueError("전체 스캔은 먼저 인벤토리 수량을 입력해야 합니다.")
        total_drives = int(total_drives)
        if not 0 < total_drives <= self.MAX_INVENTORY_COUNT:
            raise ValueError(f"인벤토리 수량은 1~{self.MAX_INVENTORY_COUNT} 사이여야 합니다.")
        ...
        if self._stopped or captured_count != total_drives:
            ...
            logger.warning("전체 스캔이 완전히 끝나지 않아 임시 스크린샷으로 현재 루트 디렉터리를 대체하지 않았습니다.")
            return 0
        ...
        logger.success("\n" + "=" * 40)
        logger.success(f"스캔 완료, 장비 {total_drives}개를 처리했습니다.")
        logger.success("=" * 40)
