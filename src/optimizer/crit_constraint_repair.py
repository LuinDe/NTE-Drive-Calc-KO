    ...
    def _repair_failure_reason(
        ...
        if not observed:
            if floor > 0 and cap == float("inf"):
                return (
                    f"치명타 확률 최소값 {floor:g}%를 달성하는 방안이 없습니다"
                    "(이번 처음부터 재배치에서 고품질 완전 방안이 없습니다)"
                )
            if floor <= 0 and cap < float("inf"):
                return (
                    f"치명타 확률 상한 {cap:g}%를 만족하는 방안이 없습니다"
                    "(이번 처음부터 재배치에서 고품질 완전 방안이 없습니다)"
                )
            return (
                f"치명타 확률 구간 {floor:g}%~{cap:g}%를 동시에 만족하는 고품질 방안이 없습니다"
                "(이번 처음부터 재배치에서 고품질 완전 방안이 없습니다)"
            )
        highest = max(observed)
        lowest = min(observed)
        if highest + 1e-9 < floor:
            return f"치명타 확률 최소값 {floor:g}%를 달성하는 방안이 없습니다 (이번 처음부터 재배치 최고 {highest:g}%)"
        if lowest > cap + 1e-9:
            return f"치명타 확률 상한 {cap:g}%를 만족하는 방안이 없습니다 (이번 처음부터 재배치 최저 {lowest:g}%)"
        return (
            f"치명타 확률 구간 {floor:g}%~{cap:g}%를 동시에 만족하는 고품질 방안이 없습니다"
            f"(이번 처음부터 재배치 범위 {lowest:g}%~{highest:g}%)"
        )
