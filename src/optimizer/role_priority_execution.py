...
def execute_role_priority(
    ...
    for group_index, group in enumerate(priority_groups):
        ...
        if not reservations.can_consume(used_uids):
            ...
            logger.error("보존 복구가 채워 넣을 청사진을 반환하지 않았습니다: 캐릭터 그룹={}", ",".join(group))
            group_allocation = {
                role_name: {"valid": False, "reason": "앞선 동점 드라이브를 일대일로 채워 넣을 수 없습니다"}
...
def _choose_group_allocation(
    ...
    while _allocation_has_full_plan(allocation, group):
        ...
        if _allocation_is_reservation_feasible(strategy, allocation, group, reservations):
            _log_recovery(
                group, "점진 보호", ordinary_allocation, allocation,
...
def _choose_single_role_plan(
    ...
    while plan.get("valid"):
        ...
        if plan.get("valid") and reservations.can_consume(
            ...
            _log_recovery(
                [role_name], "점진 보호", {role_name: ordinary_plan},
...
def _log_recovery(
    ...
    delta = f"{before_score - after_score:.2f}" if before_key[0] == after_key[0] else "스탯 우선순위 변화"
    shape_counts: dict[str, int] = {}
    for uid in protected_uids:
        shape = reservations.shape_for_uid(uid)
        if shape is not None:
            shape_counts[shape] = shape_counts.get(shape, 0) + 1
    affected = "/".join(f"{shape}×{count}" for shape, count in sorted(shape_counts.items())) or "없음"
    constraints = _recovery_constraint_status(
        strategy, group, crit_priority_modes, crit_rate_caps,
    )
    logger.info(
        "동점 복구: 캐릭터/그룹={}, 경로={}, 일반 정렬 점수={:.2f}, 제출 정렬 점수={:.2f}, "
        "정렬 차이={}, 유효 차단 수={}, 보호 라운드={}, 영향받는 유형={}, 앞선 채워 넣기=성공, 치명타/스탯 제약={}",
...
def _recovery_constraint_status(
    ...
    values = []
    if crit_enabled:
        values.append("치명타 적용 및 통과")
    if stat_enabled:
        values.append("스탯 우선 적용 및 통과")
    if blacklist_enabled:
        values.append("스탯 블랙리스트 적용 및 통과")
    return "；".join(values) if values else "미적용"
...
def _final_group_rescue(
    ...
    if _allocation_is_reservation_feasible(strategy, recovered, group, reservations):
        logger.info("같은 등급 그룹 최종 일대일 분배 복구 완료: 캐릭터 수={}", len(group))
        return recovered
    constrained = any(
    ...
    if constrained:
        ...
        if repaired is not None:
            logger.info("같은 등급 그룹 최종 제약 복구 완료: 캐릭터 수={}", len(group))
            return repaired
    logger.info("같은 등급 그룹 최종 일대일 분배에 완전한 청사진 없음: 캐릭터 수={}", len(group))
    return {
        role_name: {"valid": False, "reason": "전체 가방으로는 청사진과 앞선 채워 넣기를 만족할 수 없습니다"}
        for role_name in group
    }
...
def _final_single_rescue(
    ...
    if best_plan is not None:
        logger.info("단일 캐릭터 최종 일대일 분배 복구 완료: 캐릭터={}", role_name)
        return best_plan
    if (
        strategy._crit_rate_cap(role_name, crit_rate_caps) is not None
        or strategy._crit_floor_threshold(role_config) is not None
    ...
    return {"valid": False, "reason": "전체 가방으로는 청사진과 앞선 채워 넣기를 만족할 수 없습니다"}
...
def _best_single_role_plan(
    ...
    reason = next(
        ...
        "사용 가능한 청사진이 없거나 청사진에 필요한 형태를 맞출 수 없습니다",
    )
