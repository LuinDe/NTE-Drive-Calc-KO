...
class RolePriorityStrategy(
    ...
    def _missing_drive_reason(
        ...
        if not same_shape:
            return f"후보 풀에 {shape_id} 드라이브가 부족합니다 (청사진을 채울 수 없음)"
        cap = self._crit_rate_cap(role_name, crit_rate_caps)
        if cap is not None:
            return f"치명타 확률 상한 {cap:g}% 때문에 {shape_id} 드라이브를 현재 조합에 넣을 수 없습니다"
        return f"{shape_id} 드라이브 후보가 청사진 제약을 만족하지 못합니다"
