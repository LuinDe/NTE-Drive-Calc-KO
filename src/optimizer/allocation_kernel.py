        ...
        for role in request.role_order:
            ...
            if not set_cores:
                plan["missing_core_reason"] = (
                    f"고정 스냅샷에 세트가 {target_set or '任意套装'}인 카트리지가 없습니다"
                )
                continue
            ...
            if not main_cores:
                plan["missing_core_reason"] = (
                    f"고정 스냅샷에 {target_set or '任意套装'}과(와)"
                    f"{'/'.join(allowed_mains)} 메인 스탯을 동시에 만족하는 카트리지가 없습니다"
                )
                continue
            screened = [
                tape
            ...
            if not screened:
                plan["missing_core_reason"] = (
                    "세트와 메인 스탯을 만족하는 카트리지가 모두 서브 스탯 또는 점수 등급 강제 필터에 걸렸습니다"
                )
                continue
            available = [
            ...
            if not available:
                plan["missing_core_reason"] = (
                    f"조건을 만족하는 유일 카트리지 {len(screened)}장이 이미 다른 캐릭터에게 분배되었습니다"
                )
            elif request.crit_rate_caps.get(role) is not None or request.property_limits.get(role):
                plan["missing_core_reason"] = (
                    "후보 카트리지가 치명타 또는 속성 상하한 제약을 통과하지 못했습니다"
                )
            else:
                plan["missing_core_reason"] = (
                    "후보 카트리지가 현재 캐릭터의 강제 필터 또는 유일 분배 제약을 통과하지 못했습니다"
                )
    ...
    def _diagnostic_reason(
        ...
        *,
        ...
        if missing:
            ...
            return (
                f"{shape} 드라이브 부족: 청사진에 최소 {required}개가 필요하지만"
                f"현재 사용 가능한 것은 {available_shapes.get(shape, 0)}개입니다 ({count}개 부족)"
            )

        for group in request.priority_groups or ():
            if role in group and len(group) > 1:
                return "같은 등급 그룹 경쟁 후 남은 후보 없음: 필요한 형태의 드라이브를 같은 등급 캐릭터가 사용 중"
        return "후보 드라이브가 청사진 형태 제약을 동시에 만족하지 못했습니다"
