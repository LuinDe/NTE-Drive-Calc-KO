...
def _reject_constant(_value: str) -> None:
    raise NativeAnalysisError("분석 코어가 유한하지 않은 JSON 수치를 반환했습니다")
...
def _json_object(payload: bytes) -> dict[str, Any]:
    if len(payload) > MAX_BYTES:
        raise NativeAnalysisError("분석 코어 응답이 크기 제한을 초과했습니다")
    try:
        value = json.loads(payload, parse_constant=_reject_constant, parse_float=_parse_finite_float)
    except (UnicodeError, ValueError) as error:
        raise NativeAnalysisError("분석 코어가 잘못된 JSON을 반환했습니다") from error
    if not isinstance(value, dict):
        raise NativeAnalysisError("분석 코어 응답은 객체여야 합니다")
    return value
...
class NteAnalysisCoreClient:
    ...
    def __init__(
        ...
        *,
        ...
        if self.executable.name.casefold() != "nte-analysis-core.exe":
            raise ValueError("분석 백엔드는 독립된 nte-analysis-core.exe를 사용해야 합니다")
        if not self.executable.is_file():
            raise NativeAnalysisError("독립 분석 코어가 없습니다. 분석 구성 요소를 다시 배포하세요")
        if not dataset_version.strip() or not math.isfinite(timeout) or timeout <= 0:
            raise ValueError("분석 데이터셋 버전과 유효한 시간 초과 값은 필수입니다")
        self.dataset_version = dataset_version
        if engine_version not in SUPPORTED_ENGINE_VERSIONS:
            raise ValueError("분석 코어 버전을 지원하지 않습니다")
    ...
    def from_executable(
        ...
        *, timeout: float = 120.0, cancelled: Callable[[], bool] | None = None,
        ...
        if (identity.get("engine") != "nte-analysis-core"
            ...
            raise NativeAnalysisError("독립 분석 코어의 식별 정보·버전 또는 기능이 잘못되었습니다")
    ...
    def _checkpoint(self, checkpoint: Callable[[], None] | None) -> None:
        if self.cancelled is not None and self.cancelled():
            raise NativeAnalysisCancelled("분석 요청이 취소되었거나 계정 컨텍스트가 더 이상 유효하지 않습니다")
    ...
    def _run(
        ...
        try:
            ...
            raise NativeAnalysisError("독립 분석 코어를 시작할 수 없습니다") from error
        ...
        try:
            while True:
                ...
                if remaining <= 0:
                    raise NativeAnalysisError("독립 분석 코어 계산 시간 초과")
                try:
        ...
        if returncode != 0:
            if returncode == 2 and len(stdout) <= MAX_BYTES:
                ...
                if (rejected.get("engine_version") == self.engine_version
                    ...
                    raise BuffProjectionBatchTooLarge("Buff 후보 계획이 코어 작업량 한도를 초과했습니다")
            # Do not surface raw stderr, request JSON or private values.
            raise NativeAnalysisError(
                f"독립 분석 코어가 요청을 거부했거나 실행에 실패했습니다 (종료 코드 {returncode})"
            )
    ...
    def version(self) -> dict[str, Any]:
        ...
        if value.get("engine") != "nte-analysis-core":
            raise NativeAnalysisError("독립 분석 코어 식별 정보가 일치하지 않습니다")
        ...
        if self.supports_battle_progress and "battle_progress_v1" not in value.get("capabilities", []):
            raise NativeAnalysisError("독립 분석 코어에 전투 리포트 진행률 기능이 없습니다")
    ...
    def load_battle_page_identity(
        ...
        if not self.supports_battle_page_identity:
            raise NativeAnalysisError("독립 분석 코어가 전투 리포트 입력 식별 정보 대조를 지원하지 않습니다")
        ...
        if (set(result) != {"input_digest"} or not isinstance(digest, str)
            ...
            raise NativeAnalysisError("독립 분석 코어의 전투 리포트 입력 식별 정보가 잘못되었습니다")
        return digest
    ...
    def load_battle_page(
        ...
        if len(payload) > MAX_BYTES:
            raise NativeAnalysisError("전투 리포트 페이지 입력이 크기 제한을 초과했습니다")
        ...
        if (response.get("schema_version") != RESPONSE_SCHEMA
            ...
            raise NativeAnalysisError("전투 리포트 페이지 응답의 고정 식별 정보가 일치하지 않습니다")
        kernel_ns = response.get("compute_elapsed_ns")
        if type(kernel_ns) is not int or kernel_ns < 0:
            raise NativeAnalysisError("전투 리포트 페이지 응답의 소요 시간이 잘못되었습니다")
        with self._lock:
    ...
    def evaluate(
        ...
        if len(jobs) > MAX_JOBS:
            raise ValueError("분석 배치 히트 수가 제한을 초과했습니다")
        identifiers = [row["job_id"] for row in jobs]
        if any(not isinstance(value, str) or not value for value in identifiers):
            raise ValueError("분석 작업 ID는 비어 있지 않은 문자열이어야 합니다")
        if len(set(identifiers)) != len(identifiers):
            raise ValueError("분석 작업 ID가 중복되었습니다")
        started = time.perf_counter()
        payload = json.dumps({
        ...
        if len(payload) > MAX_BYTES:
            raise ValueError("분석 요청이 크기 제한을 초과했습니다")
        raw = self._run(payload, checkpoint=checkpoint)
        response = _json_object(raw)
        if (
            ...
            raise NativeAnalysisError("독립 분석 코어 응답의 버전 또는 데이터셋이 일치하지 않습니다")
        results = response.get("results")
        if not isinstance(results, list) or len(results) != len(jobs):
            raise NativeAnalysisError("독립 분석 코어 응답의 작업 수가 일치하지 않습니다")
        for index, row in enumerate(results):
            if not isinstance(row, dict) or row.get("job_id") != identifiers[index]:
                raise NativeAnalysisError("독립 분석 코어 응답의 작업 순서 또는 식별 정보가 일치하지 않습니다")
            ...
            if jobs[index].get("candidate") is not None:
                ...
                raise NativeAnalysisError("독립 분석 코어가 요청하지 않은 후보를 반환했습니다")
            ratio = row.get("ratio")
            state = row.get("ratio_status")
            if state not in {"complete", "unavailable", "not_requested"}:
                raise NativeAnalysisError("독립 분석 코어가 잘못된 비율 상태를 반환했습니다")
            if state == "complete":
                if not self._finite_number(ratio) or ratio < 0:
                    raise NativeAnalysisError("독립 분석 코어가 잘못된 비율을 반환했습니다")
            elif ratio is not None:
                raise NativeAnalysisError("미정량화 결과는 비율을 가질 수 없습니다")
        kernel_ns = response.get("compute_elapsed_ns", 0)
        if not isinstance(kernel_ns, int) or kernel_ns < 0:
            raise NativeAnalysisError("독립 분석 코어가 잘못된 소요 시간을 반환했습니다")
        with self._lock:
    ...
    def _validate_numeric(cls, row: object) -> None:
        if not isinstance(row, dict) or row.get("status") not in {"complete", "partial", "unavailable"}:
            raise NativeAnalysisError("독립 분석 코어가 잘못된 공식 상태를 반환했습니다")
        if not isinstance(row.get("gap_codes"), list):
            raise NativeAnalysisError("독립 분석 코어 응답에 근거 갭 정보가 없습니다")
        ...
        for key in fields:
            if key not in row or (row[key] is not None and not cls._finite_number(row[key])):
                raise NativeAnalysisError("독립 분석 코어가 잘못된 공식 수치를 반환했습니다")
        if row["status"] == "unavailable":
            if any(row[key] is not None for key in fields) or not row["gap_codes"]:
                raise NativeAnalysisError("계산 불가 공식은 피해를 가질 수 없습니다")
        elif row["non_critical_damage"] is None or not isinstance(row.get("factors"), dict):
            raise NativeAnalysisError("계산 가능 공식에 수치 또는 곱연산 구간이 없습니다")
    ...
    def project_plan_batch(
        ...
        if not self.supports_projection_plan:
            raise NativeAnalysisError("배포된 분석 구성 요소가 일괄 후보 투영을 지원하지 않습니다")
    ...
    def _project_batch(
        ...
        if (len(jobs) > MAX_JOBS or len(set(identifiers)) != len(jobs)
            ...
            raise ValueError("Buff 투영 배치 식별 정보 또는 크기가 잘못되었습니다")
        self._checkpoint(checkpoint)
        if not jobs:
            return ()
        started = time.perf_counter()
        ...
        if len(raw) > MAX_BYTES:
            raise BuffProjectionBatchTooLarge("Buff 투영 응답이 크기 제한을 초과했습니다")
        ...
        try:
            ...
            raise NativeAnalysisError("Buff 투영 응답의 공유 근거 테이블이 잘못되었습니다") from error
        kernel_ns = response.get("compute_elapsed_ns")
        encoding_ns = response.get("encoding_elapsed_ns", 0)
        evaluated_pairs = response.get("evaluated_interval_pairs", 0)
        del response
        if not isinstance(results, list) or len(results) != len(jobs):
            raise NativeAnalysisError("Buff 투영 응답 수가 일치하지 않습니다")
        projections = []
        validated_modifiers: set[int] = set()
        validated_decisions: set[int] = set()
        for index, row in enumerate(results):
            if not isinstance(row, dict) or row.get("job_id") != identifiers[index]:
                raise NativeAnalysisError("Buff 투영 응답의 식별 정보 또는 순서가 일치하지 않습니다")
    ...
    def _validate_projection(cls, value: object, validated_modifiers: set[int] | None = None,
        ...
        if not isinstance(value, dict) or not isinstance(value.get("event_id"), str):
            raise NativeAnalysisError("Buff 투영 응답 형식이 잘못되었습니다")
        if (not isinstance(value.get("confidence"), str)
            ...
            raise NativeAnalysisError("Buff 투영 응답 근거가 잘못되었습니다")
        for row in value["modifiers"]:
            ...
            if (not isinstance(row, dict)
                ...
                raise NativeAnalysisError("Buff 투영 응답 속성이 잘못되었습니다")
