...
def decode_progress(line: bytes) -> dict[str, Any] | None:
    if len(line) > MAX_PROGRESS_BYTES:
        raise NativeStreamError("독립 분석 코어 진행률 이벤트가 크기 제한을 초과했습니다")
    ...
    try:
        ...
        raise NativeStreamError("독립 분석 코어 진행률 프로토콜이 유효하지 않습니다") from None
    if (not isinstance(value, dict)
        ...
        raise NativeStreamError("독립 분석 코어 진행률 프로토콜이 유효하지 않습니다")
    ...
    if not (completed is None and total is None):
        if (type(completed) is not int or type(total) is not int
            ...
            raise NativeStreamError("독립 분석 코어 진행률 카운트가 유효하지 않습니다")
    return value
    ...
    try:
        ...
        while len(finished) != 2 or process.poll() is None:
            ...
            try:
                ...
                continue
            if kind == "error":
                raise NativeStreamError("독립 분석 코어 파이프 읽기 실패")
            if chunk is None:
                finished.add(kind)
                ...
                if len(output) + len(chunk) > max_output_bytes:
                    raise NativeStreamError("분석 코어 응답이 크기 제한을 초과했습니다")
                output.extend(chunk)
            else:
