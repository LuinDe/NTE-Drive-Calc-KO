...
class MouseEquipmentStateSync:
    ...
    def sync(self, total_items: int, state_changes: list[dict[str, Any]]) -> MouseStateSyncResult:
        ...
        try:
            ...
            if self.scanner._wait_for_selected_panel(initial_last_slot, 0) is None:
                raise RuntimeError("상태 관리 시작 시 게임이 스캔 종료 시점의 마지막 드라이브에 머물러 있지 않았습니다")
            for change in changes:
                ...
                if verified != target:
                    raise RuntimeError(
                        f"{slot.index}번째 상태 전환 후 재검토 실패:"
                        f"기대 {target}, 화면 {verified}"
                    )
                applied += 1
    ...
    def _select_slot(self, frame: Any, slot: Any) -> Any:
        ...
        if selected is None:
            raise RuntimeError(f"{slot.index}번째 상태 관리 위치 지정 후 상세 패널이 안정되지 않았습니다")
        return selected
        ...
        if target == "discarded":
            ...
            if current == "locked":
                ...
                if not self.popup_detector(frame.image):
                    raise RuntimeError("잠금에서 폐기로 전환 후 확인 팝업을 감지하지 못했습니다")
                ...
                if self.popup_detector(frame.image):
                    raise RuntimeError("잠금에서 폐기로 전환 후 확인 팝업이 사라지지 않았습니다")
                return frame
            else:
    ...
    def _write_report(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"마우스 상태 동기화 진단 보고서 기록 실패: {type(exc).__name__}")
    ...
    def _check_stopped(self) -> None:
        if self.scanner._stopped:
            raise RuntimeError("마우스 상태 동기화가 중지되었습니다")
