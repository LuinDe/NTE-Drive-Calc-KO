# 判断截图对应的装备类型。
"""Qt-independent helpers for classifying screenshots as drive or tape."""
...
def classify_item(processor, img, region_profiles):
    ...
    for profile_name, regions in region_profiles:
        ...
        if (
            ...
            logger.debug(
                f"후보 좌표[{profile_name}] shape={shape_box}"
                f"고신뢰 형태={shape_res['shape_id']}({shape_res['confidence']}), 신원 OCR 건너뜀"
            )
        ...
        logger.debug(
            f"후보 좌표[{profile_name}] identity={id_box} shape={shape_box}"
