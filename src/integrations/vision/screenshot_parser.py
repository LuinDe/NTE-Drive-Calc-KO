...
from src.integrations.vision.equipment_classifier import classify_item, locate_selected_reward_shape
from src.integrations.vision.identification_parser import detect_tape_identity_from_lines
...
def process_single_image(processor, image_path: str):
    ...
    if img is None:
        raise ValueError("이미지가 손상되었거나 읽을 수 없습니다")
    ...
    logger.debug(
        f"스크린샷 좌표 방안: {profile_name} | 크기: {width}x{height} |"
        f"유형: {item_type} | 형태: {shape_res['shape_id']}({shape_res['confidence']}) |"
        f"신원 텍스트: {hub_joined_text}"
    )

    if item_type == "drive":
        if shape_res["shape_id"] == "Unknown" or shape_res["confidence"] < 0.7:
            raise ValueError(f"형태 인식 신뢰도 부족: {shape_res['confidence']}")
...
def _prefer_complete_or_reward_item(processor, img, item, filename: str):
    ...
    try:
        ...
        logger.debug(f"보상 장면 분석 미적중: {filename} | {exc}")
    return item
...
def process_reward_scene(processor, img, filename: str = ""):
    ...
    raise ValueError("보상 장면을 인식하지 못했습니다")
...
def _detect_reward_scene_type(processor, lines: list[dict]) -> str | None:
    ...
    if "卡带" in compact:
        return "tape"
    set_name, _main_stat = detect_tape_identity_from_lines(processor, lines)
    if set_name:
        return "tape"
    return None
