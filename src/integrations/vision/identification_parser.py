...
def parse_identify_items(
    ...
    if forced_type is not None:
        try:
            ...
            logger.debug(f"표준 영역 강제 감정 분석 미적중, 전체 이미지 인식으로 전환: {exc}")
    else:
        try:
            ...
            logger.debug(f"표준 영역 감정 분석 미적중, 전체 이미지 인식으로 전환: {exc}")
    ...
    try:
        ...
        if forced_type == "tape":
            effective_forced_set_name, effective_forced_main_stat = detect_tape_identity_from_lines(
                processor,
                lines,
                forced_set_name=effective_forced_set_name,
                forced_main_stat=effective_forced_main_stat,
        ...
        logger.debug(f"전체 이미지 감정 분석 실패: {exc}")
...
def detect_tape_identity_from_lines(
    processor,
    lines: list[dict],
    forced_set_name=None,
    forced_main_stat=None,
):
    """Resolve tape set and main stat from OCR lines for all import routes."""
...
def process_identify_standard_forced(
    ...
    if forced_type == "drive":
        if not forced_shape_id:
            raise ValueError("드라이브 형태를 선택하지 않음")
    ...
    raise ValueError(f"알 수 없는 감정 유형: {forced_type}")
...
def synthesize_identify_cluster(
    ...
    if shape["shape_id"] == "Unknown":
        logger.debug(f"감정 후보에서 형태를 찾지 못해 건너뜁니다: {texts}")
        return None
...
def load_existing_inventory_signatures(processor) -> set[str]:
    ...
    if not processor.replace_output and processor.output_file and os.path.exists(processor.output_file):
        try:
            ...
            logger.debug(f"기존 인벤토리 서명 읽기 실패, 창고 바닥 첫 이미지 인벤토리 중복 제거를 건너뜁니다: {exc}")
