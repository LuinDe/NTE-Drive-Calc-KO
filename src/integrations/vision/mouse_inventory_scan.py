    ...
    def _bottom_aligned_final_slots(
        ...
        for index in range(int(first_index), int(total_items) + 1):
            ...
            if not 0 <= row < self.visible_rows:
                raise ValueError("마지막 페이지 계획이 보이는 인벤토리 격자를 벗어났습니다")
            slots.append(
...
class MouseInventoryScanner:
    ...
    def __init__(
        ...
        *,
        ...
        if self.input_speed_profile == self.INPUT_SPEED_PROFILE:
            ...
            raise ValueError(f"지원하지 않는 마우스 스캔 입력 profile: {input_speed_profile}")
    ...
    def emergency_stop(self) -> None:
        ...
        try:
            ...
            logger.warning("마우스 전체 비전 스캔이 중지 명령을 받았습니다")
    ...
    def start_scan(
        ...
        if total_drives is None:
            raise ValueError("마우스 전체 스캔은 먼저 인벤토리 수량을 입력해야 합니다.")
        ...
        if not preflight.valid:
            ...
            raise MouseInventoryScanError(
                "첫 프레임 드라이브 격자 사전 검사 실패:"
                f"있어야 할 칸 중 {preflight.matched_slots}/{preflight.checked_slots}개만 일치했습니다."
                "드라이브 목록이 열려 있고 맨 위로 스크롤되어 있으며 화면이 가려지지 않았는지 확인하세요"
            )
        logger.info(
            "마우스 전체 스캔 첫 프레임 격자 사전 검사 통과: resolution={}x{} slots={} min_warm={:.3f}",
            preflight.width,
            preflight.height,
            ...
            while captured < total:
                ...
                try:
                    ...
                    if occupancy.has_gap:
                        raise MouseInventoryScanError("현재 페이지 드라이브 칸이 연속되지 않아 누락을 막기 위해 스캔을 중지합니다")
                    expected_visible = len(slots)
                    if is_final_page and occupied_slots != expected_visible:
                        raise MouseInventoryScanError(
                            "마지막 페이지 위치가 맞지 않아 클릭 전에 중지했습니다:"
                            f"감지 {occupied_slots}, 계획 {expected_visible}"
                        )
                    if occupied_slots < expected_visible:
                        raise MouseInventoryScanError(
                            "현재 페이지의 실제 드라이브 칸 수가 남은 수량과 일치하지 않습니다:"
                            f"감지 {occupied_slots}, 최소 {expected_visible}이어야 함"
                        )
        ...
        if self._stopped or captured != total:
            logger.warning(f"마우스 전체 스캔 불완전: captured={captured} expected={total}")
            self._write_scan_report(
                telemetry,
    ...
    def _write_scan_report(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"마우스 스캔 진단 보고서 기록 실패: {type(exc).__name__}")
    ...
    def _capture_slot(self, slot: MouseScanSlot) -> MouseCapturedFrame:
        ...
        for attempt in range(2):
            if self._stopped:
                raise MouseInventoryScanError("스캔이 중지되었습니다")
            ...
            if stable is not None:
                return stable
            logger.warning(f"{slot.index}번째 상세가 안정되지 않아 클릭을 재시도합니다 ({attempt + 1}/2)")
        raise MouseInventoryScanError(f"{slot.index}번째 클릭 후 안정된 상세 패널을 감지하지 못했습니다")
    ...
    def _capture_frame(self, *, freeze: bool = False) -> MouseCapturedFrame:
        ...
        if frame.image.shape[0] != frame.rect.height or frame.image.shape[1] != frame.rect.width:
            raise MouseInventoryScanError("스크린샷 크기가 고정된 클라이언트 영역과 일치하지 않습니다")
        ...
        if int(frame.hwnd) != int(self._target_hwnd or 0):
            raise MouseInventoryScanError("스캔 중 전경 창이 바뀌었습니다")
        if frame.rect != self._target_rect:
            raise MouseInventoryScanError("스캔 중 게임 클라이언트 영역의 크기나 위치가 바뀌었습니다")
        return frame
    ...
    def _save_frame(self, frame: MouseCapturedFrame, index: int) -> str:
        ...
        if not ok:
            raise MouseInventoryScanError(f"{index}번째 스크린샷 인코딩 실패")
        encoded.tofile(path)
        return str(path)
