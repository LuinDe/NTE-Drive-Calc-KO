# 过滤截图解析结果中的重复装备。
"""Qt-independent helpers for adjacent-screenshot duplicate filtering."""
    ...
    *,
    ...
    if recognized_count < 4:
        if recognized_count == 3:
            raise RecoverableParseError("유효한 서브 스탯 3개를 인식했으며 사용자의 보완 입력 1개를 기다립니다", item_data, recognized_count)
        raise ValueError("유효한 장비 데이터를 인식하지 못했습니다")
