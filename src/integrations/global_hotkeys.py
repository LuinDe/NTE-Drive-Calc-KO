...
class GlobalHotkeyManager:
    ...
    def update_configuration(
        ...
        *,
        ...
        if not all((
            ...
            raise ValueError("전역 단축키는 비워 둘 수 없습니다")
        with self._lock:
    ...
    def start(
        ...
        *,
        ...
        if not owner_name:
            raise ValueError("단축키 세션 owner는 비워 둘 수 없습니다")
        self.stop()
    ...
    def stop(self, *, owner: str | None = None) -> None:
        ...
        if sys.platform == "win32" and thread_id:
            try:
                ...
                logger.debug("전역 단축키 스레드 깨우기 실패, 스레드가 이미 종료되었을 수 있습니다: {}", exc)
        if (
            thread is not None
    ...
    def _dispatch(self, generation: int, action: HotkeyAction) -> None:
        ...
        if callback is not None:
            try:
                ...
                logger.exception("전역 단축키 동작 실행 실패 action={}: {}", action, exc)
    ...
    def _keyboard_poll_loop(self, generation: int) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.debug("전역 단축키 폴링 예외, 감시를 계속합니다: {}", exc)
            time.sleep(0.05)
