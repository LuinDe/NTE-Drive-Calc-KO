...
def collect_windows_capture_support(
    *,
    ...
    if os.name != "nt":
        return {"supported": False, "reason": "Windows 패킷 캡처 환경 진단만 지원합니다"}
...
def _probe_json(script: str, runner: CommandRunner) -> dict[str, Any]:
    ...
    try:
        ...
        return {"state": "query_error", "error": "Windows 조회가 해석할 수 없는 결과를 반환했습니다"}
    if not isinstance(payload, (Mapping, list)):
        return {"state": "query_error", "error": "Windows 조회가 잘못된 객체를 반환했습니다"}
...
def _runtime_libraries(core_executable: Path, npcap_dir: Path, system_root: Path) -> list[dict[str, Any]]:
    locations = (
        ("코어 디렉터리", core_executable.parent),
        ("Npcap 시스템 디렉터리", system_root / "System32" / "Npcap"),
        ("System32", system_root / "System32"),
        ("Npcap 설치 디렉터리", npcap_dir),
    )
...
def _compact_error(value: str) -> str:
    return " ".join(value.split())[:240] or "Windows 조회 실패"
